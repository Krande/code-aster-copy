...
MODELE = AFFE_MODELE(
    MAILLAGE=MA, AFFE=(_F(GROUP_MA="Hexa27", PHENOMENE="MECANIQUE", MODELISATION="3D_INTSOLPIEU"),)
)
...
FONCT = DEFI_FONCTION(NOM_PARA="INST", VALE=[item for pair in zip(l_inst, l_valy) for item in pair])

LINST = DEFI_LIST_REEL(DEBUT=0.0, INTERVALLE=(_F(JUSQU_A=4.0, NOMBRE=4),))
...
RES = STAT_NON_LINE(
    ...
    COMPORTEMENT=_F(RELATION="SP_CINE", GROUP_MA=Grma27, RESI_INTE=1e-8, ITER_INTE_MAXI=15),
...
assert len(l_inst) == len(tabval2["INST"])
