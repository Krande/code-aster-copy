...
def interp(typ_i, val, x1, x2, y1, y2, tol=1.0e-6):
    """Interpolation linéaire/logarithmique entre un couple de valeurs"""
    if abs(val - x1) < tol * abs(x2 - x1):
        return y1
    if abs(val - x2) < tol * abs(x2 - x1):
        return y2
    if typ_i[0] == "LOG" and (x1 <= 0.0 or x2 <= 0.0):
        raise AsterError(
            "FONCT0_27", "Interpolation LOG invalide sur l'intervalle [%g, %g]." % (x1, x2)
        )
    if typ_i[1] == "LOG" and (y1 <= 0.0 or y2 <= 0.0):
        raise AsterError(
            "FONCT0_27", "Interpolation LOG invalide sur les ordonnées [%g, %g]." % (y1, y2)
        )
    if typ_i == ["LIN", "LIN"]:
    ...
    raise AsterError("FONCT0_27", "abscisse = %g, intervalle = [%g, %g]" % (val, x1, x2))


def is_ordo(liste):
    if len(liste) > 1:
        val = NP.array(liste, float)
        return min(val[1 : len(val)] - val[0 : len(val) - 1]) >= 0.0
    else:
        return True
...
class t_fonction:
    ...
    def __init__(self, vale_x, vale_y, para, nom=""):
        ...
        if para["INTERPOL"] not in [
            ...
            raise_function_error("fonction : parametre INTERPOL incorrect : %s" % para["INTERPOL"])
        if para["PROL_DROITE"] not in ["EXCLU", "CONSTANT", "LINEAIRE"]:
            raise_function_error(
                "fonction : parametre PROL_DROITE incorrect : %s" % para["PROL_DROITE"]
            )
        if para["PROL_GAUCHE"] not in ["EXCLU", "CONSTANT", "LINEAIRE"]:
            raise_function_error(
                "fonction : parametre PROL_GAUCHE incorrect : %s" % para["PROL_GAUCHE"]
            )
    ...
    def __add__(self, other):
        ...
        if isinstance(other, t_fonction):
            ...
            raise_function_error(
                "fonctions : erreur de type dans __add__ : %s %s" % (self, type(other))
            )
    ...
    def __mul__(self, other):
        ...
        if isinstance(other, t_fonction):
            ...
            raise_function_error(
                "fonctions : erreur de type dans __mul__%s %s" % (self, type(other))
            )
        ...
        if n == 1:
            ...
            if abs(val - self.vale_x[0]) <= tol * abs(self.vale_x[0]):
                ...
                if (self.para["PROL_DROITE"] == "CONSTANT") or (
                    self.para["PROL_DROITE"] == "LINEAIRE"
                ):
                    return self.vale_y[0]
            else:
                if (self.para["PROL_GAUCHE"] == "CONSTANT") or (
                    self.para["PROL_GAUCHE"] == "LINEAIRE"
                ):
                    return self.vale_y[0]
            raise AsterError(
                "FONCT0_29", "fonction évaluée hors du domaine de définition, para=%f" % val
            )
        # i = 0, val <= val_min
        if i == 0:
    ...
    def trapeze(self, coef):
        ...
        trapz[0] = coef
        trapz[1:n] = (
            (self.vale_y[1:n] + self.vale_y[:-1]) / 2 * (self.vale_x[1:n] - self.vale_x[:-1])
        )
    ...
    def simpson(self, coef):
        ...
        for i in range(2, len(self.vale_x)):
            ...
            fb = self.vale_y[i]
            deltah = h2 - h1
            if h1 == 0.0 or h2 == 0.0 or abs(deltah / h1) <= eps:
                ct = (1.0, 4.0, 1.0)
            else:
                epsi = deltah / (h1 * h2)
                ct = (1.0 - epsi * h2, 2.0 + epsi * deltah, 1.0 + epsi * h1)
            tabl[iperm] = tabl[iperm] + (bma / 6.0) * (ct[0] * fa + ct[1] * fm + ct[2] * fb)
    ...
    def fft(self, methode):
        ...
        if self.para["NOM_PARA"] != "INST":
            raise AsterError(
                "FONCT0_28", "fonction réelle : FFT : NOM_PARA=INST pour une transformée " "directe"
            )
...
class t_fonction_c(t_fonction):
    ...
    def __repr__(self):
        ...
        for i in range(len(self.vale_x)):
            texte.append(
                "%f %f + %f .j" % (self.vale_x[i], self.vale_y[i].real, self.vale_y[i].imag)
            )
        return "\n".join(texte)
    ...
    def fft(self, methode, syme):
        ...
        if self.para["NOM_PARA"] != "FREQ":
            raise AsterError(
                "FONCT0_28",
                "fonction complexe : FFT : NOM_PARA=FREQ pour une transformée " "directe",
            )
        pas = self.vale_x[1] - self.vale_x[0]
        for i in range(1, len(self.vale_x)):
            ecart = NP.abs(((self.vale_x[i] - self.vale_x[i - 1]) - pas) / pas)
            if ecart > 1.0e-3:
                raise_function_error(
                    "fonction complexe : FFT : la fonction doit etre à pas constant"
                )
        n = get_len_puis2(self.vale_x)
        if syme == "OUI":
...
class t_nappe:
    ...
    def __init__(self, vale_para, l_fonc, para, nom=""):
        ...
        if para["INTERPOL"] not in [
            ...
            raise_function_error("nappe : parametre INTERPOL incorrect : %s" % para["INTERPOL"])
        if para["PROL_DROITE"] not in ["EXCLU", "CONSTANT", "LINEAIRE"]:
            raise_function_error(
                "nappe : parametre PROL_DROITE incorrect : %s" % para["PROL_DROITE"]
            )
        if para["PROL_GAUCHE"] not in ["EXCLU", "CONSTANT", "LINEAIRE"]:
            raise_function_error(
                "nappe : parametre PROL_GAUCHE incorrect : %s" % para["PROL_GAUCHE"]
            )
        ...
        if len(l_fonc) != len(vale_para):
            raise_function_error(
                "nappe : nombre de fonctions différent du nombre de valeurs du paramètre"
            )
        for f in l_fonc:
        ...
        if n == 1:
            ...
            if abs(val1 - self.vale_para[0]) <= tol * abs(self.vale_para[0]):
                ...
                if (self.para["PROL_GAUCHE"] == "CONSTANT") or (
                    ...
                    return self.l_fonc[0](val2)
            raise AsterError(
                "FONCT0_29", "nappe évaluée hors du domaine de définition, para=%f" % val1
            )
        # i = 0, val1 <= val_min
        if i == 0:
            ...
            if abs(eps_g) <= tol:
                ...
                if val1 < self.vale_para[0]:
                    ...
                    raise AsterError("FONCT0_28", "valeur inattendue, para=%f" % val1)
        # i = n, val1 > val_max
        elif i == n:
    ...
    def __add__(self, other):
        ...
        if isinstance(other, t_nappe):
            if NP.all(self.vale_para != other.vale_para):
                raise AsterError("FONCT0_28", "nappes à valeurs de paramètres différentes")
            ...
            raise_function_error(
                "t_nappe : erreur de type dans __add__ : %s %s" % (other, type(other))
            )
        return t_nappe(self.vale_para, l_fonc, self.para)
    ...
    def __mul__(self, other):
        ...
        if isinstance(other, t_nappe):
            ...
            raise_function_error(
                "t_nappe : erreur de type dans __mul__ : %s %s" % (other, type(other))
            )
        return t_nappe(self.vale_para, l_fonc, self.para)
    ...
    def extreme(self):
        ...
        for i in range(len(self.vale_para)):
            for j in range(len(self.l_fonc[i].vale_y)):
                ...
                if y == val_min:
                    vm["min"].append([y, self.l_fonc[i].vale_x[j], self.vale_para[i]])
                if y == val_max:
                    vm["max"].append([y, self.l_fonc[i].vale_x[j], self.vale_para[i]])
        vm["min"].sort()
        vm["max"].sort()
        for item in vm["min"]:
            item.reverse()
...
def get_len_puis2(tab_in):
    ...
    return int(log(len(tab_in)) / log(2.0))
