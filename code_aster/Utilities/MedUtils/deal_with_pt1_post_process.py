...
def operate(isTopCall, effective_dest_file_name, nb_dest_par, origin_seq):
    ...
    for fn in effective_dest_file_name:
        ...
        if not m:
            logger.warning(
                "Le fichier MED {} du pattern de destination : impossible d extraire le proc Id !".format(
                    fn
                )
            )
            sys.exit(1)
    ...
    if set([nb for _, nb in dest_fns]) != set(range(nb_dest_par)):
        logger.error(
            "Il y a un probleme dans la detection des procIds (sur les entiers dans les noms de fichiers)"
        )
        sys.exit(1)
    ...
    orig_pt1 = mc.MEDCoupling1SGTUMesh(
        mm_orginal[-meshDim]
    ).getNodalConnectivity()  # -mm_orginal.getMeshDimension() pour dire que l'on prend les MED_POINT1
    ...
    for dest_fn, dest_proc in dest_fns:
        ...
        if -meshDim in dest_mm.getNonEmptyLevels():
            ...
            if not pt1_to_be_present.buildIntersection(pt1_already_present).isEqual(
                pt1_already_present
            ):
                raise RuntimeError(
                    "Les POINT1 dans {} ne sont pas exactement dans ceux attendus de l'origine !".format(
                        fn
                    )
                )
        ...
        if pt1_to_add.empty():
            logger.info(
                "Pour le proc {} il y a {} MED_POINT1 et rien à faire !".format(
                    dest_proc, len(pt1_already_present)
                )
            )
            continue
        ...
        logger.info(
            "Pour le proc {} on rajoute {} MED_POINT1 (avant {} apres {})".format(
                dest_proc, len(pt1_to_add), len(pt1_already_present), len(pt1_to_be_present)
            )
        )
        # on rajoute le mesh
        ...
        pt1_after = mc.MEDCoupling1SGTUMesh.Merge1SGTUMeshesOnSameCoords(
            [elt for elt in [pt1_cur_mesh, pt1_to_add_mesh] if elt is not None]
        )
        pt1_after.setName(dest_mm.getName())
        pt1_fam = mc.DataArrayInt.Aggregate(
            [
                elt
                for elt in [pt1_fam_before, fam_orig_pt1[pt1_entry_to_be_present]]
                if elt is not None
            ]
        )
        pt1_num = None
        if pt1_num_before:
            pt1_num = mc.DataArrayInt.Aggregate(
                [pt1_num_before, num_orig_pt1[pt1_entry_to_be_present]]
            )
        ...
        logger.warning(
            "On reecrit le proc {} avec les {} MED_POINT1 dans {}".format(
                dest_proc, len(pt1_to_be_present), dest_fn
            )
        )
    ...
    logger.warning(
        "Des noeuds (au nombre de {}) ont été oubliés ainsi que les cellules MED_POINT1 dessus ! On va les rajouter dans le fichier {}".format(
            len(pt1_simply_ignored), filename_with_pts_to_add
        )
    )
    ...
    if node_num:
        node_num = mc.DataArrayInt.Aggregate(
            [node_num, mm_orginal.getNumberFieldAtLevel(1)[pt1_simply_ignored]]
        )
    if node_fam:
        node_fam = mc.DataArrayInt.Aggregate(
            [node_fam, mm_orginal.getFamilyFieldAtLevel(1)[pt1_simply_ignored]]
        )
    if node_glob:
    ...
    logger.warning(
        "Les {} noeuds ont ete rajouter dans ! On va lui rajouter les MED_POINT1 dessus".format(
            len(pt1_simply_ignored), filename_with_pts_to_add
        )
    )
...
def add_pt1(args):
    ...
    if args["nb_dest_par"] < 2:
        logger.warning(
            "Le nb de proc de destination {} est < 2 : Rien à faire".format(args["nb_dest_par"])
        )
        sys.exit(0)

    logger.info(
        'Le fichier MED original depuis lequel on prend les MED_POINT1 : "{}"'.format(
            args["origin_seq"]
        )
    )
    ...
    if len(effective_dest_file_name) != args["nb_dest_par"]:
        logger.error(
            "Le nombre de fichiers MED dans le pattern {} ({}) est different de celui attendu {}".format(
                args["dest_par"], len(effective_dest_file_name), args["nb_dest_par"]
            )
        )
        sys.exit(1)

    logger.info(
        'Les fichiers MED destinations dans lesquels on va rajouter les MED_POINT1 : "{}"'.format(
            effective_dest_file_name
        )
    )

    if not args["force"]:
        res = input(
            "Confirmez vous l'ecriture dans les fichiers {}. Taper 0 pour arreter".format(
                effective_dest_file_name
            )
        )
        if res == "0":
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        dest="origin_seq",
        required=True,
        help="origin sequential med file path",
    )
    parser.add_argument(
        "-dest",
        "--dest_par",
        dest="dest_par",
        required=True,
        help="destination parallel med file pattern",
    )
    parser.add_argument(
        "-nb_dest",
        "--nb_dest_par",
        ...
        type=int,
        required=True,
        help="nb of proc destination parallel med file pattern. Just for safety",
    )
    parser.add_argument(
        "-v",
        "--verbose",
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="No question, file in destination parallel will be overwritten.",
    )
