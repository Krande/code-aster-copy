...
class sd_type_flui_stru(AsBase):
    ...
    def check_FSVI(self, checker):  # objet .FSVI
        ...
        if type_faisceau == 2:  # GRAPPE
            # -----------------------------------------
            sdu_assert(
                self.FSVI, checker, not fsvi, "type_faisceau == GRAPPE => FSVI ne doit pas exister"
            )
    ...
    def check_FSVR(self, checker):  # objet .FSVR
        ...
        if type_faisceau == 1:  # FAISCEAU_TRANS
            ...
            sdu_tous_compris(self.FSVR, checker, vmin=0.0, comment="FSVR > 0")
            ...
            if not lsimplif:
                ...
                if fsvi[2] == 1:  # enceinte circulaire
                    ...
                    sdu_tous_compris(self.FSVR, checker, fsvi[8:], vmin=0.0, comment="FSVR > 0")
                else:  # enceinte rectangulaire
                    assert len(fsvr) == 10 + nbzone, (fsvr, self)
                    sdu_tous_compris(self.FSVR, checker, fsvi[10:], vmin=0.0, comment="FSVR > 0")
