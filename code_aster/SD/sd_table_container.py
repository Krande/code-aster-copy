...
class sd_table_container(sd_table):
    ...
    def check_table_container(self, checker):
        ...
        if not self.exists():
            checker.err(self, "La sd_table_container %s ne semble" "pas exister" % (self.nomj()))
        ...
        for n in param:
            ...
            if data is not None:
                if col.data.lonuti != shape[1]:
                    checker.err(self, "Taille inconsistante %d!=%d" % (col.data.lonuti, shape[1]))
        ...
        for k in range(nbli):
            if lnom1[k] == "" or lnom2[k] == "":
                if lnom1[k] != "" or lnom2[k] != "":
                    checker.err(
                        self,
                        f"Ligne {k + 1} : TYPE_OBJET et NOM_SD " f"doivent être définis ensemble",
                    )
                else:
                    continue
