...
def C_COMPORTEMENT(COMMAND=None) :  #COMMUN#
    ...
    # Parametres d'integration
        ...
        mcfact =   FACT(statut=stcom,min=1,max='**',
            ...
            b_rigi_geom = BLOC( condition = """equal_to("RELATION", 'MULTIFIBRE')""",
            ...
            b_flua_resi        = BLOC(condition = """is_in("RELATION", ('RGI_BETON','FLUA_PORO_BETON','FLUA_ENDO_PORO'))""",
                RESI_INTE_RELA    =SIMP(statut='f',typ='R',defaut= 1.0E-6),
                ITER_INTE_MAXI    =SIMP(statut='f',typ='I',defaut= -1 ),
            ),
            b_other_resi       = BLOC(condition = """not is_in("RELATION", ('MFRONT','RGI_BETON','FLUA_PORO_BETON','FLUA_ENDO_PORO'))""",
                RESI_INTE_RELA    =SIMP(statut='f',typ='R',defaut= 1.0E-6),
                ITER_INTE_MAXI    =SIMP(statut='f',typ='I',defaut= 20 ),
            ),
