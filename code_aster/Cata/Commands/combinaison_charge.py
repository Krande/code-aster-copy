...
COMBINAISON_CHARGE=MACRO(nom="COMBINAISON_CHARGE",
         ...
         COMPORTEMENT           =FACT(statut='f',max='**',
            ...
            RELATION    = SIMP( statut='f',typ='TXM',defaut="ELAS",into=('ELAS','MULTIFIBRE','CABLE', 'ELAS_MEMBRANE_SV', 'ELAS_MEMBRANE_NH'),),
                                    ),
         TABLE_COEF     = SIMP(statut='o',typ=table_sdaster),
         CHAM_RESU           = FACT(statut='o',max='**',
         ...
         TABLE_RESU           = FACT(statut='o',max=3,
             UNITE              =SIMP(statut='f',typ=UnitType(), inout='out',),
             OPTION             =SIMP(statut='o',typ='TXM',defaut="EQ",
                                      into=('COEF_COMB','CALC_COMB','EXTREMA'),),
             TABLE              =SIMP(statut='o',typ=CO),
             b_extrema    =BLOC(condition = """equal_to("OPTION", 'EXTREMA')""",
                    CRIT_COMP = SIMP(statut='f',typ='TXM',defaut="TOUT",validators=NoRepeat(),max='**',
                                     into=('TOUT', 'MAXI','MAXI_ABS','MINI','MINI_ABS'),),
                              ),                         
                                    ),  
         IMPRESSION   = SIMP(statut='f',typ='TXM',defaut="NON",into=("OUI","NON") ),
         b_impression    =BLOC(condition = """equal_to("IMPRESSION", 'OUI')""",
