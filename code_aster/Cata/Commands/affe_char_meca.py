...
AFFE_CHAR_MECA = OPER(
    ...
    DDL_IMPO=FACT(
        ...
        BLOCAGE=SIMP(
            ...
            typ="TXM",
            validators=NoRepeat(),
            into=("DEPLACEMENT", "ROTATION", "TUYAU_FOURIER"),
            min=1,
            max=3,
        ),
        ...
        DRGX=SIMP(statut="f", typ="R"),
        DRGY=SIMP(statut="f", typ="R"),
        DRGZ=SIMP(statut="f", typ="R"),
    ...
    LIAISON_OBLIQUE=FACT(
        ...
        max="**",
        fr=tr(
            "Applique à des noeuds la meme valeur de déplacement définie composante par composante"
    ...
    LIAISON_MAIL=FACT(
        ...
        b_COQUE=BLOC(
            ...
            TRANSF_GEOM_MAIT=SIMP(
                statut="f",
                typ=(fonction_sdaster, nappe_sdaster, formule),
                min=3,
                max=3,
                fr=tr(
                    "3 fonctions fx,fy,fz définissant la transformation géométrique à appliquer"
                    " aux noeuds des mailles MAITRE avant la liaison."
                ),
            ),
            TRANSF_GEOM_ESCL=SIMP(
                statut="f",
                typ=(fonction_sdaster, nappe_sdaster, formule),
                min=3,
                max=3,
            ...
            DDL=SIMP(
                ...
                typ="TXM",
                validators=NoRepeat(),
                into=C_NOM_DDL_INTO("MECANIQUE", select_dof="DEPL"),
                max=6,
                min=1,
                fr=tr(
                    "Les DDL à prendre en compte dans la liaison (par défaut tous les DDL des coques)."
                ),
            ),
        ),
        b_MASSIF=BLOC(
            condition="""equal_to("TYPE_RACCORD", 'MASSIF')""",
            ...
            DDL=SIMP(
                ...
                typ="TXM",
                validators=NoRepeat(),
                into=C_NOM_DDL_INTO("MECANIQUE", with_dnor=True),
                max="**",
            ),
        ),
        b_COQUE_MASSIF=BLOC(
    ...
    LIAISON_CYCL=FACT(
        ...
        max="**",
        fr=tr(
            "Définit des relations linéaires permettant de recoller les bords de deux parties symétriquement cycliques"
        ),
        regles=(
    ...
    LIAISON_ELEM=FACT(
        ...
        max="**",
        fr=tr(
            "Modélise le raccord : d'une partie massive 3D avec une poutre ou avec un tuyau, d'une coque "
    ...
    LIAISON_RBE3=FACT(
        ...
        max="**",
        fr=tr(
            "Modélise des relations linéaires de type RBE3 entre "
        ...
        DDL_MAIT=SIMP(
            ...
            min=1,
            max=6,
            into=("DX", "DY", "DZ", "DRX", "DRY", "DRZ"),
        ),
        GROUP_NO_ESCL=SIMP(statut="f", typ=grno, validators=NoRepeat(), max="**"),
        DDL_ESCL=SIMP(statut="o", typ="TXM", min=1, max="**"),
        COEF_ESCL=SIMP(statut="f", typ="R", min=1, max="**", defaut=1.0),
    ),
    LIAISON_INTERF=FACT(
        statut="f",
        max="**",
    ...
    RELA_CINE_BP=FACT(
        ...
        max="**",
        fr=tr(
            "Etablir des relations cinématiques permettant de relier les noeuds cable aux noeuds du béton, et spécifier "
        ...
        b_rela_oui=BLOC(
            condition="""equal_to("RELA_CINE", 'OUI')""",
            SIGM_BPEL=SIMP(statut="f", typ="TXM", defaut="NON", into=("OUI", "NON")),
        ),
        b_rela_non=BLOC(
            condition="""equal_to("RELA_CINE", 'NON')""",
    ...
    ECHANGE_THM=FACT(
        ...
        max="**",
        fr=tr(
            "Applique à un domaine de milieu continu 2D ou 3D une condition echange"
            "(flux hydraulique)"
        ),
        regles=(
        ...
        HR_EXT=SIMP(statut="f", typ="R"),
        ALPHA=SIMP(statut="f", typ="R"),
        PVAP_SAT=SIMP(statut="f", typ="R"),
    ),
    FORCE_SOL=FACT(
        statut="f",
        max=1,
        ...
        regles=(
            UN_PARMI("GROUP_NO_INTERF", "SUPER_MAILLE"),
    ...
    translation={"AFFE_CHAR_MECA": "Assign mechanical load", "EVOL_CHAR": "Pressure from a result"},
)
