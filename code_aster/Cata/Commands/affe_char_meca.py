...
AFFE_CHAR_MECA=OPER(nom="AFFE_CHAR_MECA",op=   7,sd_prod=char_meca,
         ...
         DDL_IMPO        =FACT(statut='f',max='**',
             ...
             BLOCAGE         =SIMP(statut='f',typ='TXM',validators=NoRepeat(),into=('DEPLACEMENT','ROTATION','TUYAU_FOURIER'),min=1,max=3),
...
#  type de cham_no_sdaster CO()
        ...
        LIAISON_MAIL    =FACT(statut='f',max='**',
            ...
            b_COQUE     =BLOC( condition = """equal_to("TYPE_RACCORD", 'COQUE')""",
                ...
                DDL =SIMP(statut='f',typ='TXM',validators=NoRepeat(),into=C_NOM_DDL_INTO('MECANIQUE', select_dof='DEPL'),max=6, min=1,
                    fr=tr("Les DDL à prendre en compte dans la liaison (par défaut tous les DDL des coques)."),)
            ),
        ...
        LIAISON_CYCL    =FACT(statut='f',max='**',
         ...
         LIAISON_ELEM    =FACT(statut='f',max='**',
            ...
            b_3d_pou =BLOC( condition = """equal_to("OPTION", '3D_POU')""",
...
#
         ...
         PRE_EPSI       =FACT(statut='f',max='**',
             ...
             regles=(AU_MOINS_UN('TOUT','GROUP_MA','MAILLE','EPSI'),
                     PRESENT_ABSENT('TOUT','GROUP_MA','MAILLE'),
                     PRESENT_ABSENT('EPSI','TOUT','GROUP_MA','MAILLE'),
                     AU_MOINS_UN('EPSI','EPXX','EPYY','EPZZ','EPXY','EPXZ','EPYZ','EPX',
...
#  rajour d'un mot clé REPERE :/ LOCAL /GLOBAL
               b_force = BLOC(condition = """equal_to("TYPE_CHARGE", 'FORCE')""",
                          regles=(
                    AU_MOINS_UN('FX','FY','FZ','MX','MY','MZ','N','VY','VZ','MT','MFY','MFZ','MGX','MGY','MGZ'),
...
#  rajour d'un mot clé REPERE :/ LOCAL /GLOBAL
        ...
        INTE_ELEC       =FACT(statut='f',max='**',
         ...
         ECHANGE_THM    =FACT(statut='f',max='**',
             fr=tr("Applique à un domaine de milieu continu 2D ou 3D une condition echange"
                  "(flux hydraulique)"),
             regles=(AU_MOINS_UN('TOUT','GROUP_MA','MAILLE'),
                     PRESENT_ABSENT('TOUT','GROUP_MA','MAILLE'),
