...
AFFE_CHAR_MECA_F = OPER(
    ...
    ECHANGE_THM=FACT(
        ...
        regles=(
            UN_PARMI("TOUT", "GROUP_MA", TOUT="OUI"),
            UN_PARMI("PRE1_EXT", "HR_EXT"),
            EXCLUS("PRE1_EXT", "HR_EXT"),
            PRESENT_PRESENT("PRE1_EXT", "COEF_11"),
            PRESENT_PRESENT("PRE2_EXT", "COEF_21"),
            PRESENT_PRESENT("HR_EXT", "ALPHA",'PVAP_SAT'),
        ),
        TOUT=SIMP(statut="f", typ="TXM", into=("OUI",)),
        GROUP_MA=SIMP(statut="f", typ=grma, validators=NoRepeat(), max="**"),
        COEF_11=SIMP(statut="f", typ=(fonction_sdaster, nappe_sdaster, formule)),
        ...
        PRE1_EXT=SIMP(statut="f", typ=(fonction_sdaster, nappe_sdaster, formule)),
        PRE2_EXT=SIMP(statut="f", typ=(fonction_sdaster, nappe_sdaster, formule)),
        HR_EXT=SIMP(statut="f", typ=(fonction_sdaster, nappe_sdaster, formule)),
        ALPHA=SIMP(statut="f", typ=(fonction_sdaster, nappe_sdaster, formule)),
        PVAP_SAT=SIMP(statut="f", typ=(fonction_sdaster, nappe_sdaster, formule)),
        T_EXT=SIMP(statut="f", typ=(fonction_sdaster, nappe_sdaster, formule)),
    ),
