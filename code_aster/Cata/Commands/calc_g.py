...
CALC_G = OPER(
    ...
    b_courbure=BLOC(
        condition="""(equal_to("OPTION", 'K'))""",
        FORM_FISS=SIMP(statut="f", typ="TXM", into=("ELLIPSE", "CERCLE")),
        b_defaut_eli=BLOC(
            condition="""(equal_to("FORM_FISS", 'ELLIPSE'))""",
            DEMI_GRAND_AXE=SIMP(statut="o", typ="R"),
            DEMI_PETIT_AXE=SIMP(statut="o", typ="R"),
        ),
        b_defaut_cercle=BLOC(
            condition="""(equal_to("FORM_FISS", 'CERCLE'))""", RAYON=SIMP(statut="o", typ="R")
        ),
    ),
