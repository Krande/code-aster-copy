    ...
    **args
    ...
    if INTE_SPEC:
        if NUME_ORDRE_J and NUME_ORDRE_I != NUME_ORDRE_J:
            return fonction_c
        if (NOEUD_J and NOEUD_I != NOEUD_J) or NOM_CMP_J and NOM_CMP_I != NOM_CMP_J:
            return fonction_c
...
RECU_FONCTION = OPER(
    ...
    b_acces=BLOC(
        condition="""(exists("RESULTAT")) or (exists("RESU_GENE"))""",
    ...
    b_base_elas_flui=BLOC(
        ...
        b_connors=BLOC(
            condition="""equal_to("PARA_X", 'NB_CONNORS')""",
            PARA_Y=SIMP(statut="o", typ="TXM", into=("VITE_CRIT", "INSTAB_EDF", "INSTAB_TOUT_CMP")),
        ),
        b_cdck=BLOC(
            condition="""equal_to("PARA_X", 'VITE_FLU')""",
    ...
    b_inte_spec=BLOC(
        ...
        b_nume_ordre_i=BLOC(
            condition="""exists("NUME_ORDRE_I")""", NUME_ORDRE_J=SIMP(statut="f", typ="I", max=1)
        ),
        b_noeud_i=BLOC(
            condition="""exists("NOEUD_I")""",
    ...
    b_table=BLOC(
        ...
        regles=(UN_PARMI("PARA_X", "NOM_PARA_TABL"), PRESENT_PRESENT("PARA_X", "PARA_Y")),
        PARA_X=SIMP(
            statut="f",
            typ="TXM",
    ...
    b_resu=BLOC(
        ...
        fr=tr("Opérandes en cas de RESULTAT"),
    ...
    # ======= RESU_GENE =================================================
    b_tran_gene=BLOC(
        condition="""is_type("RESU_GENE") == tran_gene""",
        fr=tr("Récupération d'une fonction à partir d un concept TRAN_GENE"),
        regles=(UN_PARMI("NOM_CHAM", "NOEUD_CHOC", "GROUP_NO_CHOC"),),
        NOM_CHAM=SIMP(
            statut="f", typ="TXM", validators=NoRepeat(), into=("DEPL", "VITE", "ACCE", "PTEM")
        ),
        NOEUD_CHOC=SIMP(statut="c", typ=no),
        GROUP_NO_CHOC=SIMP(statut="f", typ=grno),
        b_cham=BLOC(
            condition="""(equal_to("NOM_CHAM", 'DEPL')) or (equal_to("NOM_CHAM", 'VITE')) or (equal_to("NOM_CHAM", 'ACCE'))""",
            regles=(
        ...
        b_choc=BLOC(
            condition="""(exists("NOEUD_CHOC")) or (exists("GROUP_NO_CHOC"))""",
            regles=(PRESENT_PRESENT("SOUS_STRUC", "INTITULE"),),
            PARA_X=SIMP(
                statut="o",
                typ="TXM",
                into=tuple(
                    ["INST", "FN", "FT1", "FT2", "VN", "VT1", "VT2", "DXLOC", "DYLOC", "DZLOC"]
                    + ["VINT%d" % (i) for i in range(1, 21)]
                ),
            ),
            PARA_Y=SIMP(
                statut="o",
    ...
    b_harm_gene=BLOC(
        condition="""is_type("RESU_GENE")==harm_gene""",
    ...
    # ),
    b_mode_gene=BLOC(
        condition="""is_type("RESU_GENE")==mode_gene""",
        ...
        b_cham=BLOC(
            ...
            NOM_CMP=SIMP(statut="f", typ="TXM"),
            b_cmp=BLOC(
                condition="""exists("NOM_CMP")""",
                regles=(UN_PARMI("NOEUD", "GROUP_NO"), UN_PARMI("SQUELETTE", "SOUS_STRUC")),
                NOEUD=SIMP(statut="c", typ=no),
                GROUP_NO=SIMP(statut="f", typ=grno),
    ...
    # ======= NAPPE =================================================
    b_nappe=BLOC(
        condition="""(exists("NAPPE"))""",
        fr=tr("Opérandes en cas de NAPPE"),
