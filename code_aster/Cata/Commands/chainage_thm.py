...
def chainage_thm_prod(self, TYPE_CHAINAGE, TYPE_RESU=None, **args):
    if args.get("__all__"):
        return (
            [None, evol_varc, cham_no_sdaster],
            [None, corresp_2_mailla],
            [None, corresp_2_mailla],
            [None, corresp_2_mailla],
        )

    if TYPE_CHAINAGE == "MECA_HYDR":
        return evol_varc
...
CHAINAGE_THM = MACRO(
    ...
    # Cas HYDR_MECA :
    b_hydr_meca=BLOC(
        condition="""equal_to("TYPE_CHAINAGE", 'HYDR_MECA')""",
        fr=tr("Chaînage hydraulique vers mécanique"),
        ...
        b_type_resu=BLOC(
            condition="""equal_to("TYPE_RESU", 'EVOL_VARC')""",
            fr=tr("Instant obligatoire si TYPE_RESU=EVOL_VARC"),
            INST=SIMP(statut="o", typ="R", max=1),
        ),
    ),
    # Cas MECA_HYDR :
    b_meca_hydr=BLOC(
        condition="""equal_to("TYPE_CHAINAGE", 'MECA_HYDR')""",
        fr=tr("Chaînage mécanique vers hydraulique"),
    ...
    # Cas INIT :
    b_init=BLOC(
        condition="""equal_to("TYPE_CHAINAGE", 'INIT')""",
        fr=tr("Calcul des matrices de projection"),
