...
from ...Helpers.LogicalUnit import Action, FileAccess, FileType, LogicalUnitFile
...
class MAIL_PY:
    ...
    def FromAster(self, nom):
        ...
        if type(nom) != bytes and type(nom) != str:
            ...
            nom_maillage = nom
        nom_maillage = nom_maillage.ljust(8)
...
class PARTITION:
    def __init__(self, jdc=None, nb=0):
        ...
        self.RELATIONS = {"C_plus": None, "C_moins": None, "nr": 0}
    ...
    def Partitionne_Aster(
        self, MAILLAGE, NB_PART, MODELE=None, METHODE=None, LOGICIEL=None, INFO=1
    ):
    ...
    # ---------------------------------------------------------------------------- #
    def Partitionne_Maillage(
        self, MAILLAGE_Python, NB_PART, MAILLE=None, METHODE=None, LOGICIEL=None, INFO=1
    ):
    ...
    def Creation_Dico_Correspondance_Type_Maille(self):
        ...
        maille2aretes[8] = maille2aretes[9] = maille2aretes[10] = maille2aretes[11] = maille2aretes[
            7
        ]
        # QUAD
        maille2aretes[12] = [(0, 1), (1, 2), (2, 3), (0, 3)]
        maille2aretes[13] = maille2aretes[14] = maille2aretes[15] = maille2aretes[
            16
        ] = maille2aretes[17] = maille2aretes[12]
        # TETRA
        ...
        # QUAD
        maille2nb[12] = 4
        maille2nb[13] = maille2nb[14] = maille2nb[15] = maille2nb[16] = maille2nb[17] = maille2nb[
            12
        ]
        # TETRA
        maille2nb[18] = 4
        ...
        # QUAD
        maille2dim[12] = 2
        maille2dim[13] = maille2dim[14] = maille2dim[15] = maille2dim[16] = maille2dim[
            17
        ] = maille2dim[12]
        # TETRA
        maille2dim[18] = 3
    ...
    def Connectivite_Aretes(self, OPTION=None, _LST_OK=None):
        ...
        print(
            "--- FIN Creation de la connectivite simple et inverse des aretes : ", time.clock() - t0
        )
    ...
    def Elimination_Mailles_de_bords(self, MAILLAGE_Python, _D_DIM_MAILLES, _DIM):
        ...
        _LST_IND = (
            NP.arange(len(_LST4)) + 1
        )  # on ajoute 1 pour eviter le premier 0 dans les test nonzero plus bas
        ...
        for m in _LST4:
            if self.OPTIONS["INFO"] >= 5:
                print(
                    ...
                    _LST_MAI[m].strip(),
                    " TMA:",
                    self.MAILLAGE_Python.tm[m],
                    " CO:",
                    self.MAILLAGE_Python.co[m],
                    "(noeuds de cette maille)",
                )
            ...
            try:
                if self.OPTIONS["INFO"] >= 5:
                    print(
                        "  CIA=",
                        self.MAILLAGE_Python.cia[ll],
                        "(mailles de dim N qui ont cette maille pour bord)",
                    )
                _tmp = []
                for maille in self.MAILLAGE_Python.cia[ll]:
                    if self.OPTIONS["INFO"] >= 5:
                        print(
                            ...
                            maille,
                            "Aster:",
                            _LST_MAI[maille].strip(),
                            " TMA:",
                            self.MAILLAGE_Python.tm[maille],
                        )
            ...
            for mai in _LST4:
                if self.OPTIONS["INFO"] >= 5:
                    print(
                        ...
                        _LST_MAI[mai].strip(),
                        " TMA:",
                        self.MAILLAGE_Python.tm[mai],
                        " CO:",
                        self.MAILLAGE_Python.co[mai],
                        "(noeuds de cette maille)",
                    )
                ...
                for arete in self.MAILLAGE_Python.cia:
                    ...
                    if _nb == len(_tmp):
                        if self.OPTIONS["INFO"] >= 5:
                            print(
                                "  Maille N+i:", self.MAILLAGE_Python.cia[arete], "- Arete:", arete
                            )
                        _LST_BD0.append(mai)
                        ok = 1
    ...
    def Reduction_Graphe(self, _DIM):
            ...
            for mai2 in self.GRAPH[mai]:
                if self.OPTIONS["INFO"] >= 5:
                    print(
                        ...
                        mai2,
                        "co:",
                        self.MAILLAGE_Python.co[mai2],
                        "tm:",
                        self.MAILLAGE_Python.tm[mai2],
                    )
                ...
                if _nb >= min(
                    _DIM1, _DIM2
                ):  # le min permet de faire du collage 3D-coque par exemple
                    _tmp2.append(mai2)
                    _tmp = [mai, mai2]
                    _tmp.sort()
                    _lst2.append(_tmp)
        ...
        print(
            "--- FIN Elimination des connectivités avec une interface nulle : ", time.clock() - t0
        )
        t0 = time.clock()
    ...
    def Creation_Group_ma_Aster_par_SD(self, NOM="SD", NOM2="B", INCLUSE="NON"):
        ...
        m = 0
        for sdd in self.liste_sd:
            d_gma[sdd].append(_LST_MAI[self.liste_mailles[m]].strip())
        ...
        if INCLUSE == "OUI":
            m = 0
            for sdd in self.liste_sd_bord:
                d_gma[sdd].append(_LST_MAI[self.liste_mailles_bord[m]].strip())
                # voir si le strip coute cher !
                m += 1
            ...
            for sdd in self.liste_sd_bord:
                d_gma_bord[sdd].append(
                    _LST_MAI[self.liste_mailles_bord[m]].strip()
                )  # voir si le strip coute cher !
                m += 1
        ...
        DEFI_GROUP(reuse=MAILLAGE, MAILLAGE=MAILLAGE, INFO=1, **motscle2)
        ...
        print(
            "--- FIN Creation et lancement de la commande DEFI_GROUP associée : ", time.clock() - t0
        )
