...
MACR_ECRE_CALC_CATA = MACRO(
    ...
    FISSURE=FACT(
        ...
        min=1,
        max=1,
        LONGUEUR=SIMP(statut="o", typ="R", val_min=0.0e0, fr=tr("Longueur de la fissure [zl]")),
        RUGOSITE=SIMP(statut="o", typ="R", fr=tr("Rugosite absolu (metres) [eps]")),
        ANGLE=SIMP(
            statut="o", typ="R", fr=tr("Angle par rapport a l'ascendante verticale (degres)")
        ),
        ZETA=SIMP(
            statut="o",
            typ="R",
    ...
    TEMPERATURE=FACT(
        ...
        GRADIENT=SIMP(
            statut="o",
            typ="TXM",
            into=("FOURNI", "IMPOSE", "CALCULE"),
            fr=tr("Modele de calcul du gradient de temperature [imograd]"),
        ),
        b_gradient_fourni=BLOC(
            condition="GRADIENT=='FOURNI'",
            fr=tr("Distribution de temperature fournie [imograd=-1]"),
            LISTE_COTES_TEMP=SIMP(
                statut="o",
                typ="R",
                max="**",
                fr=tr("Liste des cotes pour les temperatures"),
                validators=NoRepeat(),
            ),
            LISTE_VAL_TEMP=SIMP(
                statut="o", typ="R", max="**", fr=tr("Liste des valeurs de temperature")
            ),
        ),
        b_gradient_impose=BLOC(
            condition="GRADIENT=='IMPOSE'",
            fr=tr("Distribution imposee de temperature [imograd=0]"),
            TEMP1=SIMP(
                statut="o",
                typ="R",
            ...
            TEMP2=SIMP(
                statut="o", typ="R", fr=tr("Temperature de la paroi a l'entree (degC) [tm2]")
            ),
        ),
        b_gradient_calcule=BLOC(
            condition="GRADIENT=='CALCULE'",
            fr=tr("Profil de temperature calcule [imograd=1]"),
            EPAISSEUR_PAROI=SIMP(statut="o", typ="R", fr=tr("Epaisseur de la paroi (m) [epp]")),
            CONVECTION_AMONT=SIMP(
                statut="o",
                typ="R",
                fr=tr(
                    "Coefficient de convection a la surface de la paroi cote amont (W/degC/m2) [alphe]"
                ),
            ),
            CONVECTION_AVAL=SIMP(
                statut="o",
                typ="R",
                fr=tr(
                    "Coefficient de convection a la surface de la paroi cote aval (W/degC/m2) [alphs]"
                ),
            ),
            LAMBDA=SIMP(
                statut="o", typ="R", fr=tr("Conduction thermique de la paroi (W/degC/m) [lambd]")
            ),
            TEMP_FLUIDE_AVAL=SIMP(
                statut="o", typ="R", fr=tr("Temperature du fluide cote aval (degC) [ts]")
            ),
        ),
    ),
    #      CHOIX DES MODELES
