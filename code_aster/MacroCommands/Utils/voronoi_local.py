...
def Voronoi(seeds):
    """
    Computes an open (infinite) Voronoi tesselation centred at the coordinates
    of the given seeds
    Input:
    ...
    Output:
        vc .-  Voronoi vertices
        vr .-  Voronoi infinite regions

    """
    center = np.mean(seeds, axis=0)
    radius = 100  # Recommended value
    dt = Delaunay2D(center, 50 * radius)

    for s in seeds:  # Insert all seeds one by one
        dt.addPoint(s)
