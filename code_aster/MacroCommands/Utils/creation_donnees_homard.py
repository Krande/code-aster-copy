...
class creation_donnees_homard:
        ...
        while message_erreur is None:
            ...
            if self.ModeHOMA == 1:
                ...
                if self.mots_cles.get("ADAPTATION") == "RAFFINEMENT":
                    ...
                    self.mots_cles.get("ADAPTATION") == "RAFF_DERA"
                    or self.mots_cles.get("ADAPTATION") == "RAFF_DERA_ZONE"
                ):
                    self.TypeRaff = "libre"
                    self.TypeDera = "libre"
                ...
                if (
                    self.TypeRaff == "libre"
                    and self.mots_cles.get("ADAPTATION") != "RAFF_DERA_ZONE"
                ):
                    d_aux = {}
                ...
                if "DiametMi" in saux:
                    ...
                    if self.mots_cles.get("DIAM_MIN") < 0:
                        message_erreur = (
                            "Le diametre minimal doit etre strictement positif. "
                            + "La valeur %e est incorrecte." % self.mots_cles.get("DIAM_MIN")
                        )
                        break
                #
                if ("NiveauMi" in saux) and ("NiveauMa" in saux):
                    ...
                    if self.mots_cles.get("NIVE_MIN") >= self.mots_cles.get("NIVE_MAX"):
                        message_erreur = "Le niveau mini, %d, " % self.mots_cles.get("NIVE_MIN")
                        message_erreur += (
                            "doit etre inferieur au niveau maxi, %d."
                            % self.mots_cles.get("NIVE_MAX")
                        )
                        break
                #
    ...
    def ouvre_fichier(self, nomfic_local):
        ...
        if os.path.isfile(nomfic):
            try:
                ...
                print(
                    "Probleme au remove, erreur numéro ", codret_partiel[0], ":", codret_partiel[1]
                )
    ...
    def ecrire_fichier_configuration(self):
        ...
        while message_erreur is None:
            ...
            #
            if (
                "NOM_MED_MAILLAGE_FRONTIERE" in self.dico_configuration
                or "Frontiere_analytique" in self.dico_configuration
            ):
            ...
            if "LANGUE" in self.mots_cles:
                self.ecrire_ligne_configuration_2("Langue", self.mots_cles.get("LANGUE"))
