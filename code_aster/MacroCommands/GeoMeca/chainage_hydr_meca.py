...
def CHAINAGE_HYDR_MECA(self, args, motscles):
    ...
    __prepmec = CREA_CHAMP(
        TYPE_CHAM="NOEU_DEPL_R",
        OPERATION="ASSE",
        MAILLAGE=__maillage_h,
        ASSE=(_F(TOUT="OUI", CHAM_GD=__prep, NOM_CMP="PRE1", NOM_CMP_RESU="PTOT"),),
        **motscles
    )

    if b_type_resu_cham_no:

        __proch = PROJ_CHAMP(CHAM_GD=__prepmec, MATR_PROJECTION=MATR_HM1, **motscles)
        ...
        if not (b_inst_initial):
            ...
            __premmec = CREA_CHAMP(
                TYPE_CHAM="NOEU_DEPL_R",
                OPERATION="ASSE",
                MAILLAGE=__maillage_h,
                ASSE=(_F(TOUT="OUI", CHAM_GD=__prem, NOM_CMP="PRE1", NOM_CMP_RESU="PTOT"),),
                **motscles
            )
            ...
            __premmec = CREA_CHAMP(
                ...
                OPERATION="AFFE",
                MAILLAGE=__maillage_h,
                AFFE=(_F(TOUT="OUI", VALE=0.0, NOM_CMP="PTOT"),),
                **motscles
            )
        ...
        if inst_coincident:
            ...
            __ptotre = CREA_RESU(
                ...
                TYPE_RESU="EVOL_VARC",
                NOM_CHAM="PTOT",
                AFFE=(_F(CHAM_GD=__premmec, INST=instm), _F(CHAM_GD=__prepmec, INST=instp)),
            )
            ...
            __ptotre = CREA_RESU(
                ...
                TYPE_RESU="EVOL_VARC",
                NOM_CHAM="PTOT",
                AFFE=(_F(CHAM_GD=__premmec, INST=instp), _F(CHAM_GD=__prepmec, INST=INST)),
            )
