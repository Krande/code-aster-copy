...
from ...Commands import CALC_CHAMP, CREA_CHAMP, CREA_RESU, DEFI_LIST_REEL, PROJ_CHAMP
...
def CHAINAGE_MECA_HYDR(self, args, motscles):
    ...
    __epsir = CALC_CHAMP(
        RESULTAT=RESU_MECA, DEFORMATION="EPSI_ELNO", TOUT="OUI", LIST_INST=__listinst, **motscles
    )
    ...
    __defvp = CREA_CHAMP(
        ...
        PROL_ZERO="OUI",
        ASSE=(
            _F(CHAM_GD=__epsip, NOM_CMP="EPXX", NOM_CMP_RESU="EPXX", CUMUL="OUI", TOUT="OUI"),
            _F(CHAM_GD=__epsip, NOM_CMP="EPYY", NOM_CMP_RESU="EPXX", CUMUL="OUI", TOUT="OUI"),
            _F(CHAM_GD=__epsip, NOM_CMP="EPZZ", NOM_CMP_RESU="EPXX", CUMUL="OUI", TOUT="OUI"),
        ),
        **motscles
    )
    ...
    __defvm = CREA_CHAMP(
        ...
        PROL_ZERO="OUI",
        ASSE=(
            _F(CHAM_GD=__epsim, NOM_CMP="EPXX", NOM_CMP_RESU="EPXX", CUMUL="OUI", TOUT="OUI"),
            _F(CHAM_GD=__epsim, NOM_CMP="EPYY", NOM_CMP_RESU="EPXX", CUMUL="OUI", TOUT="OUI"),
            _F(CHAM_GD=__epsim, NOM_CMP="EPZZ", NOM_CMP_RESU="EPXX", CUMUL="OUI", TOUT="OUI"),
        ),
        **motscles
    )
    ...
    __defvr1 = CREA_RESU(
        ...
        NOM_CHAM="EPSI_ELNO",
        AFFE=(
            _F(CHAM_GD=__defvm, MODELE=__modele, INST=instm),
            _F(CHAM_GD=__defvp, MODELE=__modele, INST=instp),
        ),
    )

    __defvr1 = CALC_CHAMP(
        reuse=__defvr1, RESULTAT=__defvr1, DEFORMATION="EPSI_NOEU", LIST_INST=__listinst, TOUT="OUI"
    )

    __epsrpro = PROJ_CHAMP(
        RESULTAT=__defvr1, NOM_CHAM="EPSI_NOEU", MATR_PROJECTION=MATR_MH, **motscles
    )

    __defvrppro = CREA_CHAMP(
    ...
    if b_info:
        UTMESS("I", "CHAINAGE_7", valk=["divu", MODELE_HYDR.getName()], valr=[instm])

    __divum = CREA_CHAMP(
        TYPE_CHAM="NOEU_EPSI_R",
        OPERATION="ASSE",
        MODELE=MODELE_HYDR,
        ASSE=(_F(CHAM_GD=__defvrmpro, NOM_CMP="EPXX", NOM_CMP_RESU="DIVU", TOUT="OUI"),),
        **motscles
    )

    if b_info:
        UTMESS("I", "CHAINAGE_7", valk=["divu", MODELE_HYDR.getName()], valr=[instp])

    __divup = CREA_CHAMP(
        TYPE_CHAM="NOEU_EPSI_R",
        OPERATION="ASSE",
        MODELE=MODELE_HYDR,
        ASSE=(_F(CHAM_GD=__defvrppro, NOM_CMP="EPXX", NOM_CMP_RESU="DIVU", TOUT="OUI"),),
        **motscles
    )
    ...
    if inst_coincident:
        nomres = CREA_RESU(
            ...
            TYPE_RESU="EVOL_VARC",
            NOM_CHAM="DIVU",
            AFFE=(_F(CHAM_GD=__divum, INST=instm), _F(CHAM_GD=__divup, INST=instp)),
        )
    else:
        nomres = CREA_RESU(
            OPERATION="AFFE",
            TYPE_RESU="EVOL_VARC",
            NOM_CHAM="DIVU",
            AFFE=(_F(CHAM_GD=__divum, INST=instp), _F(CHAM_GD=__divup, INST=INST)),
        )
    return nomres
