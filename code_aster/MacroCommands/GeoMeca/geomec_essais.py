...
# ----------------------------------------------------------------------- #
# ----------------------------------------------------------------------- #
def essai_TRIA_DR_M_D(self, str_n_essai, DicoEssai, MATER, COMPORTEMENT, CONVERGENCE, INFO):
    """
    ...
    preparer_graphique("2", DicoEssai, str_fich, Courbes, NomsFich, Leg_x, Leg_y, {}, {})

    # Creation de la liste d'instants
    # -----------------------------------------
    __RLIST = DEFI_LIST_REEL(DEBUT=0.0, INTERVALLE=_F(JUSQU_A=100.0, NOMBRE=NB_INST), INFO=INFO)

    __DLIST = DEFI_LIST_INST(
        DEFI_LIST=_F(LIST_INST=__RLIST),
        ECHEC=_F(SUBD_METHODE="MANUEL", SUBD_PAS=2, SUBD_NIVEAU=10),
        INFO=INFO,
    )
    # ---
    ...
    for i, sig0__ in enumerate(PRES_CONF):
        ...
        # ---
        __CHAR1 = DEFI_FONCTION(INFO=INFO, NOM_PARA="INST", VALE=(0.0, 0.0, 100.0, -EPSI_IMPOSE[i]))

        __CHAR2 = DEFI_FONCTION(
            INFO=INFO, NOM_PARA="INST", VALE=(0.0, KZERO * sig0, 100.0, KZERO * sig0)
        )
        # ---
        # Calcul
        # ---
        try:
            __EVOL = SIMU_POINT_MAT(
                ...
                MATER=MATER,
                INCREMENT=_F(LIST_INST=__DLIST, INST_INIT=0.0, INST_FIN=100.0),
                NEWTON=_F(MATRICE="TANGENTE", REAC_ITER=1),
                ARCHIVAGE=_F(LIST_INST=__RLIST),
                SIGM_IMPOSE=_F(SIXX=__CHAR2, SIYY=__CHAR2),
                EPSI_IMPOSE=_F(EPZZ=__CHAR1),
                SIGM_INIT=_F(SIXX=KZERO * sig0, SIYY=KZERO * sig0, SIZZ=sig0),
    ...
    # --------------------------------------------------------
    impr_graphique(self, DicoEssai, Courbes, NomsFich, Leg_x, Leg_y, {}, {}, typessai)
...
# ----------------------------------------------------------------------- #
# ----------------------------------------------------------------------- #
def essai_TRIA_ND_M_D(self, str_n_essai, DicoEssai, MATER, COMPORTEMENT, CONVERGENCE, INFO):
    """
    ...
    for i, sig0__ in enumerate(PRES_CONF):
        ...
        __CHAR2 = DEFI_FONCTION(
            INFO=INFO, NOM_PARA="INST", VALE=(0.0, 0.0, 100.0, 0.5 * EPSI_IMPOSE[i])
        )
        ...
        try:
            __EVOL = SIMU_POINT_MAT(
                ...
                NEWTON=_F(MATRICE="TANGENTE", REAC_ITER=1),
                ARCHIVAGE=_F(LIST_INST=__RLIST),
                EPSI_IMPOSE=_F(EPZZ=__CHAR1, EPXX=__CHAR2, EPYY=__CHAR2),
                SIGM_INIT=_F(SIXX=KZERO * sig0, SIYY=KZERO * sig0, SIZZ=sig0),
                EPSI_INIT=_F(EPXX=0.0, EPYY=0.0, EPZZ=0.0, EPXY=0.0, EPXZ=0.0, EPYZ=0.0),
            )
        ...
        eps_vol = eps_xx + eps_yy + eps_zz
        pprime = (sig_xx + sig_yy + sig_zz) / 3.0
        q = abs(sig_zz - sig_xx)
        p = -1.0 * q / 3.0 + sig0
...
def essai_CISA_DR_C_D(self, str_n_essai, DicoEssai, MATER, COMPORTEMENT, CONVERGENCE, INFO):
    ...
    # ---
    __RLIST = DEFI_LIST_REEL(
        DEBUT=0.0,
        INTERVALLE=[_F(JUSQU_A=10.0, NOMBRE=NB_INST)]
        + [_F(JUSQU_A=10.0 * (2 * k + 1), NOMBRE=2 * NB_INST) for k in range(1, 2 * NB_CYCLE + 1)],
        INFO=INFO,
    )
    ...
    for i, sig0__ in enumerate(PRES_CONF):
        ...
        # ----------------------------------------
        str_fich_niv2 = str_fich_niv1 + "CONF" + int_2_str(i + 1, len(PRES_CONF)) + "_"
        ...
        # ---
        Gs_max = Calc_Gs_max(self, GAMMA_ELAS, sig0, KZERO, MATER, COMPORTEMENT, CONVERGENCE)
        ...
        for j, eps0 in enumerate(GAMMA_IMPOSE):
            ...
            # ---
            if sinusoidal:
                absc_peak = [10.0 * (2 * k + 1) for k in range(2 * NB_CYCLE + 1)]
                abscisse = [10.0 * k / 3.0 / NB_INST for k in range(3 * NB_INST)]

                for inst_peak in absc_peak:
                    abscisse += [inst_peak + 10.0 * kk / 3.0 / NB_INST for kk in range(6 * NB_INST)]

                # absc_sinus varie de 0 a Pi/2 par intervalles de NB_INST=10s
                # (sinus varie de 0 a 1)
                # -------------------------------------------------------------
                absc_sinus = M.pi * NP.array(abscisse) / 20.0

                ordonnee = NP.sin(absc_sinus) * 0.5 * eps0

            else:
                abscisse = [0.0] + [10.0 * (2 * k + 1) for k in range(2 * NB_CYCLE + 1)]
                ordonnee = [0.0] + [0.5 * eps0 * (-1.0) ** (k + 1) for k in range(2 * NB_CYCLE + 1)]

            __CHAR1 = DEFI_FONCTION(
                INFO=INFO, NOM_PARA="INST", ABSCISSE=abscisse, ORDONNEE=list(ordonnee)
            )

            __CHAR2 = DEFI_FONCTION(
                INFO=INFO, NOM_PARA="INST", VALE=(0.0, sig0, 10.0 * (4 * NB_CYCLE + 1), sig0)
            )

            __CHAR3 = DEFI_FONCTION(
                INFO=INFO,
                NOM_PARA="INST",
                VALE=(0.0, KZERO * sig0, 10.0 * (4 * NB_CYCLE + 1), KZERO * sig0),
            )
            # ---
            # Calcul
            # ---
            try:
                __EVOL = SIMU_POINT_MAT(
                    ...
                    MATER=MATER,
                    INCREMENT=_F(
                        LIST_INST=__DLIST, INST_INIT=0.0, INST_FIN=10.0 * (4 * NB_CYCLE + 1)
                    ),
                    NEWTON=_F(MATRICE="TANGENTE", REAC_ITER=1),
                    ARCHIVAGE=_F(LIST_INST=__RLIST),
                    SIGM_IMPOSE=_F(SIXX=__CHAR3, SIYY=__CHAR3, SIZZ=__CHAR2),
            ...
            # ------------------------------------------------------
            ind1 = inst.index(10.0 * (2 * (2 * NB_CYCLE - 2) + 1))
            ind2 = inst.index(10.0 * (2 * (2 * NB_CYCLE - 1) + 1))
            ind3 = inst.index(10.0 * (2 * (2 * NB_CYCLE) + 1))
            ...
            I = NP.sum(
                0.5
                * (sig_xy[ind1:ind2] + sig_xy[ind1 + 1 : ind2 + 1])
            ...
            J = NP.sum(
                0.5
                * (sig_xy[ind2:ind3] + sig_xy[ind2 + 1 : ind3 + 1])
...
# ----------------------------------------------------------------------- #
# ----------------------------------------------------------------------- #
def essai_TRIA_ND_C_F(self, str_n_essai, DicoEssai, MATER, COMPORTEMENT, CONVERGENCE, INFO):
    """
    ...
    for i, t in enumerate(DicoEssai["CRIT_LIQUEFACTION"]):
        ...
        if t == "EPSI_RELA_MAX":
            ...
            print("   !          LE CRITERE DE LIQUEFACTION NO.%1d EST:    !" % (i + 1))
            print("   !                                                  !")
            print("   !    EPSI_AXI_MAXI - EPSI_AXI_MINI > %.4e    !" % (vale_crit[t]))
            print("   !                                                  !")
            print("   !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
    ...
    # 'NCYCL','DSIGM',
    # -----------------------------------------
    cle = ["INST", "EPS_AXI", "EPS_LAT", "EPS_VOL", "SIG_AXI", "SIG_LAT", "P", "Q", "PRE_EAU", "RU"]
    ...
    preparer_graphique(
        "1", DicoEssai, str_fich_niv1, Courbes_niv1, NomsFich_niv1, Leg_x_niv1, Leg_y_niv1, {}, {}
    )
    ...
    __RLIST = DEFI_LIST_REEL(
        DEBUT=0.0,
        INTERVALLE=[_F(JUSQU_A=10.0 * (k + 1), NOMBRE=NB_INST) for k in range(4 * NB_CYCLE)],
        INFO=INFO,
    )
    ...
    # Date:    31/03/2015
    # ------------------------------------------------------------
    inst_peak = [10.0 + 40.0 * k for k in range(NB_CYCLE)]
    ...
    for i, sig0 in enumerate(PRES_CONF):
        ...
        for j, dsig__ in enumerate(SIGM_IMPOSE):
            ...
            # ---
            if sinusoidal:
                absc_peak = [10.0 * (2 * k + 1) for k in range(2 * NB_CYCLE)] + [
                    10.0 * (4 * NB_CYCLE)
                ]
                ...
                for ipeak in absc_peak[:-1]:
                    abscisse += [ipeak + 10.0 * kk / 3.0 / NB_INST for kk in range(6 * NB_INST)]
                ...
                abscisse = (
                    [0.0]
                    + [10.0 * (2 * k + 1) for k in range(2 * NB_CYCLE)]
                    + [10.0 * (4 * NB_CYCLE)]
                )

                ordonnee = (
                    [-sig0] + [-sig0 + dsig * (-1) ** k for k in range(2 * NB_CYCLE)] + [-sig0]
                )
            ...
            __CHAR2 = DEFI_FONCTION(
                INFO=INFO,
                NOM_PARA="INST",
                VALE=(0.0, -KZERO * sig0, 10.0 * (4 * NB_CYCLE), -KZERO * sig0),
            ...
            try:
                ...
                calc.run_(
                    ...
                    CONVERGENCE=CONVERGENCE,
                    MATER=MATER,
                    INCREMENT=_F(LIST_INST=__DLIST, INST_INIT=0.0, INST_FIN=10.0 * (4 * NB_CYCLE)),
                    NEWTON=_F(MATRICE="TANGENTE", REAC_ITER=1),
                    ARCHIVAGE=_F(LIST_INST=__RLIST),
                    VECT_IMPO=(
                        _F(NUME_LIGNE=1, VALE=__CHAR2),
            ...
            p = (NP.array(TabRes["SIZZ"]) + 2.0 * NP.array(TabRes["SIXX"])) / 3.0
            ...
            for k, n in enumerate(indx_peak):
                ...
                if info_dbg:
                    ...
                    print(
                        "   + CRIT       = %d   NUME_DEPS = %d   NUME_DQDP = %d"
                        % (ncrit, nume1, nume2)
                    )
                ...
                if ncrit >= 2:
                    ...
                    print("   !                                                   !")
                    print(
                        "   !      INSTABILITE DETECTEE A L INSTANT %.3f     !" % (inst[indx_init])
                    )
                    print("   !               AU CYCLE NUMERO %3d               !" % (numcyc + 1))
                    print("   !                                                   !")
                    print("   !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n")
            ...
            if numcyc == -1 and (not calc_ok):
                ...
                for n in range(NB_CYCLE):
                    if inst[-1] >= 10.0 + 40.0 * n and inst[-1] <= 10.0 + 40.0 * (n + 1):
                        numcyc = n
                        ...
                        print(
                            "   !       L INSTANT %.3f AU CYCLE NUMERO %3d       !"
            ...
            if indx_init > 0:
                ...
                sigm, epsi = [0.0] * 6, [0.0] * 6
                for n in range(3):
                    sigm[n] = TabRes["SI%s" % (comp[n])][indx_init]
                    epsi[n] = TabRes["EP%s" % (comp[n])][indx_init]

                    if info_dbg:
                        print("   + EP%s =%.6e   SI%s =%.6e" % (comp[n], epsi[n], comp[n], sigm[n]))

                p = (sigm[0] + sigm[1] + sigm[2]) / 3.0
                q = sigm[2] - sigm[0]
                ...
                __EVOLM, calc_ok_mono = essai_TRIA_ND_C_D_mono(
                    self,
                    inst_max,
                    sigm,
                    epsi,
                ...
                if NP.sign(qcalc1 - qcalc2) == NP.sign(q - 0.5 * q2 - 0.5 * q3):
                ...
                print(
                    "     SIGNE(Q_ESSAI[-1]-Q_ESSAI[-2]) ="
                    + " SIGNE(2*Q_CALC[-1]-Q_CALC[-2]-Q_CALC[-3]) = %.0f"
                    % (NP.sign(q - 0.5 * q2 - 0.5 * q3))
                )
                ...
                if abs(qcalc1 - q0) > abs(dsig):
                    ...
                    if info_dbg:
                        print(
                            "   @ CONSIGNE DEPASSEE :: CHARGEMENT INVERSE ::"
                            + " | Q_ESSAI[-1] - Q0 | = %e > | DSIG | = %e"
                            % (abs(qcalc1 - q0), abs(dsig))
                        )

                elif abs(q - q2) < 1.0e-4 * abs(q):
                    # le trajet s'est arrete sur un plateau
                    # nbc0+=0

                    if info_dbg:
                        print(
                            "   @ PLATEAU :: CHARGEMENT COLINEAIRE ::"
                ...
                for nbc in range(NB_CYCLE_EPSILON):
                    ...
                    if abs(q - q0) < abs(0.95 * dsig):
                        if info_dbg:
                            print("\n   !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
                            print("   !                                                   !")
                            print(
                                "   !            NB REDECOUPAGE INITIAL %4d            !"
                                % (nb_redec1)
                            )
                            print(
                                "   !         A GAMMA_MAX = %.4e INSUFFISANT      !" % (abs(gamma))
                            )
                            print("   !                                                   !")
                            print(
                                "   !     AUGMENTATION GAMMA_MAX = %.4e           !"
                                % (coef_gamma * abs(gamma))
                            )
                            print(
                                "   !     NB PAS DE REDECOUPAGE  = %4d                 !"
                                % (nb_redec2)
                            )
                            print("   !                                                   !")
                            print("   !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
                    ...
                    p = (NP.array(__TabResm["SIZZ"]) + 2 * NP.array(__TabResm["SIXX"])) / 3.0
                    ...
                    for jcrit, crit in enumerate(DicoEssai["CRIT_LIQUEFACTION"]):
                        if crit == "RU_MAX":
                            ...
                            eps_vol = eps_xx + eps_yy + eps_zz
                            pre_eau = -1.0 * coef_eau * eps_vol
                            ru = pre_eau * 3.0 / (1.0 + 2.0 * KZERO) / sig0
                            ...
                            if not rubool[jcrit]:
                                ...
                                if info_dbg and rubool[jcrit]:
                                    print(
                                        ...
                                        % (M.ceil((nbc + 1.0) / 2.0) + numcyc, NB_CYCLE_EPSILON)
                                        + "\n   !!!                                               !!!"
                                        + "\n   !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!"
                                    )

                        if crit == "EPSI_ABSO_MAX":
                            if vale_crit[crit] > 0.0:
                        ...
                        if crit == "EPSI_RELA_MAX":
                            ...
                            if not rubool[jcrit]:
                                rubool[jcrit] = eps_zz.max() - eps_zz.min() >= vale_crit[crit]
                    ...
                    # sont actifs
                    # ------------------------------------------------------
                    if not any([not rubool[jcr] for jcr in range(nbcrit)]) and arret:
                        break
                # ---------------------------------------------------------
                #
            ...
            if info_dbg:
                ...
                for v in Vari_Supp:
                    print("     - LEN[%s[%d]] = %d (Vari_Supp)" % (v, i, len(Vari_Supp[v][i])))
            ...
            if crit:
                ...
                # --------------------------------------------------------
                ncycrit = M.ceil(inst[indcrit] / 40.0)
                ...
                Resu_Essai["DSIGM"][i].append(abs(dsig * 3.0 / (1.0 + 2.0 * KZERO) / sig0))
            ...
            if not calc_ok:
                if not inst_init:
                    ncyerro = M.ceil(inst[-1] / 40.0)

                elif inst[-1] <= inst_init:
                    ncyerro = M.ceil(inst[-1] / 40.0)

                else:
                    ncyerro = M.ceil((nbc + 1.0) / 2.0) + numcyc
            ...
            list_y = [list(-q), list(pre_eau), list(pre_eau), list(ru), list(ru), list(-q)]
            ...
            # --------------------------------------------------
            if str_n_essai:
                affiche_alarm_TRIA_ND_C_F(
                    str_n_essai, sig0, -dsig, codret, NB_CYCLE, ncycrit, ncyerro
                )
            else:
                affiche_alarm_TRIA_ND_C_F("1", sig0, -dsig, codret, NB_CYCLE, ncycrit, ncyerro)
...
# ----------------------------------------------------------------------- #
# ----------------------------------------------------------------------- #
def essai_TRIA_DR_C_D(self, str_n_essai, DicoEssai, MATER, COMPORTEMENT, CONVERGENCE, INFO):
    """
    ...
    for i, sig0__ in enumerate(PRES_CONF):
        ...
        # ---
        Es_max = Calc_Es_max(self, EPSI_ELAS, sig0, KZERO, MATER, COMPORTEMENT, CONVERGENCE)
        ...
        for j, eps0__ in enumerate(EPSI_MAXI):
            ...
            if sinusoidal:
                ...
                abscisse = [0.0] + [10.0 * (2 * k + 1) for k in range(2 * NB_CYCLE + 1)]
                ordonnee = [0.0] + [eps0, eps1] * NB_CYCLE + [eps0]

            __CHAR1 = DEFI_FONCTION(
                INFO=INFO, NOM_PARA="INST", ABSCISSE=abscisse, ORDONNEE=ordonnee
            )
            ...
            if not (Es <= 1.0 or abs(Es - 1.0) <= 1.0e-8):
...
# ----------------------------------------------------------------------- #
# ----------------------------------------------------------------------- #
def essai_TRIA_ND_C_D(self, str_n_essai, DicoEssai, MATER, COMPORTEMENT, CONVERGENCE, INFO):
    """
...
# ----------------------------------------------------------------------- #
# ----------------------------------------------------------------------- #
def essai_OEDO_DR_C_F(
    self, str_n_essai, DicoEssai, MATER, COMPORTEMENT, CONVERGENCE, INFO, oedometer=True
):
    """
    ...
    preparer_graphique(
        "2", DicoEssai, str_fich_niv2, Courbes_niv2, NomsFich_niv2, Leg_x_niv2, Leg_y_niv2, {}, {}
    )
    ...
    __RLIST = DEFI_LIST_REEL(
        DEBUT=0.0,
        INTERVALLE=[_F(JUSQU_A=10.0, NOMBRE=NB_INST)]
        + [_F(JUSQU_A=10.0 * (2 * k + 1), NOMBRE=2 * NB_INST) for k in range(1, 2 * NB_CYCLE + 2)],
        INFO=INFO,
    )
    ...
    for i, sig0__ in enumerate(PRES_CONF):
        ...
        if sinusoidal:
            ...
            ordonnee = list(
                sigm_mean + NP.sin(-0.5 * M.pi + 2.0 * absc_sinus[: 3 * NB_INST]) * dsigm
            )
            ...
            ordonnee += list(sigm_mean + NP.sin(absc_sinus[3 * NB_INST : 9 * NB_INST]) * dsigm)
            ...
            for j, sigcomp__ in enumerate(SIGM_IMPOSE[1:]):
                ...
                ordonnee += list(
                    sigm_mean
                    + NP.sin(absc_sinus[(9 + 12 * j) * NB_INST : (9 + 12 * (j + 1)) * NB_INST])
                    * dsigm
                )
        else:
            abscisse = [0.0] + [10.0 * (2 * k + 1) for k in range(2 * NB_CYCLE + 2)]
        ...
        __CHAR2 = DEFI_FONCTION(
            INFO=INFO,
            NOM_PARA="INST",
            VALE=(0.0, 0.0, 10.0 * (4 * NB_CYCLE + 3), 0.0),
            PROL_DROITE="CONSTANT",
        )
        if oedometer:
            sigm_impo = _F(SIZZ=__CHAR1)

            epsi_impo = _F(EPXX=__CHAR2, EPYY=__CHAR2, EPXY=__CHAR2, EPXZ=__CHAR2, EPYZ=__CHAR2)
        else:
            sigm_impo = _F(SIXX=__CHAR1, SIYY=__CHAR1, SIZZ=__CHAR1)

            epsi_impo = _F(EPXY=__CHAR2, EPXZ=__CHAR2, EPYZ=__CHAR2)
        # ---
        # Calcul
        # ---
        try:
            __EVOL = SIMU_POINT_MAT(
                ...
                MATER=MATER,
                SUPPORT="POINT",
                INCREMENT=_F(LIST_INST=__DLIST, INST_INIT=0.0, INST_FIN=10.0 * (4 * NB_CYCLE + 3)),
                NEWTON=_F(MATRICE="TANGENTE", REAC_ITER=1),
                ARCHIVAGE=_F(LIST_INST=__RLIST),
        ...
        # NIVEAU 2: remplissage des graphiques
        # ------------------------------------------------------
        str_leg2 = "PCONF=" + str("%.2E" % (-sig0)) + " SIDECH=" + str("%.2E" % (-sigtrac))
    ...
    # ------------------------------------------------------
    impr_graphique(
        self, DicoEssai, Courbes_niv2, NomsFich_niv2, Leg_x_niv2, Leg_y_niv2, {}, {}, typessai
    )
...
def essai_ISOT_DR_C_F(self, str_n_essai, DicoEssai, MATER, COMPORTEMENT, CONVERGENCE, INFO):
    ...
    essai_OEDO_DR_C_F(
        self, str_n_essai, DicoEssai, MATER, COMPORTEMENT, CONVERGENCE, INFO, oedometer=False
    )
