...
class CALCULS_ASTER:
    ...
    def run_distrib(self, list_val):
        ...
        # ----------------------------------------------------------------------------
        sys.argv = [""]
        run = AsRunFactory(debug_stderr=False, log_progress="asrun.log")
        ...
        if not prof["mode"][0] or (prof["mode"][0] == "batch" and run.get("batch") == "non"):
        ...
        prof.WriteExportTo(os.path.join(resudir, "master.export"), dbg=True)
        ...
        if self.DYNAMIQUE:
            if (
                ...
                and self.graph_mac
                and (True in ["MAC" in reponses[ii][2] for ii in range(len(reponses))])
        ...
        for result in task.exec_result:
            ...
            if not diag[0:2] in ["OK", "<A"]:
                ...
                try:
                    ...
                    error_filename = (
                        ".".join(output_filename.split(".")[0:-1])
                        + ".e"
                        + output_filename.split(".")[-1][1:]
                    )
        ...
        for c in labels:
            tbl = get_tables(
                tables_calc=calcul, tmp_repe_table=os.path.join(resudir, c, "REPE_OUT"), prof=prof
            )
    ...
    def ajout_post_mac(self, reponse):
        ...
        txt.append(
            "from code_aster.MacroCommands.CalcEssai.ce_calcul_expansion import extract_mac_array"
        )
        ...
        txt.append(
            str(reponse[0])
            + "=CREA_TABLE(LISTE=(_F(PARA='NUME_ORDRE',LISTE_I=list(range(1,nb_freq+1)),),_F(PARA='MAC',LISTE_R=l_mac,),),)"
        )
        return "\n".join(txt)
...
class CALC_ERROR:
    ...
    def CalcSensibilityMatrix(self, Lcalc, val, dX=None, pas=None):
        ...
        if len(dX) != len(val):
            raise Exception(
                "Error : 'dX' and 'val' parameters aren't compatible (lenght are not equal).\ndX = %s\nval = %s"
                % (dx, val)
            )
        ...
        F = Lcalc[0]
        F_interp = self.Simul.multi_interpole_sensib(
            F, reponses
        )  # F_interp est une liste contenant des tab num des reponses interpolés
                    ...
                    if NP.all(h != 0.0):
                        ...
                        if self.unite_resu:
                            ...
                            fic.write(
                                "\n Le parametre "
                                + para[k]
                                + "est nul ou plus petit que la précision machine"
                            )
                            fic.close()
        ...
        self.gradient_init = self.calcul_gradient(
            self.A, self.erreur
        )  # utile pour le test de convergence, on prend les valeurs dimensionnées
        self.residu = self.test_convergence(
            self.gradient_init, self.erreur, self.A, NP.zeros(len(self.gradient_init))
        )
