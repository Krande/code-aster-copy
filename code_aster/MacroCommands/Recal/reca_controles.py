...
def compare__dim_rep__dim_RESU_EXP(REPONSES, RESU_EXP):
    ...
    if len(REPONSES) != len(RESU_EXP):
        txt = (
            ...
            + " réponses et "
            + str(len(RESU_EXP))
            + " expériences ; On doit avoir autant de réponses que de résultats expérimentaux"
        )
    return txt
...
def compare__dim_poids__dim_RESU_EXP(POIDS, RESU_EXP):
    ...
    if len(POIDS) != len(RESU_EXP):
        txt = (
            ...
            + " poids et "
            + str(len(RESU_EXP))
            + " expériences ; On doit avoir autant de poids que de résultats expérimentaux"
        )
    return txt
...
def verif_fichier(UL, PARAMETRES, REPONSES):
    ...
    for i in range(len(PARAMETRES)):
        if (fic.find(PARAMETRES[i][0]) == -1) or (
            (fic.find(PARAMETRES[i][0] + "=") == -1) and (fic.find(PARAMETRES[i][0] + " ") == -1)
        ):
            txt += (
                "\nLe paramètre "
                + PARAMETRES[i][0]
                + " que vous avez entré pour la phase d'optimisation n'a pas été trouvé dans votre fichier de commandes ASTER"
            )
    for i in range(len(REPONSES)):
        if (fic.find(REPONSES[i][0]) == -1) or (
            (fic.find(REPONSES[i][0] + "=") == -1) and (fic.find(REPONSES[i][0] + " ") == -1)
        ):
            txt_alarme += (
                "\nLa réponse  "
                + REPONSES[i][0]
                + " que vous avez entrée pour la phase d'optimisation n'a pas été trouvée dans votre fichier de commandes ASTER"
            )
    return txt, txt_alarme
...
def verif_valeurs_des_PARAMETRES(PARAMETRES):
    ...
    for i in range(len(PARAMETRES)):
        if PARAMETRES[i][2] > PARAMETRES[i][3]:
            txt = (
                ...
                + " de  "
                + PARAMETRES[i][0]
                + "est plus grande que sa borne supérieure"
                + str(PARAMETRES[i][3])
            )
    ...
    for i in range(len(PARAMETRES)):
        if (PARAMETRES[i][1] < PARAMETRES[i][2]) or (PARAMETRES[i][1] > PARAMETRES[i][3]):
            txt = (
                ...
                + " de "
                + PARAMETRES[i][0]
                + " n'est pas dans l'intervalle [borne_inf,born_inf]=["
                + str(PARAMETRES[i][2])
                + " , "
                + str(PARAMETRES[i][3])
                + "]"
    ...
    for i in range(len(PARAMETRES)):
        if PARAMETRES[i][1] == 0.0:
            txt = (
                txt
                + "\nProblème de valeurs initiales pour le paramètre "
                + PARAMETRES[i][0]
                + " : ne pas donner de valeur initiale nulle mais un ordre de grandeur."
            )
    # verification que borne_sup !=0
    for i in range(len(PARAMETRES)):
        if PARAMETRES[i][3] == 0.0:
            txt = (
                txt
                + "\nProblème de borne supérieure pour le paramètre "
                + PARAMETRES[i][0]
                + " : ne pas donner de valeur strictement nulle."
            )
    # verification que borne_inf !=0
    for i in range(len(PARAMETRES)):
        if PARAMETRES[i][2] == 0.0:
            txt = (
                txt
