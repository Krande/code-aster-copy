...
def calc_precont_ops(
    ...
    **args
    ...
    __TINT = (9.0 * __L2[-1] + __L2[-2]) / 10.0
    __TINT2 = (9.5 * __L2[-1] + 0.5 * __L2[-2]) / 10.0
    ...
    if adher == "OUI":
        ...
        for ica, mcabl in enumerate(CABLE_BP):
            ...
            motscles["RELA_CINE_BP"].append(_F(CABLE_BP=mcabl, SIGM_BPEL="OUI", RELA_CINE="NON"))
            motscle2["RELA_CINE_BP"].append(_F(CABLE_BP=mcabl, SIGM_BPEL="NON", RELA_CINE="OUI"))
            motscle3["RELA_CINE_BP"].append(_F(CABLE_BP=mcabl, SIGM_BPEL="OUI", RELA_CINE="OUI"))
        ...
        if CABLE_BP_INACTIF:
            ...
            for mcabl in CABLE_BP_INACTIF:
                ...
                motscle6["RELA_CINE_BP"].append(
                    _F(CABLE_BP=mcabl, SIGM_BPEL="NON", RELA_CINE="OUI")
                )
        ...
        if __GROUP_MA_A_SEG2 != []:
            ...
            affe_mo.append(
                {"GROUP_MA": __GROUP_MA_A_SEG2, "PHENOMENE": "MECANIQUE", "MODELISATION": "BARRE"}
            )
        ...
        __REAC = CREA_CHAMP(
            ...
            MODELE=MODELE,
            ASSE=(
                _F(TOUT="OUI", CHAM_GD=__REAC0, COEF_R=1.0),
                _F(GROUP_MA=__GROUP_MA_A, CHAM_GD=__REA, COEF_R=-1.0),
            ),
        )
        ...
        for mcabl in CABLE_BP_INACTIF:
            # Creation de mots-cles pour les AFFE_CHAR_MECA
            motscle6["RELA_CINE_BP"].append(_F(CABLE_BP=mcabl, SIGM_BPEL="NON", RELA_CINE="OUI"))
        info_actif_actif = []
        for mcabl in CABLE_BP:
            __TCAB1 = RECU_TABLE(CO=mcabl, NOM_TABLE="CABLE_GL")

            motscle2["RELA_CINE_BP"].append(_F(CABLE_BP=mcabl, SIGM_BPEL="NON", RELA_CINE="OUI"))
            motscle5["RELA_CINE_BP"].append(_F(CABLE_BP=mcabl, SIGM_BPEL="NON", RELA_CINE="OUI"))
            ...
            for icable in range(nb_cable):
                ...
                __nom_noeu = (
                    table_cable.NOM_ANCRAGE1.values()[icable],
                ...
                for j in range(2):
                    if __typ_ancr[j].strip() == "PASSIF":
                        ...
                        #                                                          GLIS=0.))
                        #                        else:
                        motscle2["DDL_IMPO"].append(_F(GROUP_NO=__nom_noeu[j].strip(), GLIS=0.0))
                        if __recul_exists:
                            motscle5["DDL_IMPO"].append(
                                _F(GROUP_NO=__nom_noeu[j].strip(), GLIS=0.0)
                            )
                    else:
                        actif += 1
                        #                        if __typ_noeu[j].strip() == 'NOEUD':
        ...
        __FCT1 = DEFI_FONCTION(
            INTERPOL=("LIN", "LIN"), NOM_PARA="INST", VALE=(__TMIN, 0.0, t_fin_etape1, 1.0)
        )

        if __recul_exists:
            if nb_etapes == 2:
                __FCTREC = DEFI_FONCTION(
                    INTERPOL=("LIN", "LIN"),
                    NOM_PARA="INST",
        ...
        if __ActifActif:
            ...
            __CH1a = CREA_CHAMP(
                TYPE_CHAM="NOEU_DEPL_R", OPERATION="AFFE", MODELE=MODELE, **motscle3a
            )
            __CH1b = CREA_CHAMP(
            ...
            if __recul_exists:
                dExcit2.append(_F(CHARGE=_C_RA, TYPE_CHARGE="DIDI", FONC_MULT=__FCTREC))
        ...
        if __ActifActif:
            ...
            __RESREA = CALC_CHAMP(FORCE=("REAC_NODA",), RESULTAT=RES)
