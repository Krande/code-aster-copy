...
def calc_thermeca_mult_ops(self,TEMP_FIN,TEMP_INIT, RESU_MECA_UNIT,RESU_SUPL_THER,**args):
        ...
        if RESU_SUPL_THER == 'OUI':
                ...
                for i in range(nbinst):
                        ...
                        cst = CREA_CHAMP(OPERATION='AFFE',
                                 MODELE=modele_ther,
                                 AFFE=_F(TOUT='OUI',NOM_CMP='TEMP',VALE=TEMP_INIT),
                                 TYPE_CHAM='NOEU_TEMP_R')

                        chamuthe = CREA_CHAMP(NOM_CHAM='TEMP',
                                              OPERATION='EXTR',
                                              RESULTAT=RESU_THER_UNIT,
                                              INST = inst,
                                              TYPE_CHAM='NOEU_TEMP_R')

                        pourmul = CREA_CHAMP(OPERATION='ASSE',
                                              MODELE=modele_ther,
                                              TYPE_CHAM='NOEU_TEMP_R',
                                              ASSE=(_F(CHAM_GD=chamuthe, COEF_R= 1.,TOUT='OUI',CUMUL='OUI'),
                                                      _F(CHAM_GD=cst, COEF_R=-1.,TOUT='OUI',CUMUL='OUI'),))

                        chamTmul = CREA_CHAMP(ASSE=_F(CHAM_GD=pourmul,
                                                                        COEF_R=TEMP_FIN,
                                                                        TOUT='OUI'),
                                                      MODELE=modele_ther,
                                                      OPERATION='ASSE',
                        ...
                        final[i] = CREA_CHAMP(OPERATION='ASSE',
                                              MODELE=modele_ther,
                                              TYPE_CHAM='NOEU_TEMP_R',
                                              ASSE=(_F(CHAM_GD=chamTmul, COEF_R= 1.,TOUT='OUI',CUMUL='OUI'),
                                                      _F(CHAM_GD=cst, COEF_R=1.,TOUT='OUI',CUMUL='OUI'),))
                        
                        arguments_affe += (_F(CHAM_GD=final[i],
                                              CHAM_MATER=cham_mater_ther,
                                              INST=(inst, ),
                                              MODELE=modele_ther),)

                resTmult = CREA_RESU(AFFE=arguments_affe,
                                    NOM_CHAM='TEMP',
                                     OPERATION='AFFE',
                                     TYPE_RESU='EVOL_THER')
        ...
        dico_equivalence_NOM_TYPE_CHAM = {'SIGM_NOEU': 'NOEU_SIEF_R', 'SIPM_ELNO': 'ELNO_SIEF_R', 'HYDR_ELNO': 'ELNO_HYDR_R', 'SIGM_ELGA': 'ELGA_SIEF_R', 
                                          'EPME_ELNO': 'ELNO_EPSI_R', 'MODE_FLAMB': 'NOEU_DEPL_R', 'EPSG_ELGA': 'ELGA_EPSI_R', 'QIRE_NOEU': 'NOEU_ERRE_R', 
                                          'EPMQ_ELGA': 'ELGA_EPSI_R', 'DEPL': 'NOEU_DEPL_R', 'INTE_NOEU': 'NOEU_INTE_R', 'DEGE_ELNO': 'ELNO_EPSI_R', 
                                          'EPSA_ELNO': 'ELNO_EPSI_R', 'EPVC_NOEU': 'NOEU_EPSI_R', 'SIPO_ELNO': 'ELNO_SIEF_R', 'DISS_ELGA': 'ELGA_DISS_R', 
                                          'SIEQ_ELNO': 'ELNO_SIEF_R', 'EPFD_NOEU': 'NOEU_EPSI_R', 'ERTH_NOEU': 'NOEU_ERRE_R', 'EPSP_ELGA': 'ELGA_EPSI_R',
                                          'PDIL_ELGA': 'ELGA_PDIL_R', 'META_ELNO': 'ELNO_VARI_R', 'EPFP_ELNO': 'ELNO_EPSI_R', 'EPME_ELGA': 'ELGA_EPSI_R', 
                                          'SIGM_ELNO': 'ELNO_SIEF_R', 'VARC_ELGA': 'ELGA_VARC_R', 'HYDR_NOEU': 'NOEU_HYDR_R', 'DEGE_ELGA': 'ELGA_EPSI_R', 
                                          'EPSP_NOEU': 'NOEU_EPSI_R', 'EPMG_ELNO': 'ELNO_EPSI_R', 'SIEF_ELGA': 'ELGA_SIEF_R', 'ERZ2_ELEM': 'ELEM_ERRE_R', 
                                          ...
                                          'ELEM_ENER_R', 'FORC_EXTE': 'NOEU_DEPL_R', 'ERZ1_ELEM': 'ELEM_ERRE_R', 'PRAC_ELNO': 'ELNO_PRAC_R', 'ETOT_ELGA': 
                                          'ELGA_ENER_R', 'SIZ1_NOEU': 'NOEU_SIEF_R', 'ERME_ELNO': 'ELNO_ERRE_R', 'NEUT': 'NOEU_NEUT_R', 'ENEL_ELGA': 'ELGA_ENER_R', 
                                          'EPEQ_ELGA': 'ELGA_EPSI_R', 'ERTH_ELNO': 'ELNO_ERRE_R', 'FLUX_ELGA': 'ELGA_FLUX_R', 'META_NOEU': 'NOEU_VARI_R', 
                                          'VARI_NOEU': 'NOEU_VAR2_R', 'INDL_ELGA': 'ELGA_INDL_R', 'EPSA_NOEU': 'NOEU_EPSI_R', 'VITE': 'NOEU_DEPL_R', 
                                          'SECO_ELEM': 'ELEM_NEUT_R', 'SISE_ELNO': 'ELNO_SIEF_R', 'FORC_NODA': 'NOEU_DEPL_R', 'ENEL_NOEU': 'NOEU_ENER_R', 
                                          'VITE_ABSOLU': 'NOEU_DEPL_R', 'FLHN_ELGA': 'ELGA_FLHN_R', 'ECIN_ELEM': 'ELEM_ENER_R', 'COHE_ELEM': 'ELEM_NEUT_R', 
                                          'QIZ1_ELEM': 'ELEM_ERRE_R', 'COMPORTEMENT': 'CART_COMPOR', 'THETA': 'NOEU_DEPL_R', 'DIVU': 'NOEU_EPSI_R', 
                                          'ETOT_ELNO': 'ELGA_ENER_R', 'PRAC_NOEU': 'NOEU_PRAC_R', 'UTXX_ELGA': 'RIEN', 'ENEL_ELNO': 'ELNO_ENER_R', 
                                          'ENDO_ELGA': 'ELGA_SIEF_R', 'EPMQ_ELNO': 'ELNO_EPSI_R', 'PRES': 'NOEU_PRES_C', 'EPFD_ELGA': 'ELGA_EPSI_R', 
                                          ...
                                          'UTXX_ELNO': 'RIEN', 'ACCE_ABSOLU': 'NOEU_DEPL_R', 'SING_ELNO': 'ELNO_SING_R', 'IRRA': 'NOEU_IRRA_R', 
                                          'SOUR_ELGA': 'ELGA_SOUR_R', 'EPSI_ELNO': 'ELNO_EPSI_R', 'EPSP_ELNO': 'ELNO_EPSI_R', 'SIPO_NOEU': 'NOEU_SIEF_R', 
                                          'COMPORTHER': 'CART_COMPOR', 'SIEQ_NOEU': 'NOEU_SIEF_R', 'EPEQ_ELNO': 'ELNO_EPSI_R', 'VARI_ELNO': 'ELNO_VARI_R', 
        ...
        for i in range(nbchamps):
                ...
                for j in range(nbinst):
                        ...
                        chamu[i][j] = CREA_CHAMP(NOM_CHAM=nom,
                                                 OPERATION='EXTR',
                                                 RESULTAT=RESU_MECA_UNIT,
                                                 INST=inst,
                                                 TYPE_CHAM= typeCham)

                        chamm[i][j] = CREA_CHAMP(ASSE=_F(CHAM_GD=chamu[i][j],
                                                          COEF_R=TEMP_FIN,
                                                          TOUT='OUI'),
                                                  MODELE=modele,
                                                  OPERATION='ASSE',
                        ...
                        arguments_affe += (_F(CHAM_GD=chamm[i][j],
                                              CHAM_MATER=cham_mater,
                                              INST=(inst, ),
                                              MODELE=modele),)

                if i==0:
                        resMmult = CREA_RESU(AFFE=arguments_affe,
                                            NOM_CHAM=nom,
                                            OPERATION='AFFE',
                                            TYPE_RESU='EVOL_ELAS')

                else:
                        resMmult = CREA_RESU(reuse=resMmult,
                                             RESULTAT=resMmult,
                                             AFFE=arguments_affe,
                                            NOM_CHAM=nom,
                                            OPERATION='AFFE',
