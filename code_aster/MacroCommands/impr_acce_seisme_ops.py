...
def impr_acce_seisme_ops(self, **args):
    ...
    if NOCI_REFE is not None:
        ...
        if noci_refe[0]["VMAX"] is not None:
            vmaxi_refe = noci_refe[0]["VMAX"]
        if noci_refe[0]["DMAX"] is not None:
            dmaxi_refe = noci_refe[0]["DMAX"]
        if noci_refe[0]["INTE_ARIAS"] is not None:
            inte_arias_refe = noci_refe[0]["INTE_ARIAS"]
        if noci_refe[0]["DUREE_PHASE_FORTE"] is not None:
            duree_refe = noci_refe[0]["DUREE_PHASE_FORTE"]
        if noci_refe[0]["VITE_ABSO_CUMU"] is not None:
            cav_refe = noci_refe[0]["VITE_ABSO_CUMU"]
        if noci_refe[0]["ACCE_SUR_VITE"] is not None:
            asv_refe = noci_refe[0]["ACCE_SUR_VITE"]
    ...
    __specci = CALC_FONCTION(COMB=_F(FONCTION=SPEC_OSCI, COEF=1.0 / ratio_hv))
    ...
    for idi in range(1, dim + 1):
        ...
        for ii in range(nb_tirage):
            ...
            if dim >= 2:
                __F_ACCE[ii] = RECU_FONCTION(
                    TABLE=TABLE,
                    FILTRE=(
                        _F(NOM_PARA="NUME_ORDRE", VALE_I=ii + 1),
            ...
            if DUREE is not None:
                ...
                for i_inst, inst in enumerate(l_inst):
                    if inst > DUREE:
                        break
                __FONC_D = DEFI_FONCTION(
                    NOM_PARA="INST", ABSCISSE=l_inst[:i_inst], ORDONNEE=l_acce[:i_inst]
                )
                ...
                IMPR_FONCTION(
                    FORMAT="XMGRACE",
                    PILOTE="PNG",
                    COURBE=_F(FONCTION=__FONC_D, MARQUEUR=0, LEGENDE=" Tirage %i" % (ii + 1)),
                    UNITE=unite,
                    TITRE=titre,
                    LEGENDE_X="Temps (s)",
                ...
                IMPR_FONCTION(
                    FORMAT="XMGRACE",
                    PILOTE="PNG",
                    COURBE=_F(FONCTION=__F_ACCE[ii], MARQUEUR=0, LEGENDE=" Tirage %i" % (ii + 1)),
                    UNITE=unite,
                    TITRE=titre,
                    LEGENDE_X="Temps (s)",
                    LEGENDE_Y="Acc\xe9l\xe9ration (m/s\\S2\\v{}\\z{})",
            ...
            if LIST_FREQ:
                ...
                __F_SRO[ii] = CALC_FONCTION(
                    SPEC_OSCI=_F(FONCTION=__F_ACCE[ii], NORME=pesant, AMOR_REDUIT=(amo_sro,))
                )
            ...
            __intea = INFO_FONCTION(
                NOCI_SEISME=_F(OPTION="INTE_ARIAS", FONCTION=__F_ACCE[ii], PESANTEUR=pesant)
            )
            tab = __intea.EXTR_TABLE()
            inte_arias.extend(tab.values()["INTE_ARIAS"])

            __duree = INFO_FONCTION(
                NOCI_SEISME=_F(OPTION="DUREE_PHAS_FORT", FONCTION=__F_ACCE[ii], PESANTEUR=pesant)
            )
        ...
        courbes = []
        for ii in range(nb_tirage):
            dic = {"FONCTION": __F_ACCE[ii], "LEGENDE": "Tirage %i" % (ii + 1), "MARQUEUR": 0}
            courbes.append(dic)
        ...
        courbes = []
        for ii in range(nb_tirage):
            dic = {"FONCTION": __F_DEPL[ii], "LEGENDE": "Tirage %i" % (ii + 1), "MARQUEUR": 0}
            courbes.append(dic)
        ...
        for i, triplet in enumerate(
            [amaxi_refe, vmaxi_refe, dmaxi_refe, asv_refe, duree_refe, cav_refe, inte_arias_refe]
        ):
            for ii, val in enumerate(triplet):
                vale_refe.extend([i + 1 + epsilon[ii], val])
        if vale_refe != []:
            __refe = DEFI_FONCTION(NOM_PARA="INST", VALE=vale_refe)

        vale_calc = []
        for i, lival in enumerate([amaxi, vmaxi, dmaxi, asv, duree, cav, inte_arias]):
            for ii, val in enumerate(lival):
                vale_calc.extend([i + 1 + epsilon[ii], val])
        __calc = DEFI_FONCTION(NOM_PARA="INST", VALE=vale_calc)
        ...
        l_crea_table_liste = []
        l_crea_table_liste.append({"PARA": para[0], "LISTE_K": criteres, "TYPE_K": "K24"})
        ...
        IMPR_TABLE(TABLE=__TAB, UNITE=unite)
        ...
        courbes = []
        dic = {"FONCTION": __specci, "LEGENDE": "Spectre cible", "MARQUEUR": 4}
        courbes.append(dic)

        dic = {
            "FONCTION": __MOY[idi - 1],
        ...
        for ii, fonc in enumerate(liste_sro):
            dic = {"FONCTION": fonc, "LEGENDE": "Tirage %i" % (ii + 1), "MARQUEUR": 0}
            courbes.append(dic)
        ...
        if SPEC_1_SIGMA:
            ...
            __SPMSIG = DEFI_FONCTION(NOM_PARA="FREQ", ABSCISSE=freq_cib, ORDONNEE=acce_cib3)

            __MEDI = CALC_FONCTION(FRACTILE=_F(FONCTION=liste_sro, FRACT=0.5))
            __PSIG = CALC_FONCTION(FRACTILE=_F(FONCTION=liste_sro, FRACT=0.84))
            __MSIG = CALC_FONCTION(FRACTILE=_F(FONCTION=liste_sro, FRACT=0.15))

            courbes = []

            dic = {
                "FONCTION": SPEC_1_SIGMA,
    ...
    if dim >= 2:
        ...
        for val in acce_spec:
            if val < 0:
                raise Exception("Erreur de développement, accélération spectrale négative")
            acce_spec2.append(val ** (1.0 / racn))

        __F_MOYG = DEFI_FONCTION(NOM_PARA="FREQ", ABSCISSE=freqs, ORDONNEE=acce_spec2)
