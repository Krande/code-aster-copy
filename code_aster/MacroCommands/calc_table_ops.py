...
def calc_table_ops(self, TABLE, ACTION, INFO, **args):
    ...
    for fOP in ACTION:
        ...
        if occ["OPERATION"] == "FILTRE":
            ...
            opts = [occ[k] for k in ("VALE", "VALE_I", "VALE_C", "VALE_K") if k in occ]
            kargs = {}
        ...
        if occ["OPERATION"] == "OPER":
            if occ.get("NOM_COLONNE") and len(occ["NOM_COLONNE"]) != len(
                occ["FORMULE"].getVariables()
            ):
                UTMESS("F", "TABLE0_19", vali=len(occ["FORMULE"].getVariables()))
            # ajout de la colonne dans la table
            tab.fromfunction(occ["NOM_PARA"], occ["FORMULE"], l_para=occ.get("NOM_COLONNE"))
            if INFO == 2:
                vectval = getattr(tab, occ["NOM_PARA"]).values()
                aster.affiche(
                    "MESSAGE", "Ajout de la colonne %s : %s" % (occ["NOM_PARA"], repr(vectval))
                )
