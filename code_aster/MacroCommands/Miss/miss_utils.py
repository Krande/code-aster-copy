...
class MISS_PARAMETER(object):
    ...
    def check(self):
        ...
        if self["TYPE_RESU"] not in ("CHARGE",):
            if self["LIST_FREQ"] is not None and self["TYPE_RESU"] not in (
                "FICHIER",
                "HARM_GENE",
                "TABLE_CONTROL",
            ):
            ...
            if self["BASE_MODALE"]:
                if self["AMOR_REDUIT"]:
                    ...
                    if nval < self["NBM_DYN"] + self["NBM_STA"]:
                        # on ajoute 0.
                        self._keywords["AMOR_REDUIT"].append(0.0)
...
def en_ligne(valeurs, format, cols, separateur=" ", format_ligne="%(valeurs)s"):
    ...
    while len(valeurs) > 0:
        ...
        ind += 1
        line = format_ligne % {"valeurs": separateur.join(lv), "index": ind, "index_1": ind + 1}
        res.append(line)
    return res
...
def calc_param_auto(l_coor_x, l_coor_y, l_coor_z, surf, coef_offset):
    ...
    if round(long, 0) < long:
        offset_max = int(round(long, 0) + 1.0)
    else:
    ...
    for ic, row in enumerate(tab):
        if ic == 0:
            ...
            sol_homogene = (
                ...
                and (row["NU"] == nu)
                and (row["RHO"] == rho)
                and (row["AMOR_HYST"] == hyst)
            )
    vs = (young / (2.0 * (1.0 + nu) * rho)) ** 0.5
    vp = (young * (1 - nu) / ((1 + nu) * (1 - 2 * nu) * rho)) ** 0.5
    betamin = hyst
