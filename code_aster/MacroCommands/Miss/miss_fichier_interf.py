...
def fichier_mvol(struct):
    ...
    for i, connec in enumerate(struct.maille_connec):
        ngr = i + 1
        cont.extend(
            en_ligne(connec, dict_format["sI"], 20, format_ligne="%%(valeurs)s     GR    %d" % ngr)
        )
    cont.append("")
...
def fichier_chp(param, struct):
    ...
    fmt_ligne = partial(
        en_ligne, format=dict_format["sR"], cols=3, format_ligne="%(index_1)6d%(valeurs)s"
    )
...
def fichier_sign(param):
    ...
    if param["FREQ_MIN"] is not None:
        lfreq = list(
            NP.arange(param["FREQ_MIN"], param["FREQ_MAX"] + param["FREQ_PAS"], param["FREQ_PAS"])
        )
