...
def syme_point_plan(P1, P2, P3, PM):
    ...
    t = (A*PM[0] + B*PM[1] + C*PM[2] + D) / (A*A + B*B + C*C) * -1.

    PM_sym = [2*(PM[0] + A*t) - PM[0], 2*(PM[1] + B*t) - PM[1],
              2*(PM[2] + C*t) - PM[2]]
...
def field_node_to_gauss(self, MODE, __FIELD, nameCmp, nameCmpResu):
    ...
    __FIELD = CREA_CHAMP(OPERATION='ASSE',
                         TYPE_CHAM='NOEU_EPSI_R',
                            MODELE=MODE,
                         ASSE=(_F(TOUT='OUI',
                                  CHAM_GD=__FIELD,
                                  NOM_CMP=(nameCmp,),
                                  NOM_CMP_RESU=(nameCmpResu,),),),)

    __FIELD_GAUSS = CREA_CHAMP(TYPE_CHAM='ELGA_EPSI_R',
                               OPERATION='DISC',
                               MODELE=MODE,
                               PROL_ZERO='OUI',
                               CHAM_GD=__FIELD)
...
def calc_area(self, MAILAREA, nameGroupMa):
    ...
    __MODE = AFFE_MODELE(MAILLAGE=MAILAREA,
                         AFFE=_F(TOUT='OUI',
                                 PHENOMENE='MECANIQUE',
                                 MODELISATION='3D'))

    __FIELD_AREA = CREA_CHAMP(OPERATION='AFFE',
    ...
    __RESU_AREA = CREA_RESU(OPERATION='AFFE',
                            TYPE_RESU='EVOL_ELAS',
                            NOM_CHAM='VARI_ELGA',
                            AFFE=_F(MODELE=__MODE,
                                    INST=0.0,
                                    CHAM_GD=__FIELD_AREA,),)

    __POST_AREA = POST_ELEM(RESULTAT=__RESU_AREA,
                            MODELE=__MODE, INST=0.0,
                            INTEGRALE=_F(NOM_CHAM='VARI_ELGA',
                                         GROUP_MA=nameGroupMa,
                                         NOM_CMP='X1',
                                         TYPE_MAILLE='2D'),)

    __AREA = __POST_AREA.EXTR_TABLE().values()['INTE_X1'][0]

    DETRUIRE(CONCEPT=(_F(NOM=__MODE),
                      _F(NOM=__FIELD_AREA),
                      _F(NOM=__RESU_AREA),
                      _F(NOM=__POST_AREA),
                        ),)
...
def calc_vari_area_no_bord(self, MAIL, NB_COUCHES, lNode1, lNode2, NODESBOUGE,
    ...
    if symeType == 'OUI':
        ...
        crea_group_no_from_no(self, MAIL, 'NOAREAX',
    ...
    if symeType == 'OUI':
        crea_group_no_from_no(self, __MAIL2, 'NOAREAX',
                                  lNode1[1:len(lNode1)-1])
        crea_group_no_from_no(self, __MAIL2, 'NOAREAY',
                                  lNode2[1:len(lNode2)-1])
    else:
        crea_group_no_from_no(self, __MAIL2, 'NOAREAX',
                lNode1[0][1:len(lNode1[0])-1] + lNode1[1][1:len(lNode1[1])-1])
        crea_group_no_from_no(self, __MAIL2, 'NOAREAY',
...
def calc_vari_area_no_midd(self, MAIL, NB_COUCHES, lNode, NODESBOUGE, lVect,
    ...
    if symeType == 'OUI':
        ...
        crea_group_no_from_no(self, MAIL, 'NOAREA',
    ...
    if symeType == 'OUI':
        ...
        crea_group_no_from_no(self, __MAIL2, 'NOAREA',
                lNode[0][1:len(lNode[0])-1] + lNode[1][1:len(lNode[1])-1])
...
def calc_vari_area_no_glob_one_elem(self, MAIL, NB_COUCHES, NPP, lNode,
    ...
    if symeType == 'OUI':
        ...
        crea_group_no_from_no(self, MAIL, 'NOAREAY',
                                  lNode[NPP][1:len(lNode[NPP])-1])
    else:
        crea_group_no_from_no(self, MAIL, 'NOAREAX',
                                  lNode[0][1][1:len(lNode[0][1])-1] +
                                  lNode[1][1][1:len(lNode[1][1])-1])
        crea_group_no_from_no(self, MAIL, 'NOAREAY',
...
def cal_strain_energy(self, MODE, __EPSI_ELGA, __SIGF):
    ...
    __CHW = CREA_CHAMP(OPERATION='ASSE', TYPE_CHAM='ELGA_NEUT_R',
                       ...
                       ASSE=(
                          _F(TOUT='OUI', CHAM_GD=__EPSI_ELGA,
                             NOM_CMP=('EPXX',), NOM_CMP_RESU=('X1',),),
                          _F(TOUT='OUI', CHAM_GD=__EPSI_ELGA,
                             NOM_CMP=('EPYY',), NOM_CMP_RESU=('X2',),),
                          _F(TOUT='OUI', CHAM_GD=__EPSI_ELGA,
                             NOM_CMP=('EPZZ',), NOM_CMP_RESU=('X3',),),
                          _F(TOUT='OUI', CHAM_GD=__EPSI_ELGA,
                             NOM_CMP=('EPXY',), NOM_CMP_RESU=('X4',),),
                          _F(TOUT='OUI', CHAM_GD=__EPSI_ELGA,
                             NOM_CMP=('EPXZ',), NOM_CMP_RESU=('X5',),),
                          _F(TOUT='OUI', CHAM_GD=__EPSI_ELGA,
                             NOM_CMP=('EPYZ',), NOM_CMP_RESU=('X6',),),
                          _F(TOUT='OUI', CHAM_GD=__SIGF,
                             NOM_CMP=('SIXX',), NOM_CMP_RESU=('X7',),),
                          _F(TOUT='OUI', CHAM_GD=__SIGF,
                             NOM_CMP=('SIYY',), NOM_CMP_RESU=('X8',),),
                          _F(TOUT='OUI', CHAM_GD=__SIGF,
                             NOM_CMP=('SIZZ',), NOM_CMP_RESU=('X9',),),
                          _F(TOUT='OUI', CHAM_GD=__SIGF,
                             NOM_CMP=('SIXY',), NOM_CMP_RESU=('X10',),),
                          _F(TOUT='OUI', CHAM_GD=__SIGF,
                             NOM_CMP=('SIXZ',), NOM_CMP_RESU=('X11',),),
                          _F(TOUT='OUI', CHAM_GD=__SIGF,
                             NOM_CMP=('SIYZ',), NOM_CMP_RESU=('X12',),),
                          ),)

    __FMULTW = FORMULE(NOM_PARA=('X1','X2','X3','X4','X5','X6',
                                 'X7','X8','X9','X10','X11','X12'),
                       VALE='(X1*X7/2)+(X2*X8/2)+(X3*X9/2)'+
                            '+(X4*X10)+(X5*X11)+(X6*X12)')

    __CHFMUW = CREA_CHAMP(OPERATION='AFFE',
    ...
    __WELAS = CREA_CHAMP(OPERATION='EVAL',
                         TYPE_CHAM='ELGA_NEUT_R',
                         CHAM_F=__CHFMUW,
                         CHAM_PARA=(__CHW,),)
...
def field_q_local(self, MAIL, nameGroupNo, nameCmp, value):
    ...
    __QX = CREA_CHAMP(OPERATION='AFFE',
...
def field_q_global(self, MAIL, NPP, nameCmp, value):
    ...
    __tmp = [{'NOM_CMP': ('DX','DY','DZ'), 'TOUT': 'OUI', 'VALE': (0.,0.,0.)}]
...
def grad_u(self, MAIL, MODE, MATE, namePara, DEPL, lInst):
    ...
    __DEPU = CREA_RESU(OPERATION='AFFE',
                       NOM_CHAM='DEPL',
                       TYPE_RESU='EVOL_ELAS',
                       AFFE=_F(CHAM_GD=__DEPU,
                               INST=lInst),)

    __GRADEP = CALC_CHAMP(reuse=__DEPU,
                          MODELE=MODE,
                          CHAM_MATER=MATE,
                          RESULTAT=__DEPU,
...
def grad_noeu(self, MAIL, MODE, MATE, __FIELD, inst):
    ...
    __FIELD_CALX = CREA_CHAMP(OPERATION='AFFE',
                              TYPE_CHAM='NOEU_NEUT_F',
                              MAILLAGE=MAIL,
                              AFFE=_F(TOUT='OUI',
                                      NOM_CMP=('X1','X2','X3'),
                                      VALE_F=(__FIELD_NOEU_X1,
                                              __FIELD_NOEU_X2,
                                              __FIELD_NOEU_X3),),)

    __FIELD_CAL = CREA_CHAMP(OPERATION='EVAL',
                             TYPE_CHAM='NOEU_NEUT_R',
                             CHAM_F=__FIELD_CALX,
                             CHAM_PARA=__FIELD,)

    __FIELD_CAL = CREA_CHAMP(OPERATION='ASSE',
                             MODELE=MODE,
                             TYPE_CHAM ='NOEU_DEPL_R',
                             ASSE=_F(CHAM_GD=__FIELD_CAL,
                                     TOUT='OUI',
                                     NOM_CMP=('X1','X2','X3'),
                                     NOM_CMP_RESU=('DX','DY','DZ'),),)

    __RFIELD = CREA_RESU(OPERATION='AFFE',
                         NOM_CHAM='DEPL',
                         TYPE_RESU='EVOL_ELAS',
                         AFFE=_F(CHAM_GD=__FIELD_CAL,
                                 INST=inst),)

    __GRAD_FIELD = CALC_CHAMP(reuse=__RFIELD,
                              MODELE=MODE,
                              CHAM_MATER=MATE,
                              RESULTAT=__RFIELD,
                              DEFORMATION=('EPSI_ELGA'),)

    __GRAD_FIELD = CREA_CHAMP(TYPE_CHAM='ELGA_EPSI_R',
                              OPERATION='EXTR',
                              RESULTAT=__GRAD_FIELD, INST=inst,
                              NOM_CHAM='EPSI_ELGA',)

    DETRUIRE(CONCEPT=(_F(NOM=__FIELD_CAL),
...
def grad_elno(self, MAIL, MODE, MATE, listElemTMAIL, __EPSI_ELGA,
                        __FIELD_CAL, inst):
    """
    ...
    for iElemTMAIL in listElemTMAIL:
        ...
        __DEPE = CREA_CHAMP(OPERATION='AFFE',
                            TYPE_CHAM='NOEU_DEPL_R',
                            MAILLAGE=MAIL,
                            AFFE=__tmpLocal)

        __RDEPE = CREA_RESU(OPERATION='AFFE',
                            NOM_CHAM='DEPL',
                            TYPE_RESU='EVOL_ELAS',
                            AFFE=_F(CHAM_GD=__DEPE,
                                    INST=inst),)

        __RDEPE = CALC_CHAMP(reuse=__RDEPE,
                             MODELE=MODE,
                             CHAM_MATER=MATE,
                             RESULTAT=__RDEPE,
                             DEFORMATION=('EPSI_ELGA'),)

        __GRAE = CREA_CHAMP(TYPE_CHAM='ELGA_EPSI_R',
                            OPERATION='EXTR',INST=inst,
                            RESULTAT=__RDEPE,
                            NOM_CHAM='EPSI_ELGA',)
    ...
    DETRUIRE(CONCEPT=(_F(NOM=__DEPE),
                      _F(NOM=__RDEPE),
                      _F(NOM=__GRAE),
                        ),)
...
def cal_j01(self, MODE, TMAIL, lInst, __WELAS, __GQX, __GQY, __GQZ):
    ...
    __CHJ01 = CREA_CHAMP(OPERATION='ASSE', TYPE_CHAM='ELGA_NEUT_R',
                         MODELE=MODE, PROL_ZERO='OUI',
                         ASSE=(
                            _F(GROUP_MA=TMAIL, CHAM_GD=__WELAS,
                               NOM_CMP=('X13',), NOM_CMP_RESU=('X1',),),
                            _F(GROUP_MA=TMAIL, CHAM_GD=__GQX,
                               NOM_CMP=('EPXX',), NOM_CMP_RESU=('X2',),),
                            _F(GROUP_MA=TMAIL, CHAM_GD=__GQY,
                               NOM_CMP=('EPYY',), NOM_CMP_RESU=('X3',),),
    ...
    __CHFMUJ01 = CREA_CHAMP(OPERATION='AFFE',
                            TYPE_CHAM='ELGA_NEUT_F',
                            MODELE=MODE, PROL_ZERO='OUI',
                            AFFE=_F(GROUP_MA=TMAIL, NOM_CMP='X5',
                                    VALE_F=__FMULTJ01),)

    __CHJ01INT = CREA_CHAMP(OPERATION='EVAL',
                            TYPE_CHAM='ELGA_NEUT_R',
    ...
    __J01 = POST_ELEM(RESULTAT=__RESUJ01,
...
def cal_j02(self, MODE, TMAIL, lInst, __SIGF, __GDEPX, __GDEPY, __GDEPZ,
                               __GQX, __GQY, __GQZ):
    """
    Calculation term J02 = SIGF * GRAD U * GRAD Q
    """

    __CHGRAUQ = CREA_CHAMP(OPERATION='ASSE',
                           TYPE_CHAM='ELGA_NEUT_R',
                           MODELE=MODE, PROL_ZERO='OUI',
                           ASSE=(
                              _F(GROUP_MA=TMAIL, CHAM_GD=__GDEPX,
                                 NOM_CMP=('EPXX',), NOM_CMP_RESU=('X1',),),
                              _F(GROUP_MA=TMAIL, CHAM_GD=__GDEPX,
                                 NOM_CMP=('EPXY',), NOM_CMP_RESU=('X2',),),
                              _F(GROUP_MA=TMAIL, CHAM_GD=__GDEPX,
                                 NOM_CMP=('EPXZ',), NOM_CMP_RESU=('X3',),),
                              _F(GROUP_MA=TMAIL, CHAM_GD=__GDEPY,
                                 NOM_CMP=('EPXY',), NOM_CMP_RESU=('X4',),),
                              _F(GROUP_MA=TMAIL, CHAM_GD=__GDEPY,
                                 NOM_CMP=('EPYY',), NOM_CMP_RESU=('X5',),),
                              _F(GROUP_MA=TMAIL, CHAM_GD=__GDEPY,
                                 NOM_CMP=('EPYZ',), NOM_CMP_RESU=('X6',),),
                              _F(GROUP_MA=TMAIL, CHAM_GD=__GDEPZ,
                                 NOM_CMP=('EPXZ',), NOM_CMP_RESU=('X7',),),
                              _F(GROUP_MA=TMAIL, CHAM_GD=__GDEPZ,
                                 NOM_CMP=('EPYZ',), NOM_CMP_RESU=('X8',),),
                              _F(GROUP_MA=TMAIL, CHAM_GD=__GDEPZ,
                                 NOM_CMP=('EPZZ',), NOM_CMP_RESU=('X9',),),
                              _F(GROUP_MA=TMAIL, CHAM_GD=__GQX,
                                 NOM_CMP=('EPXX',), NOM_CMP_RESU=('X10',),),
                              _F(GROUP_MA=TMAIL, CHAM_GD=__GQX,
                                 NOM_CMP=('EPXY',), NOM_CMP_RESU=('X11',),),
                              _F(GROUP_MA=TMAIL, CHAM_GD=__GQX,
                                 NOM_CMP=('EPXZ',), NOM_CMP_RESU=('X12',),),
                              _F(GROUP_MA=TMAIL, CHAM_GD=__GQY,
                                 NOM_CMP=('EPXY',), NOM_CMP_RESU=('X13',),),
                              _F(GROUP_MA=TMAIL, CHAM_GD=__GQY,
                                 NOM_CMP=('EPYY',), NOM_CMP_RESU=('X14',),),
                              _F(GROUP_MA=TMAIL, CHAM_GD=__GQY,
                                 NOM_CMP=('EPYZ',), NOM_CMP_RESU=('X15',),),
                              _F(GROUP_MA=TMAIL, CHAM_GD=__GQZ,
                                 NOM_CMP=('EPXZ',), NOM_CMP_RESU=('X16',),),
                              _F(GROUP_MA=TMAIL, CHAM_GD=__GQZ,
                                 NOM_CMP=('EPYZ',), NOM_CMP_RESU=('X17',),),
                              _F(GROUP_MA=TMAIL, CHAM_GD=__GQZ,
    ...
    __FMULTGRAUQ_X21 = FORMULE(NOM_PARA=('X1','X2','X3','X12','X15','X18'),
                               VALE='X1*(X12*2)+(X2*2)*(X15*2)+(X3*2)*X18')
    __FMULTGRAUQ_X22 = FORMULE(NOM_PARA=('X4','X5','X6','X10','X13','X16'),
                               VALE='(X4*2)*X10+X5*(X13*2)+(X6*2)*(X16*2)')
    __FMULTGRAUQ_X23 = FORMULE(NOM_PARA=('X4','X5','X6','X11','X14','X17'),
                               VALE='(X4*2)*(X11*2)+X5*X14+(X6*2)*(X17*2)')
    __FMULTGRAUQ_X24 = FORMULE(NOM_PARA=('X4','X5','X6','X12','X15','X18'),
                               VALE='(X4*2)*(X12*2)+X5*(X15*2)+(X6*2)*X18')
    __FMULTGRAUQ_X25 = FORMULE(NOM_PARA=('X7','X8','X9','X10','X13','X16'),
                               VALE='(X7*2)*X10+(X8*2)*(X13*2)+X9*(X16*2)')
    __FMULTGRAUQ_X26 = FORMULE(NOM_PARA=('X7','X8','X9','X11','X14','X17'),
    ...
    __CHFMUGRAUQ = CREA_CHAMP(OPERATION='AFFE',
                              TYPE_CHAM='ELGA_NEUT_F',
                              MODELE=MODE, PROL_ZERO='OUI',
                              AFFE=_F(GROUP_MA=TMAIL,
                                      NOM_CMP=('X19','X20','X21',
                                               'X22','X23','X24',
                                               'X25','X26','X27',),
                                      VALE_F=(__FMULTGRAUQ_X19,
                                              __FMULTGRAUQ_X20,
                                              __FMULTGRAUQ_X21,
                                              __FMULTGRAUQ_X22,
                                              __FMULTGRAUQ_X23,
    ...
    DETRUIRE(CONCEPT=(_F(NOM=__CHGRAUQ),
    ...
    __CHJ02 = CREA_CHAMP(OPERATION='ASSE',
                         TYPE_CHAM='ELGA_NEUT_R',
                         MODELE=MODE, PROL_ZERO='OUI',
                         ASSE=(
                            _F(GROUP_MA=TMAIL, CHAM_GD=__CHGRAUMQ,
                               NOM_CMP=('X19',), NOM_CMP_RESU=('X19',),),
                            _F(GROUP_MA=TMAIL, CHAM_GD=__CHGRAUMQ,
                               NOM_CMP=('X20',), NOM_CMP_RESU=('X20',),),
                            _F(GROUP_MA=TMAIL, CHAM_GD=__CHGRAUMQ,
                               NOM_CMP=('X21',), NOM_CMP_RESU=('X21',),),
                            _F(GROUP_MA=TMAIL, CHAM_GD=__CHGRAUMQ,
                               NOM_CMP=('X22',), NOM_CMP_RESU=('X22',),),
                            _F(GROUP_MA=TMAIL, CHAM_GD=__CHGRAUMQ,
                               NOM_CMP=('X23',), NOM_CMP_RESU=('X23',),),
                            _F(GROUP_MA=TMAIL, CHAM_GD=__CHGRAUMQ,
                               NOM_CMP=('X24',), NOM_CMP_RESU=('X24',),),
                            _F(GROUP_MA=TMAIL, CHAM_GD=__CHGRAUMQ,
                               NOM_CMP=('X25',), NOM_CMP_RESU=('X25',),),
                            _F(GROUP_MA=TMAIL, CHAM_GD=__CHGRAUMQ,
                               NOM_CMP=('X26',), NOM_CMP_RESU=('X26',),),
                            _F(GROUP_MA=TMAIL, CHAM_GD=__CHGRAUMQ,
                               NOM_CMP=('X27',), NOM_CMP_RESU=('X27',),),
                            _F(GROUP_MA=TMAIL, CHAM_GD=__SIGF,
                               NOM_CMP=('SIXX',), NOM_CMP_RESU=('X1',),),
                            _F(GROUP_MA=TMAIL, CHAM_GD=__SIGF,
                               NOM_CMP=('SIXY',), NOM_CMP_RESU=('X2',),),
                            _F(GROUP_MA=TMAIL, CHAM_GD=__SIGF,
                               NOM_CMP=('SIXZ',), NOM_CMP_RESU=('X3',),),
                            _F(GROUP_MA=TMAIL, CHAM_GD=__SIGF,
                               NOM_CMP=('SIYY',), NOM_CMP_RESU=('X4',),),
                            _F(GROUP_MA=TMAIL, CHAM_GD=__SIGF,
                               NOM_CMP=('SIYZ',), NOM_CMP_RESU=('X5',),),
                            _F(GROUP_MA=TMAIL, CHAM_GD=__SIGF,
                               NOM_CMP=('SIZZ',), NOM_CMP_RESU=('X6',),),
                            ),)

    __FMULTJ02 = FORMULE(NOM_PARA=('X19','X20','X21','X22','X23',
                                   'X24','X25','X26','X27','X1',
                                   'X2','X3','X4','X5','X6',),
                         VALE='(X19*X1)+(X20*X2)+(X21*X3)+(X22*X2)+'+
                              '(X23*X4)+(X24*X5)+(X25*X3)+(X26*X5)+(X27*X6)')

    __CHFMUJ02 = CREA_CHAMP(OPERATION='AFFE',
                            TYPE_CHAM='ELGA_NEUT_F',
                            MODELE=MODE, PROL_ZERO='OUI',
    ...
    __J02 = POST_ELEM(RESULTAT=__RESUJ02, MODELE=MODE, INST=lInst,
                      INTEGRALE=_F(NOM_CHAM='VARI_ELGA',
                                   GROUP_MA=TMAIL,
                                   NOM_CMP='X7',
                                   TYPE_MAILLE='3D'),)

    DETRUIRE(CONCEPT=(_F(NOM=__CHGRAUMQ),
                      _F(NOM=__CHJ02),
                      _F(NOM=__FMULTJ02),
                      _F(NOM=__CHFMUJ02),
                      _F(NOM=__CHJ02INT),
...
def cal_j04(self, MODE, TMAIL, lInst, SIG_CMP, EPS_CMP, __SIGF, dicGradEps,
                __QX_GAUSS, __QY_GAUSS, __QZ_GAUSS):
    """
    ...
    for iSIG, iEPS in zip(SIG_CMP, EPS_CMP):
        ...
        __CHJ04 = CREA_CHAMP(OPERATION='ASSE',
                             TYPE_CHAM='ELGA_NEUT_R',
                             MODELE=MODE, PROL_ZERO='OUI',
                             ASSE=(
                                _F(GROUP_MA=TMAIL, CHAM_GD=__SIGF,
                                   NOM_CMP=(iSIG,), NOM_CMP_RESU=('X1',),),
                                _F(GROUP_MA=TMAIL, CHAM_GD=dicGradEps[iEPS],
                                   NOM_CMP=('EPXX',), NOM_CMP_RESU=('X2',),),
                                _F(GROUP_MA=TMAIL, CHAM_GD=dicGradEps[iEPS],
                                   NOM_CMP=('EPXY',), NOM_CMP_RESU=('X3',),),
                                _F(GROUP_MA=TMAIL, CHAM_GD=dicGradEps[iEPS],
                                   NOM_CMP=('EPXZ',), NOM_CMP_RESU=('X4',),),
                                _F(GROUP_MA=TMAIL, CHAM_GD=__QX_GAUSS,
                                   NOM_CMP=('EPXX',), NOM_CMP_RESU=('X5',),),
                                _F(GROUP_MA=TMAIL, CHAM_GD=__QY_GAUSS,
                                   NOM_CMP=('EPYY',), NOM_CMP_RESU=('X6',),),
        ...
        __CHFMUJ04 = CREA_CHAMP(OPERATION='AFFE',
                                TYPE_CHAM='ELGA_NEUT_F',
                                MODELE=MODE, PROL_ZERO='OUI',
        ...
        DETRUIRE(CONCEPT=(_F(NOM=__CHJ04),
                          _F(NOM=__FMULTJ04),
                          _F(NOM=__CHFMUJ04),
                          _F(NOM=__CHJ04INT),
                          _F(NOM=__RESUJ04),
...
def cal_j05(self, MODE, TMAIL, lInst, __GRAD_WELAS,
                __QX_GAUSS, __QY_GAUSS, __QZ_GAUSS):
    """
    Calculation term J05 = GRAD W * Q
    """

    __CHJ05 = CREA_CHAMP(OPERATION='ASSE',
                         TYPE_CHAM='ELGA_NEUT_R',
                         MODELE=MODE, PROL_ZERO='OUI',
                         ASSE=(
                            _F(GROUP_MA=TMAIL, CHAM_GD=__GRAD_WELAS,
                               NOM_CMP=('EPXX',), NOM_CMP_RESU=('X1',),),
                            _F(GROUP_MA=TMAIL, CHAM_GD=__GRAD_WELAS,
                               NOM_CMP=('EPXY',), NOM_CMP_RESU=('X2',),),
                            _F(GROUP_MA=TMAIL, CHAM_GD=__GRAD_WELAS,
    ...
    __J05 = POST_ELEM(RESULTAT=__RESUJ05,
                      MODELE=MODE, INST=lInst,
                      INTEGRALE=_F(NOM_CHAM='VARI_ELGA',
                                   GROUP_MA=TMAIL,
                                   NOM_CMP='X7',
...
def get_result2D(self, J, __J01, linst, liord, nom_modelisation, NB_COUCHES,
                   e, nu, checksym, TITRE):
    ...
    if checksym == 'OUI':
        if nom_modelisation == 'D_PLAN':
            K = [np.sqrt(iJ * e / (1. - nu ** 2)) for iJ in J]
...
def get_result(self, __J, volume, NB_COUCHES, TITRE, TPFISS, FOND_FISS,
                   iNP, NP, listNP, inst, iord, tab_result, nume):
    """
    Print results
    """
...
#   Get mesh, model, material and results
    ...
    if ndim == 2:
        ...
        DEFI_GROUP(reuse=MAIL,
                    MAILLAGE=MAIL,
                    CREA_GROUP_NO=(_F(NOM='NMAIL',
                                      GROUP_MA='MBOUGER',),),)

        DEFI_GROUP(reuse=MAIL,
                    MAILLAGE=MAIL,
                    CREA_GROUP_MA=_F(NOM='MMAIL',
                                     OPTION='APPUI',
                                     GROUP_NO='NMAIL',
                                     TYPE_APPUI='AU_MOINS_UN',),)
        ...
        if modified_J:
            ...
            DEFI_GROUP(reuse=MAIL,
                        MAILLAGE=MAIL,
                        CREA_GROUP_MA=_F(NOM='MMAIL_IMPR',
                                        DIFFE=('MMAIL', 'MBOUGER_IMPR'),),)
        ...
        __QX_NOEU = CREA_CHAMP(OPERATION='AFFE',
                            TYPE_CHAM='NOEU_EPSI_R',
                            MAILLAGE=MAIL,
                            AFFE=(_F(TOUT='OUI',
        ...
        __QY_NOEU = CREA_CHAMP(OPERATION='AFFE',
                            TYPE_CHAM='NOEU_EPSI_R',
                            MAILLAGE=MAIL,
                            AFFE=(_F(TOUT='OUI',
                                        NOM_CMP=('EPXX','EPYY','EPZZ','EPXY'),
        ...
        DETRUIRE(CONCEPT=(_F(NOM=__QX_NOEU),
                          _F(NOM=__QY_NOEU),
                        ),)
        ...
        if MODELISATION == 'AXIS':
            ...
            __CHCOOR_NOEU = CREA_CHAMP(OPERATION='EXTR',
                                       TYPE_CHAM='NOEU_GEOM_R',
                                       NOM_CHAM='GEOMETRIE',
                                       MAILLAGE=MAIL,)

            __CHCOOR_ELGA = CREA_CHAMP(OPERATION='DISC',
            ...
            __ncoorCrackTip = POST_RELEVE_T(ACTION=_F(RESULTAT=RESULTAT,
            ...
            tcoorCrackTip = __ncoorCrackTip.EXTR_TABLE().NUME_ORDRE\
        ...
        for iord, inst in zip(liord, linst):
            ...
            __RDEPIX = CALC_CHAMP(reuse=__RDEPIX,
                                MODELE=MODE,
                                CHAM_MATER=MATE,
                                RESULTAT=__RDEPIX,
                                DEFORMATION=('EPSI_ELGA'),)

            __GDEPIX = CREA_CHAMP(TYPE_CHAM='ELGA_EPSI_R',
                                 OPERATION='EXTR',
                                 RESULTAT=__RDEPIX,
            ...
            __RESU = CALC_CHAMP(reuse=__RESU,
                                RESULTAT=__RESU,
                                DEFORMATION=('EPSI_NOEU', 'EPSI_ELGA'),)

            __EPSNOEU = CREA_CHAMP(TYPE_CHAM='NOEU_EPSI_R',
                                    OPERATION='EXTR',
                                    RESULTAT=__RESU,
                                    NOM_CHAM='EPSI_NOEU',
                                    NUME_ORDRE=iord,)
            ...
            if ETAT_INIT is None:
                ...
                __EPSF = CREA_CHAMP(TYPE_CHAM='ELGA_EPSI_R',
                                    OPERATION='EXTR',
                                    RESULTAT=__RESU,
                                    NOM_CHAM='EPSI_ELGA',
                                    NUME_ORDRE=iord,)
                ...
                __CHW = CREA_CHAMP(OPERATION='ASSE', TYPE_CHAM='ELGA_NEUT_R',
                            ...
                            ASSE=(
                                ...
                                _F(TOUT='OUI', CHAM_GD=__EPSF,
                                    NOM_CMP=('EPYY',), NOM_CMP_RESU=('X3',),),
                                _F(TOUT='OUI', CHAM_GD=__SIGF,
                                    NOM_CMP=('SIXX',), NOM_CMP_RESU=('X4',),),
                                _F(TOUT='OUI', CHAM_GD=__SIGF,
                                    NOM_CMP=('SIXY',), NOM_CMP_RESU=('X5',),),
                                _F(TOUT='OUI', CHAM_GD=__SIGF,
                                    NOM_CMP=('SIYY',), NOM_CMP_RESU=('X6',),),
                                _F(TOUT='OUI', CHAM_GD=__EPS0ELGA,
                                    NOM_CMP=('EPXX',), NOM_CMP_RESU=('X7',),),
                                _F(TOUT='OUI', CHAM_GD=__EPS0ELGA,
                                    NOM_CMP=('EPXY',), NOM_CMP_RESU=('X8',),),
                                _F(TOUT='OUI', CHAM_GD=__EPS0ELGA,
                                    NOM_CMP=('EPYY',), NOM_CMP_RESU=('X9',),),
                                ),)

                __FMULTW = FORMULE(NOM_PARA=('X1','X2','X3','X4','X5','X6','X7','X8','X9'),
                                    VALE='((X1-X7)*X4/2)+((X2-X8)*X5)+((X3-X9)*X6/2)')

                __CHFMUW = CREA_CHAMP(OPERATION='AFFE', TYPE_CHAM='ELGA_NEUT_F',
                            MODELE=MODE, PROL_ZERO='OUI',
                            AFFE=_F(TOUT='OUI', NOM_CMP='X10', VALE_F=__FMULTW),)

                __WELAS = CREA_CHAMP(OPERATION='EVAL', TYPE_CHAM='ELGA_NEUT_R',
                            CHAM_F=__CHFMUW, CHAM_PARA=(__CHW,),)
            ...
            if MODELISATION != 'AXIS':
                ...
                __CHJ01 = CREA_CHAMP(OPERATION='ASSE', TYPE_CHAM='ELGA_NEUT_R',
                            ...
                            ASSE=(
                                _F(TOUT='OUI', CHAM_GD=__WELAS ,
                                    NOM_CMP=('X10',), NOM_CMP_RESU=('X1',),),
                                _F(TOUT='OUI', CHAM_GD=__GDEPIX,
                                    NOM_CMP=('EPXX',), NOM_CMP_RESU=('X2',),),
                                _F(TOUT='OUI', CHAM_GD=__GDEPIY,
                                    NOM_CMP=('EPYY',), NOM_CMP_RESU=('X3',),),
                                ),)

                __FMULTJ01 = FORMULE(NOM_PARA=('X1','X2','X3'),
                                    VALE='X1*(X2+X3)')
                ...
                __CHJ01 = CREA_CHAMP(OPERATION='ASSE', TYPE_CHAM='ELGA_NEUT_R',
                            ...
                            ASSE=(
                                ...
                                _F(TOUT='OUI', CHAM_GD=__CHCOOR_ELGA,
                                    NOM_CMP=('X',), NOM_CMP_RESU=('X5',),),
                                ),)

                __FMULTJ01 = FORMULE(NOM_PARA=('X1','X2','X3','X5'),
                                    VALE='(X1*(X2+X3))*X5')

            __CHFMUJ01 = CREA_CHAMP(OPERATION='AFFE', TYPE_CHAM='ELGA_NEUT_F',
                        MODELE=MODE, PROL_ZERO='OUI',
            ...
            __J01 = POST_ELEM(RESULTAT=__RESUJ01, MODELE=MODE, INST=inst,
                        INTEGRALE=_F(NOM_CHAM='VARI_ELGA', GROUP_MA='MMAIL',
                                    NOM_CMP='X4', TYPE_MAILLE='2D'),)
            ...
            __CHGRAUQ = CREA_CHAMP(OPERATION='ASSE', TYPE_CHAM='ELGA_NEUT_R',
                        MODELE=MODE, PROL_ZERO='OUI',
                        ASSE=(
                            _F(TOUT='OUI', CHAM_GD=__GDEPX,
                                NOM_CMP=('EPXX',), NOM_CMP_RESU=('X1',),),
                            _F(TOUT='OUI', CHAM_GD=__GDEPX,
                                NOM_CMP=('EPXY',), NOM_CMP_RESU=('X2',),),
                            _F(TOUT='OUI', CHAM_GD=__GDEPY,
                                NOM_CMP=('EPXY',), NOM_CMP_RESU=('X3',),),
                            _F(TOUT='OUI', CHAM_GD=__GDEPY,
                                NOM_CMP=('EPYY',), NOM_CMP_RESU=('X4',),),
                            _F(TOUT='OUI', CHAM_GD=__GDEPIX,
                                NOM_CMP=('EPXX',), NOM_CMP_RESU=('X5',),),
                            _F(TOUT='OUI', CHAM_GD=__GDEPIX,
                                NOM_CMP=('EPXY',), NOM_CMP_RESU=('X6',),),
                            _F(TOUT='OUI', CHAM_GD=__GDEPIY,
                                NOM_CMP=('EPXY',), NOM_CMP_RESU=('X7',),),
                            _F(TOUT='OUI', CHAM_GD=__GDEPIY,
                                NOM_CMP=('EPYY',), NOM_CMP_RESU=('X8',),),
                            ),)

            __FMULTGRAUQ_X9  = FORMULE(NOM_PARA=('X1','X2','X5','X7'),
                                VALE='(X1*X5)+((X2*2)*(X7*2))')

            __FMULTGRAUQ_X10 = FORMULE(NOM_PARA=('X1','X2','X6','X8'),
                                VALE='(X1*(X6*2))+((X2*2)*X8)')
            ...
            __CHFMUGRAUQ = CREA_CHAMP(OPERATION='AFFE', TYPE_CHAM='ELGA_NEUT_F',
                        MODELE=MODE, PROL_ZERO='OUI',
            ...
            if MODELISATION != 'AXIS':
                ...
                __CHJ02 = CREA_CHAMP(OPERATION='ASSE', TYPE_CHAM='ELGA_NEUT_R',
                            MODELE=MODE, PROL_ZERO='OUI',
                            ASSE=(
                                _F(TOUT='OUI', CHAM_GD=__CHGRAUMQ,
                                    NOM_CMP=('X9',), NOM_CMP_RESU=('X9',),),
                                _F(TOUT='OUI', CHAM_GD=__CHGRAUMQ,
                                    NOM_CMP=('X10',), NOM_CMP_RESU=('X10',),),
                                _F(TOUT='OUI', CHAM_GD=__CHGRAUMQ,
                                    NOM_CMP=('X11',), NOM_CMP_RESU=('X11',),),
                                _F(TOUT='OUI', CHAM_GD=__CHGRAUMQ,
                                    NOM_CMP=('X12',), NOM_CMP_RESU=('X12',),),
                                _F(TOUT='OUI', CHAM_GD=__SIGF,
                                    NOM_CMP=('SIXX',), NOM_CMP_RESU=('X13',),),
                                _F(TOUT='OUI', CHAM_GD=__SIGF,
                                    NOM_CMP=('SIXY',), NOM_CMP_RESU=('X14',),),
                                _F(TOUT='OUI', CHAM_GD=__SIGF,
            ...
            __J02 = POST_ELEM(RESULTAT=__RESUJ02, MODELE=MODE, INST=inst,
                        INTEGRALE=_F(NOM_CHAM='VARI_ELGA', GROUP_MA='MMAIL',
                                    NOM_CMP='X16', TYPE_MAILLE='2D'),)
                ...
                for ISIGCOMP, IEPSCOMP in zip(SIGCOMP, EPSCOMP):
                    ...
                    if MODELISATION != 'AXIS':
                        ...
                        __CHJ03 = CREA_CHAMP(OPERATION='ASSE', TYPE_CHAM='ELGA_NEUT_R',
                                    ...
                                    ASSE=(
                                        _F(TOUT='OUI', CHAM_GD=__SIGF,
                                            NOM_CMP=(ISIGCOMP,), NOM_CMP_RESU=('X1',),),
                                        _F(TOUT='OUI', CHAM_GD=__GREPS0,
                                            NOM_CMP=('EPXX',), NOM_CMP_RESU=('X2',),),
                                        _F(TOUT='OUI', CHAM_GD=__GREPS0,
                                            NOM_CMP=('EPXY',), NOM_CMP_RESU=('X3',),),
                                        _F(TOUT='OUI', CHAM_GD=__QX_NOEU_GAUSS,
                                            NOM_CMP=('EPXX',), NOM_CMP_RESU=('X4',),),
                                        _F(TOUT='OUI', CHAM_GD=__QY_NOEU_GAUSS,
                                            NOM_CMP=('EPYY',), NOM_CMP_RESU=('X5',),),
                                        ),)

                        __FMULTJ03 = FORMULE(NOM_PARA=('X1','X2','X3','X4','X5'),
                        ...
                        __CHJ03 = CREA_CHAMP(OPERATION='ASSE', TYPE_CHAM='ELGA_NEUT_R',
                                    ...
                                    ASSE=(
                                        ...
                                        _F(TOUT='OUI', CHAM_GD=__QY_NOEU_GAUSS,
                                            NOM_CMP=('EPYY',), NOM_CMP_RESU=('X5',),),
                                        _F(TOUT='OUI', CHAM_GD=__CHCOOR_ELGA,
                                            NOM_CMP=('X',), NOM_CMP_RESU=('X7',),),
                                        ),)

                        __FMULTJ03 = FORMULE(NOM_PARA=('X1','X2','X3','X4','X5','X7'),
                    ...
                    DETRUIRE(CONCEPT=(_F(NOM=__GREPS0),
                                      _F(NOM=__CHJ03),
                                      _F(NOM=__FMULTJ03),
                                      _F(NOM=__CHFMUJ03),
                                      _F(NOM=__CHJ03INT),
            ...
            if modified_J:
                ...
                for ISIGCOMP, IEPSCOMP in zip(SIGCOMP, EPSCOMP):
                    ...
                    if MODELISATION != 'AXIS':
                        ...
                        __FMULTJ04 = FORMULE(NOM_PARA=('X1','X2','X3','X4','X5'),
                                            VALE='(X1*X2*X4)+(X1*(X3*2)*X5)')
                        ...
                        __FMULTJ04 = FORMULE(NOM_PARA=('X1','X2','X3','X4','X5','X7'),
                                            VALE='((X1*X2*X4)+(X1*(X3*2)*X5))*X7')
                    ...
                    DETRUIRE(CONCEPT=(_F(NOM=__GREPS),
                                      _F(NOM=__CHJ04),
                                      _F(NOM=__FMULTJ04),
                                      _F(NOM=__CHFMUJ04),
                                      _F(NOM=__CHJ04INT),
                ...
                __CHWELASNOEUX = CREA_CHAMP(OPERATION='AFFE',
                                    ...
                                    MAILLAGE=MAIL,
                                    AFFE=_F(TOUT='OUI', NOM_CMP=('X1','X2'),
                                            VALE_F=(__WELASNOEUX1, __WELASNOEUX2)))

                __CHWELASNOEU = CREA_CHAMP(OPERATION='EVAL',
                                    TYPE_CHAM='NOEU_NEUT_R',
                ...
                __CHWELASNOEU = CREA_CHAMP(OPERATION='ASSE',
                                    MODELE=MODE,
                                    TYPE_CHAM ='NOEU_DEPL_R',
                                    ASSE=_F(CHAM_GD=__CHWELASNOEU,
                                            TOUT='OUI',
                                            NOM_CMP=('X1','X2'),
                                            NOM_CMP_RESU=('DX','DY'),),)

                __RWELASNOEU = CREA_RESU(OPERATION='AFFE',
                                    NOM_CHAM='DEPL',
                                    TYPE_RESU='EVOL_ELAS',
                ...
                DETRUIRE(CONCEPT=(_F(NOM=__CHWELASNOEU),
                ...
                if MODELISATION != 'AXIS':
                    ...
                    __CHJ05 = CREA_CHAMP(OPERATION='ASSE', TYPE_CHAM='ELGA_NEUT_R',
                                MODELE=MODE, PROL_ZERO='OUI',
                                ASSE=(
                                    _F(TOUT='OUI', CHAM_GD=__GRWELAS,
                                        NOM_CMP=('EPXX',), NOM_CMP_RESU=('X1',),),
                                    _F(TOUT='OUI', CHAM_GD=__GRWELAS,
                                        NOM_CMP=('EPXY',), NOM_CMP_RESU=('X2',),),
                                    _F(TOUT='OUI', CHAM_GD=__QX_NOEU_GAUSS,
                                        NOM_CMP=('EPXX',), NOM_CMP_RESU=('X3',),),
                                    _F(TOUT='OUI', CHAM_GD=__QY_NOEU_GAUSS,
                                        NOM_CMP=('EPYY',), NOM_CMP_RESU=('X4',),),
                                    ),)

                    __FMULTJ05 = FORMULE(NOM_PARA=('X1','X2','X3','X4'),
                                        VALE='(X1*X3)+(2*X2*X4)')
            ...
            if MODELISATION == 'AXIS':
                ...
                __DEPINT_NEUT = CREA_CHAMP(OPERATION='ASSE',
                                    MODELE=MODE,
                                    TYPE_CHAM ='NOEU_NEUT_R',
                                    ASSE=_F(CHAM_GD=__DEPINT,
                                            TOUT='OUI',
                                            NOM_CMP=('DX',),
                                            NOM_CMP_RESU=('X1',),),)

                __DEPINT_ELGA = CREA_CHAMP(TYPE_CHAM='ELGA_NEUT_R',
                                    OPERATION='DISC',
                                    MODELE=MODE,
                                    PROL_ZERO='OUI',
                                    CHAM_GD=__DEPINT_NEUT)

                __CHJ06 = CREA_CHAMP(OPERATION='ASSE', TYPE_CHAM='ELGA_NEUT_R',
                            MODELE=MODE, PROL_ZERO='OUI',
                            ASSE=(
                                _F(TOUT='OUI', CHAM_GD=__SIGF,
                                    NOM_CMP=('SIZZ',), NOM_CMP_RESU=('X1',),),
                                _F(TOUT='OUI', CHAM_GD=__DEPINT_ELGA,
                                    NOM_CMP=('X1',), NOM_CMP_RESU=('X2',),),
                                _F(TOUT='OUI', CHAM_GD=__CHCOOR_ELGA,
                                    NOM_CMP=('X',), NOM_CMP_RESU=('X3',),),
                                _F(TOUT='OUI', CHAM_GD=__QX_NOEU_GAUSS,
                                    NOM_CMP=('EPXX',), NOM_CMP_RESU=('X4',),),
                                ),)

                __FMULTJ06 = FORMULE(NOM_PARA=('X1','X2','X3','X4'),
                                    VALE='X1*(X2/(X3+1.E-18))*X4')

                __CHFMUJ06 = CREA_CHAMP(OPERATION='AFFE', TYPE_CHAM='ELGA_NEUT_F',
                            MODELE=MODE, PROL_ZERO='OUI',
                ...
                __J06 = POST_ELEM(RESULTAT=__RESUJ06, MODELE=MODE, INST=inst,
                            INTEGRALE=_F(NOM_CHAM='VARI_ELGA', GROUP_MA='MMAIL',
                                        NOM_CMP='X5', TYPE_MAILLE='2D'),)

                DETRUIRE(CONCEPT=(_F(NOM=__DEPINT_NEUT),
                                  _F(NOM=__CHJ06),
                                  _F(NOM=__FMULTJ06),
                                  _F(NOM=__CHFMUJ06),
                                  _F(NOM=__CHJ06INT),
                ...
                __CHFMUJ07 = CREA_CHAMP(OPERATION='AFFE', TYPE_CHAM='ELGA_NEUT_F',
                            MODELE=MODE, PROL_ZERO='OUI',
                            AFFE=_F(TOUT='OUI', NOM_CMP='X3', VALE_F=__FMULTJ07),)

                __CHJ07INT = CREA_CHAMP(OPERATION='EVAL', TYPE_CHAM='ELGA_NEUT_R',
                            CHAM_F=__CHFMUJ07, CHAM_PARA=(__CHJ07,),)

                __RESUJ07 = CREA_RESU(OPERATION='AFFE', TYPE_RESU='EVOL_ELAS',
                            NOM_CHAM='VARI_ELGA', AFFE=_F(MODELE=MODE, INST=inst,
                                                        CHAM_GD=__CHJ07INT,),)

                __J07 = POST_ELEM(RESULTAT=__RESUJ07, MODELE=MODE, INST=inst,
                            INTEGRALE=_F(NOM_CHAM='VARI_ELGA', GROUP_MA='MMAIL',
                                        NOM_CMP='X3', TYPE_MAILLE='2D'),)

                DETRUIRE(CONCEPT=(_F(NOM=__CHJ07),
                                  _F(NOM=__FMULTJ07),
                                  _F(NOM=__CHFMUJ07),
                                  _F(NOM=__CHJ07INT),
                                  _F(NOM=__RESUJ07),
    ...
    if ndim == 3:   
        ...
        #
            TLIPSUP, TLIPINF = no_lips(self, NP, FOND_FISS, NB_COUCHES,
                                             symeType, closedCrack)
        #   --------------------------------------------------------------------------
        ...
        #
            ...
            for iord, inst in enumerate(lInst):
                ...
                __GDEPY = grad_u(self, MAIL, MODE, MATE,
                                       ['0.', 'DY', '0.'], __DEPINT, inst)
                __GDEPZ = grad_u(self, MAIL, MODE, MATE,
                                       ['0.', '0.', 'DZ'], __DEPINT, inst)

                if ETAT_INIT is None:

                    __WELAS = get_strain_energy(self, __RESU, inst)

                    __WELAS = CREA_CHAMP(OPERATION='ASSE',
                                         TYPE_CHAM='ELGA_NEUT_R',
                                         MODELE=MODE, PROL_ZERO='OUI',
                    ...
                    __EPSI_ELGA = CREA_CHAMP(TYPE_CHAM='ELGA_EPSI_R',
                                             OPERATION='ASSE',
                                             MODELE=MODE,
                                             ASSE=(_F(CHAM_GD=__EPSI_TOTA, TOUT='OUI',
                                                      CUMUL='OUI', COEF_R= 1.),
        ...
        #           J03: GRAD EPS0
                    ...
                    if grad_elno_type_j03 == 'OUI':
                        ...
                        __EPS0_ELNO = CREA_CHAMP(TYPE_CHAM='ELNO_EPSI_R',
                                                 OPERATION='DISC',
                                                 MODELE=MODE,
                                                 CHAM_GD=__EPS0_ELGA)
                        ...
                        __EPS0_NOEU = CREA_CHAMP(TYPE_CHAM='NOEU_EPSI_R',
                                                 OPERATION='DISC',
                                                 MODELE=MODE,
                                                 CHAM_GD=__EPS0_ELGA)
                    ...
                    for iEPS_CMP in EPS_CMP:
                        ...
                        if grad_elno_type_j03 == 'OUI':
                            ...
                            __GRAD_EPS0 = grad_elno(self, MAIL, MODE, MATE, listElemTMAIL,
        ...
        #           J04: GRAD EPSI
                    ...
                    if grad_elno_type_j04 == 'OUI':
                        ...
                        __EPSI_ELNO = CREA_CHAMP(TYPE_CHAM='ELNO_EPSI_R',
                                                 OPERATION='DISC',
                                                 MODELE=MODE,
                                                 CHAM_GD=__EPSI_ELGA)
                        ...
                        __EPSI_NOEU = CREA_CHAMP(TYPE_CHAM='NOEU_EPSI_R',
                                                 OPERATION='DISC',
                                                 MODELE=MODE,
                                                 CHAM_GD=__EPSI_ELGA)
                    ...
                    for iEPS_CMP in EPS_CMP:
                        ...
                        if grad_elno_type_j04 == 'OUI':
                            ...
                            __GRAD_EPSI = grad_elno(self, MAIL, MODE, MATE, listElemTMAIL,
        ...
        #           J05: GRAD W
                    ...
                    __WELASV = CREA_CHAMP(OPERATION='ASSE',
                    ...
                    if grad_elno_type_j05 == 'OUI':
                        ...
                        __WELAS_NOEU = CREA_CHAMP(TYPE_CHAM='NOEU_VAR2_R',
                                                  OPERATION='DISC',
                                                  MODELE=MODE,
                                                  CHAM_GD=__WELASV)
                        ...
                        __GRAD_WELAS = grad_noeu(self, MAIL, MODE, MATE,
                                                     __WELAS_CAL, inst)
                ...
                for iNP in listNP:
                    ...
                    if iNP < NP:
                        ...
                        __GQY = grad_q(self, MAIL, MODE, MATE, 'TMBOUGER'+str(iNP+1),
                                         'DY', TQ[iNP+1][1], inst)
                        __GQZ = grad_q(self, MAIL, MODE, MATE, 'TMBOUGER'+str(iNP+1),
                                         'DZ', TQ[iNP+1][2], inst)

                    else:

                        __GQX = grad_q_glob(self, MAIL, MODE, MATE, NP, 'DX', TQGLOB,
                                                0, inst)
                        __GQY = grad_q_glob(self, MAIL, MODE, MATE, NP, 'DY', TQGLOB,
                                                1, inst)
                        __GQZ = grad_q_glob(self, MAIL, MODE, MATE, NP, 'DZ', TQGLOB,
                                                2, inst)

        #           ------------------------------------------------------------------
        #           J01 = W * DIV Q
        #
                    __J01_J, volume = cal_j01(self, MODE, 'TMAIL', inst,
                                                  __WELAS, __GQX, __GQY, __GQZ)
        ...
        #
                    __J02_J = cal_j02(self, MODE, 'TMAIL', inst, __SIGF, __GDEPX,
                                          __GDEPY, __GDEPZ, __GQX, __GQY, __GQZ)
        ...
        #
                    if OPTION == 'JMOD' or ETAT_INIT is not None:
                        ...
                        if iNP < NP:
                            __QX, __QY, __QZ = field_q_local(self, MAIL,
                                                       'TMBOUGER'+str(iNP+1),
                                                       ['DX', 'DY', 'DZ'], TQ[iNP+1])
                        else:
                            __QX, __QY, __QZ = field_q_global(self, MAIL, NP,
                                                          ['DX', 'DY', 'DZ'], TQGLOB)
        ...
        #
                    ...
                    tab_result = get_result(self, __J, volume, NB_COUCHES, TITRE,
                                                TPFISS, FOND_FISS, iNP, NP, listNP,
                                                inst, iord, tab_result, nume)
