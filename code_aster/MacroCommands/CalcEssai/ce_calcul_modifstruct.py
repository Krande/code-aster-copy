...
class CalcEssaiModifStruct:
    ...
    def get_modele_support(self):
        ...
        for name, resu in list(self.objects.resultats.items()):
            ...
            self.mess.disp_mess("Impossible de trouver le mode meca calcule " "du modele support")
    ...
    def calc_base_es(self, opt_grnocapt=None):
        ...
        _MODST = MODE_STATIQUE(MATR_RIGI=self.matr_rig, FORCE_NODALE=grnocapt)
    ...
    def calc_base_lmme(self, base_mod_es, calc_freq=None):
        ...
        __NUMGEN = NUME_DDL_GENE(BASE=base_mod_es.obj, STOCKAGE="PLEIN")

        __KPROJ = PROJ_MATR_BASE(
            BASE=base_mod_es.obj, NUME_DDL_GENE=__NUMGEN, MATR_ASSE=self.matr_rig
        )

        __MPROJ = PROJ_MATR_BASE(
            BASE=base_mod_es.obj, NUME_DDL_GENE=__NUMGEN, MATR_ASSE=self.matr_masse
        )
        ...
        try:
            __MODGEN = MODE_ITER_SIMULT(
                MATR_RIGI=__KPROJ,
                MATR_MASS=__MPROJ,
                SOLVEUR=_F(NPREC=nprec),
                VERI_MODE=_F(SEUIL=1.0e-05, STOP_ERREUR="OUI"),
        ...
        self.x_resgen = None
        __RESGEN = REST_GENE_PHYS(RESU_GENE=__MODGEN, TOUT_ORDRE="OUI", NOM_CHAM="DEPL")
    ...
    def condensation(self, resolution=None, nomcham="DEPL"):
        ...
        self.x_mide = None
        __MIDE = EXTR_MODE(FILTRE_MODE=_F(MODE=modmesu.obj, NUME_MODE=modes_mesure_retenus))
        self.x_mide = __MIDE
        ...
        if self.method_name == "LMME":
            _MEXP = EXTR_MODE(
                FILTRE_MODE=_F(MODE=self.base_expansion.obj, NUME_MODE=modes_expansion_retenus)
            )
        ...
        self.x_proj = None
        try:
            _PROJ = PROJ_MESU_MODAL(
                MODELE_CALCUL=_F(MODELE=modlsup.obj, BASE=self.x_mexp),
                MODELE_MESURE=_F(MODELE=modlexp.obj, MESURE=self.x_mide, NOM_CHAM=nomcham),
        ...
        __MAILCD = DEFI_MAILLAGE(
            DEFI_SUPER_MAILLE=_F(MACR_ELEM=self.x_ssexp, SUPER_MAILLE=self.sumail),
            DEFI_NOEUD=_F(TOUT="OUI", INDEX=(1, 0, 1, 8)),  # XXX
        )
    ...
    def modele_couple(self):
        ...
        try:
            ...
            self.mess.disp_mess("Une erreur est survenue lors " "de la creation du modele couple")
            self.mess.disp_mess(str(err))
            raise

        if not self.cpl.mat_rigi:
            self.mess.disp_mess("Le modele couple n'a pas de matrice " "de rigidite assemblee")
        if not self.cpl.mat_mass:
            self.mess.disp_mess("Le modele couple n'a pas de matrice " "de masse assemblee")
        if not self.cpl.mat_amor:
            self.mess.disp_mess("Le modele couple n'a pas de matrice " "d'amortissement assemblee")
        # ...
    ...
    def maillage_iface(self):
        ...
        __MAIL = CREA_MAILLAGE(
            MAILLAGE=self.support_maillage,
            CREA_POI1=_F(GROUP_NO=group_no_ext, NOM_GROUP_MA=self.group_ma_int),
        )

        __MAILM = ASSE_MAILLAGE(MAILLAGE_1=self.x_mailcond, MAILLAGE_2=__MAIL, OPERATION="SOUS_STR")

        self.i_modlint = None
        __MODL = AFFE_MODELE(
            MAILLAGE=__MAILM,
            AFFE=_F(GROUP_MA=self.group_ma_int, PHENOMENE="MECANIQUE", MODELISATION="DIS_T"),
            AFFE_SOUS_STRUC=_F(SUPER_MAILLE=self.sumail, PHENOMENE="MECANIQUE"),
        )
        ...
        __CARA = AFFE_CARA_ELEM(
            ...
            DISCRET=(
                _F(
                    GROUP_MA=self.group_ma_int,
                    REPERE="GLOBAL",
                    CARA="K_T_D_N",
                    VALE=(0.0, 0.0, 0.0),
                ),
                _F(GROUP_MA=self.group_ma_int, REPERE="GLOBAL", CARA="M_T_D_N", VALE=(0.0,)),
            ),
        )

        __KEL = CALC_MATR_ELEM(OPTION="RIGI_MECA", MODELE=self.i_modlint, CARA_ELEM=__CARA)

        __MEL = CALC_MATR_ELEM(OPTION="MASS_MECA", MODELE=self.i_modlint, CARA_ELEM=__CARA)
    ...
    def indicateur_choix_base_expansion(self):
        ...
        self.modstint
        __MODSTI = MODE_STATIQUE(MATR_RIGI=self.kassup, FORCE_NODALE=self._get_force_nodale())
        self.modstint = __MODSTI

        self.projmsint = None
        try:
            __PROJMS = PROJ_MESU_MODAL(
                MODELE_CALCUL=_F(MODELE=self.support_modele.obj, BASE=self.modstint),
                MODELE_MESURE=_F(
                    MODELE=self.resu_exp.modele.obj, MESURE=self.modes_retr.obj, NOM_CHAM="DEPL"
                ),
                RESOLUTION=self.param_condens,
            )
        except AsterError as err:
        ...
        self.i_deplpr = None
        __DEPLPR = REST_GENE_PHYS(RESU_GENE=self.projmsint, TOUT_ORDRE="OUI", NOM_CHAM="DEPL")
        ...
        if self.crit_method == "MAC":
            if self.mat_ponder:
                __MACINT = MAC_MODES(
                    BASE_1=self.i_deplint.obj,
                    BASE_2=self.i_deplxint.obj,
                    MATR_ASSE=self.mat_ponder,
                    INFO=2,
                )
            else:
                __MACINT = MAC_MODES(BASE_1=self.i_deplint.obj, BASE_2=self.i_deplxint.obj, INFO=2)
        else:  # IERI
            if self.crit_ponder == "SANS" or self.crit_ponder == "Choisir":
                self.mess.disp_mess("Il faut une matrice de ponderation " "pour le critere IERI.")
                self.mac_int = None
                self.mac_val = None
                return
            if self.mat_ponder:
    ...
    def modes_modele_couple(self, mode_simult, calc_freq):
        ...
        if mode_simult:
            ...
            try:
                _MODCPL = MODE_ITER_INV(MATR_RIGI=kcouple, MATR_MASS=mcouple, CALC_FREQ=calc_freq)
            except AsterError as err:
                message = "ERREUR ASTER : " + err.message
                self.mess.disp_mess(message)
    ...
    def _can_get_nume_support_model(self):
        ...
        if not self.nume_support_modele:
            numesup = ModeMeca(
                None, self.support_modele_res.nom, self.support_modele_res, self.mess
            ).nume
            if not numesup:
                self.mess.disp_mess(
                    "Impossible de retrouver la numerotation " "utilisee pour le modele support"
                )
                return False
            self.nume_support_modele = numesup
        return True
    ...
    def _get_force_nodale(self):
        ...
        for grp in self.sensor_groups:
            force_nodale.append(_F(GROUP_NO=grp["NOM"], AVEC_CMP=grp["NOM_CMP"]))
        return force_nodale
...
class CopyModelMeca:
    ...
    def copy(self):
        ...
        # par nume_ddl
        nume_args = self.retrieve_nume_ddl(self.orig_modl.nom, list(self.matr_elem.keys()))
    ...
    def create_modele(self, affe):
        ...
        _MDLCPL = AFFE_MODELE(MAILLAGE=self.maillage, AFFE=affe)
        self.modele = _MDLCPL
    ...
    def affe_materiau(self, mater):
        ...
        if not mater:
            raise RuntimeError(
                "MODELE est un attribut facultatif de AFFE_MATERIAU \n"
...
class CreateModeleCouple(CopyModelMeca):
    ...
    def create_maillage(self):
        ...
        _MLCPL = ASSE_MAILLAGE(MAILLAGE_1=self.mail1, MAILLAGE_2=self.mail2, OPERATION="SOUS_STR")
    ...
    def nume_ddl(self, args_lst):
        if len(args_lst) != 1:
            raise RuntimeError(
                "Plusieurs, ou aucun, NUME_DDL presents, " "on ne peut en avoir qu'un"
            )
        args, sd = args_lst[0]
        self.nume_lst = []
        args = convert_args(args, self.concepts)
