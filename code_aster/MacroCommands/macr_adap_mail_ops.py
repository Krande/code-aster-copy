    ...
    *DataStructure* objects, only *list* and *dict*.
    ...
    def load_runs(self):
        ...
        for obj in context["objects"].values():
            ...
            logger.info("%-24s %s", obj.getName(), obj)
    ...
    def update_run_params(
        ...
        Nom_Co_Mail_NP1,
        maillage_np1,
        maillage_np1_nom_med,
        Nom_Co_Mail_NP1_ANNEXE,
        Rep_Calc_HOMARD_local,
        ...
        if niter == 0:
            ...
            #        que l'utilisateur ne s'est pas trompe dans l'enchainement
            for dico in self._run_params:
                valk = (Nom_Co_Mail_N, Nom_Co_Mail_NP1, dico["Maillage_NP1"], dico["Maillage_0"])
                UTMESS("A", "HOMARD0_9", valk=valk, vali=dico["niter"] + 1)
...
def cree_configuration(
    ...
    if INFO >= 3:
        if "Champs_supplementaires" in dico_configuration:
            print(
                "dico_configuration[Champs_supplementaires] =",
                dico_configuration["Champs_supplementaires"],
            )
    #
...
def macr_adap_mail_ops(self, INFO, VERSION_HOMARD, LOGICIEL=None, MAILLAGE_FRONTIERE=None, **args):
    """
       Traitement des macros MACR_ADAP_MAIL/MACR_INFO_MAIL
    ...
    if mode_homard != "LECT":
        ...
        if niter == 0:
            ...
            if os.path.isdir(Rep_Calc_HOMARD_global):
                UTMESS("A", "HOMARD0_11", valk=(Nom_Co_Mail_N, Rep_Calc_HOMARD_global))
    ...
    if mode_homard != "LECT":
        ...
        # print ".. Debut de 4.1."
        DEFI_FICHIER(
            ACTION="ASSOCIER", UNITE=unite_fichier_aster_vers_homard, TYPE="LIBRE", INFO=infocomm
        )
        #
        # 4.2. Le(s) maillage(s)
    ...
    if mode_homard != "LECT":
        ...
        EXEC_LOGICIEL(
            ARGUMENT=(
                Rep_Calc_HOMARD_global,  # nom du répertoire
                ...
                if mode_homard in ["ADAP", "MODI"]:
                    ...
                    if type_cham[0:2] == "EL":
                        ...
                        le_champ = LIRE_CHAMP(
                            ...
                            NOM_MAIL_MED=maillage_np1_nom_med,
                            NOM_MED=dico["NOM_CHAM_MED"],
                            TYPE_CHAM=type_cham,
                            INFO=infocomm,
                            **motscsi,
                        )
                ...
                #
                    ...
                    if type_cham[0:2] == "EL":
                        le_champ = LIRE_CHAMP(
                            ...
                            FORMAT="MED",
                            MAILLAGE=maillage_n,
                            NOM_MAIL_MED=maillage_n_nom_med,
    ...
    #
    if mode_homard != "LECT":
        DEFI_FICHIER(ACTION="LIBERER", UNITE=unite_fichier_aster_vers_homard, INFO=infocomm)
        # print "Repertoire ", Rep_Calc_HOMARD_global
        # print os.listdir(Rep_Calc_HOMARD_global)
