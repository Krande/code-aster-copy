...
from ..Commands import CREA_TABLE, IMPR_TABLE, STAT_NON_LINE, MECA_STATIQUE, CALC_CHAMP, IMPR_RESU
from ..Commands import CREA_CHAMP, POST_RELEVE_T, CALC_TABLE, COMB_SISM_MODAL, DEFI_LIST_REEL
...
def combinaison_charge_ops(self, MODELE_MECA, BLOC, EXCIT_MECA, CARA_ELEM_MECA, **args):
    ...
    if COMPORTEMENT is not None :
        for ii in range(len(COMPORTEMENT)): comport.append(COMPORTEMENT[ii]['RELATION'])
            ...
            for ii in range(len(COMPORTEMENT)):
                if COMPORTEMENT[ii]['RELATION'] in ['CABLE', 'ELAS_MEMBRANE_SV']:
                    if COMPORTEMENT[ii]['GROUP_MA'] is not None :
                        ...
                        comport['COMPORTEMENT'].append(_F(TOUT = 'OUI', RELATION = COMPORTEMENT[ii]['RELATION'], DEFORMATION = 'GROT_GDEP'))
                else :
                    if COMPORTEMENT[ii]['GROUP_MA'] is not None :
    ...
    dict_list_sortie = {"MAXI" : "MAX", "MINI" : "MIN", "MAXI_ABS" : "MAXI_ABS", "MINI_ABS" : "MINI_ABS"}
    imprresu ={}
    imprresu['RESU']=[]
    comblist = []
    extrlist = []
    ...
    for kk in range(len(list_cham)) :
        ...
        for ii in range(nb_char_meca):
            ...
            nom_champ[champ][ii]= CREA_CHAMP ( OPERATION ='EXTR', TYPE_CHAM=type_champ[champ],
                                               RESULTAT = calc[ii], NUME_ORDRE=1,
                                               NOM_CHAM = champ,);
        ...
        for ii in range(nb_char_ther):
            # Création des champs de résultats 
            nom_champ[champ][nb_char_meca + 2 * ii]= CREA_CHAMP ( OPERATION ='EXTR', 
                                    TYPE_CHAM=type_champ[champ],
                                    RESULTAT = calc[nb_char_meca + ii], 
                                    TOUT_ORDRE= 'OUI',
                                    TYPE_MAXI = 'MAXI', 
                                    NOM_CHAM = champ,);
            nom_champ[champ][nb_char_meca + 2 * ii + 1]= CREA_CHAMP ( OPERATION ='EXTR', 
                                    TYPE_CHAM=type_champ[champ],
                                    RESULTAT = calc[nb_char_meca + ii], 
                                    TOUT_ORDRE= 'OUI',
                                    TYPE_MAXI = 'MINI', 
        ...
        for ii in range(nb_char, nb_tot):
            ...
            for jj in range(nb_char):
                combinaison['ASSE'].append(_F( CUMUL='OUI', TOUT='OUI', CHAM_GD= nom_champ[champ][jj], COEF_R= col[jj+1][ii],),)
                
            
            nom_champ[champ][ii] = CREA_CHAMP (MODELE=MODELE_MECA, OPERATION = 'ASSE', 
                                    TYPE_CHAM=type_champ[champ],**combinaison);
        ...
        for comp in range(nb_comp):
            ...
            for ii in range(nb_tot):
                if 'TOUT' in list_cmp[kk] :
                    actcmp['ACTION'].append(_F(CHAM_GD=nom_champ[champ][ii], INTITULE=numcomb[ii], OPERATION='EXTREMA'),)
                else :
                    actcmp['ACTION'].append(_F(CHAM_GD=nom_champ[champ][ii], INTITULE=numcomb[ii], NOM_CMP=list_cmp[kk][comp], OPERATION='EXTREMA'),)
                
            nom_table[champ][comp]=POST_RELEVE_T(**actcmp)
            nom_table[champ][comp]=CALC_TABLE(reuse=nom_table[champ][comp], TABLE=nom_table[champ][comp], 
            ...
            for extr in range(len(list_sortie)):
                tab[comp][extr] = CALC_TABLE (TABLE = nom_table[champ][comp],
                                              ACTION = (_F(OPERATION = 'FILTRE',
                                                               NOM_PARA = 'EXTREMA',VALE_K=dict_list_sortie[list_sortie[extr]]),
                                                            _F(OPERATION = 'FILTRE',
                                                               NOM_PARA = 'VALE', CRIT_COMP = list_sortie[extr],),
                                                            _F(OPERATION = 'AJOUT_COLONNE',
                                                               NOM_PARA = 'TYPE_VALE',
                                                               VALE_COLONNE = list_sortie[extr],),
                                                            _F(OPERATION = 'FILTRE',
                                                               NOM_PARA = 'TYPE_VALE', CRIT_COMP = 'NON_VIDE',),),)
                                                             
                if extr == 0:
                    res[comp] = tab[comp][extr]
                else:
                    res[comp] = CALC_TABLE (reuse = res[comp],
                                            TABLE = res[comp],
                                            ACTION = _F(OPERATION = 'COMB',
                                                         TABLE = tab[comp][extr],
                                                         RESTREINT='NON'),)
            
            res[comp]=CALC_TABLE(reuse=res[comp], TABLE=res[comp], 
                                 ACTION = _F( OPERATION='SUPPRIME', NOM_PARA='CHAM_GD',))
    ...
    tabcalc=CREA_TABLE(TYPE_TABLE = 'TABLE_CONTAINER', **motcles)
    self.register_result(tabcalc, nomco2)

    if num_unite2 > 0 :
        IMPR_TABLE( TABLE = tabcalc, FORMAT = 'TABLEAU', UNITE = num_unite2,
                    NOM_PARA=('INTITULE', 'EXTREMA', 'NOEUD', 'CMP', 'VALE'))

    table3 = Table(extrlist, extrpara, extrtype)
    motcles=table3.dict_CREA_TABLE()
    tabextr=CREA_TABLE(TYPE_TABLE = 'TABLE_CONTAINER', **motcles)
    self.register_result(tabextr, nomco3)

    if num_unite3 > 0 :
        IMPR_TABLE( TABLE = tabextr, FORMAT = 'TABLEAU', UNITE = num_unite3,
