...
def post_liquefaction_ops(self, AXE, RESULTAT, CRITERE, **args):
    ...
    if CRITERE != "P_SIGM":
        ...
        __fmul = FORMULE(
            NOM_PARA=("X1", "X2", "X3", "X5", "X6"), VALE="fmul0(X1,X2,X3,X5,X6)", fmul0=fmul
        )
        __chfmu = CREA_CHAMP(
            OPERATION="AFFE",
    ...
    if CRITERE != "P_SIGM":
        for i, ordre in enumerate(__numo):
            ...
            if AXE == "X":
                __sig2 = CREA_CHAMP(
                    ...
                    MODELE=__model,
                    PROL_ZERO="OUI",
                    ASSE=_F(TOUT="OUI", CHAM_GD=__sigt, NOM_CMP=("SIPXX",), NOM_CMP_RESU=("X3",)),
                )
            elif AXE == "Y":
                __sig2 = CREA_CHAMP(
                    OPERATION="ASSE",
                    ...
                    MODELE=__model,
                    PROL_ZERO="OUI",
                    ASSE=_F(TOUT="OUI", CHAM_GD=__sigt, NOM_CMP=("SIPYY",), NOM_CMP_RESU=("X3",)),
                )
            elif AXE == "Z":
                __sig2 = CREA_CHAMP(
                    OPERATION="ASSE",
                    ...
                    MODELE=__model,
                    PROL_ZERO="OUI",
                    ASSE=_F(TOUT="OUI", CHAM_GD=__sigt, NOM_CMP=("SIPZZ",), NOM_CMP_RESU=("X3",)),
                )
            ...
            __sig = CREA_CHAMP(
                ...
                TYPE_CHAM="ELGA_NEUT_R",
                ASSE=(
                    _F(CHAM_GD=__sig1, TOUT="OUI", CUMUL="OUI", COEF_R=1.0),
                    _F(CHAM_GD=__sig2, TOUT="OUI", CUMUL="OUI", COEF_R=1.0),
                ),
            )

            ### Calcul du critere de liquefaction
            __liqq[i] = CREA_CHAMP(
                OPERATION="EVAL", TYPE_CHAM="ELGA_NEUT_R", CHAM_F=__chfmu, CHAM_PARA=(__sig,)
            )
            ...
            __sigf[i] = CREA_CHAMP(
                ...
                MODELE=__model,
                PROL_ZERO="OUI",
                ASSE=_F(TOUT="OUI", CHAM_GD=__liqq[i], NOM_CMP=("X4",), NOM_CMP_RESU=("X4",)),
            )
        ...
        __fmul = FORMULE(NOM_PARA=("X2", "X3", "X5", "X6"), VALE="fmul0(X2,X3,X5,X6)", fmul0=fmul0)
    ...
    for i, ordre in enumerate(__numo):
        ...
        if AXE == "X":
            __siff[i] = CREA_CHAMP(
                ...
                PROL_ZERO="OUI",
                MODELE=__model,
                ASSE=_F(TOUT="OUI", CHAM_GD=__sigf[i], NOM_CMP=("X4",), NOM_CMP_RESU=("SIPXX",)),
            )
        if AXE == "Y":
            __siff[i] = CREA_CHAMP(
                OPERATION="ASSE",
                ...
                PROL_ZERO="OUI",
                MODELE=__model,
                ASSE=_F(TOUT="OUI", CHAM_GD=__sigf[i], NOM_CMP=("X4",), NOM_CMP_RESU=("SIPYY",)),
            )
        if AXE == "Z":
            __siff[i] = CREA_CHAMP(
                OPERATION="ASSE",
                ...
                PROL_ZERO="OUI",
                MODELE=__model,
                ASSE=_F(TOUT="OUI", CHAM_GD=__sigf[i], NOM_CMP=("X4",), NOM_CMP_RESU=("SIPZZ",)),
            )
