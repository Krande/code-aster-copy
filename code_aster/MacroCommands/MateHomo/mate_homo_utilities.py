...
def parse_mater_groups(ls_affe, varc_name, ls_group_tout):
    ...
    * Transformer les affectations TOUT=OUI en affectations sur GROUP_MA=BODY
    ...
    mat1, mat2, mat3 = 'ELAS_FO', 'THER_FO', 'THER_NL'
    ...
    for item in ls_affe :
        ...
        l_elas_fo = [i for i in mater.getVectorOfMaterialProperties() if i.getName() == mat1]
        l_ther_fo = [i for i in mater.getVectorOfMaterialProperties() if i.getName() == mat2]
        l_ther_nl = [i for i in mater.getVectorOfMaterialProperties() if i.getName() == mat3]

        if len(l_elas_fo) == 0:
            UTMESS('F', 'HOMO1_8', valk=(mat1, mater.getName(), mater.userName))
        else:
            mate_elas = l_elas_fo[0]

        if (len(l_ther_fo) == 0 and len(l_ther_nl) == 0) :
            UTMESS('F', 'HOMO1_9', valk=(mat2, mat3, mater.getName(), mater.userName))
        else:
            mate_ther = l_ther_fo[0] if (len(l_ther_nl) == 0) else l_ther_nl[0]
        ...
        for p in (*mandatory_elas, *optional_elas):
            pname = p.capitalize()
            if mate_elas.hasValueGenericFunction(pname):
                if p in missing_in_at_least_one:
                    UTMESS('F', 'HOMO1_12', valk=p)
                f_para[p] = mate_elas.getValueGenericFunction(pname)
            else:
                missing_in_at_least_one.append(p)

        for p in (*mandatory_ther, *optional_ther):
            pname = p.capitalize()
            if mate_ther.hasValueGenericFunction(pname):
                if p in missing_in_at_least_one:
                    UTMESS('F', 'HOMO1_12', valk=p)
                f_para[p] = mate_ther.getValueGenericFunction(pname)
            else:
                missing_in_at_least_one.append(p)
        ...
        elas_fo_kw = {'E' : f_para_temp['E'],
                      'NU' : f_para_temp['NU'],
                      'ALPHA' : f_zero,
                      'TEMP_DEF_ALPHA' : 20.0}

        ther_fo_kw = {'LAMBDA' : f_para_temp['LAMBDA'],
                      'RHO_CP' : f_zero}

        new_item_calc['MATER'] = DEFI_MATERIAU(ELAS_FO=_F(**elas_fo_kw),
                                               THER_FO=_F(**ther_fo_kw))
        ...
        new_item_mate['MATER'] = DEFI_MATERIAU(ELAS_FO=_F(**elas_fo_kw),
                                               THER_FO=_F(**ther_fo_kw))
...
def prepare_alpha_loads(ls_affe_mod_mate, varc_values):
    ...
    for item in ls_affe_mod_mate:
        ...
        mater = item['MATER']
        mate_elas = [i for i in mater.getVectorOfMaterialProperties() if i.getName() == 'ELAS_FO'][0]
        f_alpha_temp =  mate_elas.getValueGenericFunction("ALPHA".capitalize())
        ...
        ls_alpha_calc.append({'GROUP_MA' : group_ma_affe,
                              'FONC_ALPHA_TIME' : f_alpha_time})
...
def setup_calcul(mesh, ls_group_tout, ls_affe, varc_name, varc_values):
    ...
    MODTH = AFFE_MODELE(MAILLAGE=mesh,
                        AFFE=_F(GROUP_MA=ls_group_tout,
                                MODELISATION='3D',
                                PHENOMENE='THERMIQUE'))

    MODME = AFFE_MODELE(MAILLAGE=mesh,
                        AFFE=_F(GROUP_MA=ls_group_tout,
                                MODELISATION='3D',
                                PHENOMENE='MECANIQUE'))

    EVOLVARC = CREA_RESU(OPERATION='AFFE',
                         TYPE_RESU='EVOL_VARC',
                         NOM_CHAM='TEMP',
                         AFFE=[_F(CHAM_GD = CREA_CHAMP(TYPE_CHAM = 'NOEU_TEMP_R',
                                                       OPERATION = 'AFFE',
                                                       MAILLAGE = mesh,
                                                       AFFE = _F(GROUP_MA=ls_group_tout,
                                                                 NOM_CMP = 'TEMP',
                                                                 VALE = value)),
                                  INST = inst) for inst, value in enumerate(varc_values)])

    affevarckw = {'GROUP_MA' : ls_group_tout,
                  'NOM_VARC' : 'TEMP',
                  'EVOL' : EVOLVARC,
                  'NOM_CHAM' : 'TEMP',
                  'VALE_REF' : 20.0}

    CHLOIME = AFFE_MATERIAU(MODELE=MODME,
                            AFFE= [_F(**affekw) for affekw in ls_affe_mod_mate],
                            AFFE_VARC = _F(**affevarckw))

    CHMATME = AFFE_MATERIAU(MODELE=MODME,
                            AFFE= [_F(**affekw) for affekw in ls_affe_mod_calc],
                            AFFE_VARC = _F(**affevarckw))
    ...
    LOCK_MECA = AFFE_CHAR_CINE(MODELE=MODME,
                               MECA_IMPO=(
                                   _F(GROUP_MA=ls_group_tout,
                                      DX=0.0,
                                      DY=0.0,
                                      DZ=0.0)))
...
def combine_enerpot(RESU1, RESU2, N_ORDRE, ls_group_tout):
    ...
    CH1 = CREA_CHAMP(RESULTAT=RESU1,
                     TYPE_CHAM='NOEU_%s_R'%CH_TYPE,
                     OPERATION='EXTR',
                     NOM_CHAM=CH_TYPE,
                     NUME_ORDRE=N_ORDRE)

    EPOT_CH1 = POST_ELEM(CHAM_GD=CH1,
                         MODELE=MOD,
                         CHAM_MATER=CHMAT,
    ...
    if RESU1 is RESU2:
        ...
        CH2 = CREA_CHAMP(RESULTAT=RESU2,
                         TYPE_CHAM='NOEU_%s_R'%CH_TYPE,
                         OPERATION='EXTR',
                         NOM_CHAM=CH_TYPE,
                         NUME_ORDRE=N_ORDRE)

        EPOT_CH2 = POST_ELEM(CHAM_GD=CH2,
                             MODELE=MOD,
                             CHAM_MATER=CHMAT,
                             ENER_POT=_F(GROUP_MA=ls_group_tout))
        epot_ch2 = abs(sum(EPOT_CH2.EXTR_TABLE().values()['TOTALE']))

        EPOT_SOMME = POST_ELEM(CHAM_GD=CH1+CH2,
                               MODELE=MOD,
                               CHAM_MATER=CHMAT,
                               ENER_POT=_F(GROUP_MA=ls_group_tout))
