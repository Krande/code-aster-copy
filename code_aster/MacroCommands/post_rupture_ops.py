...
def sittmax(k1, k2):
    ...
    if k2 == 0.0:
        ...
        return 2.0 * NP.arctan(((k1 / k2) - NP.sign(k2) * NP.sqrt((k1 / k2) ** 2 + 8)) / 4.0)
...
def amestoy(k1, k2, crit_ang):
    ...
    K11 = [1.0, 0.9886, 0.9552, 0.9018, 0.8314, 0.7479, 0.6559, 0.5598, 0.4640, 0.3722]
    K21 = [0.0, 0.0864, 0.1680, 0.2403, 0.2995, 0.3431, 0.3696, 0.3788, 0.3718, 0.3507]
    K12 = [0.0, -0.2597, -0.5068, -0.7298, -0.9189, -1.0665, -1.1681, -1.2220, -1.2293, -1.1936]
    K22 = [1.0, 0.9764, 0.9071, 0.7972, 0.6540, 0.4872, 0.3077, 0.1266, -0.0453, -0.1988]
...
def caract_mater(self, mater):
    ...
    key = matElas[0]
    young = mater.getFunction(key, "E")
    try:
        temp_eval = mater.getValueReal(key, "TEMP_DEF_ALPHA")
        if young:
            UTMESS("A", "XFEM2_85", valr=temp_eval)
    except RuntimeError:
        temp_eval = None

    if young:
        poisson = mater.getFunction(key, "NU")
        type_young = young.getProperties()[0]
        type_nu = poisson.getProperties()[0]
        if temp_eval is None and (type_young != "CONSTANT" or type_nu != "CONSTANT"):
            UTMESS("F", "RUPTURE1_68")
        else:
            temp_eval = 0.0
        young = young(temp_eval)
        poisson = poisson(temp_eval)
    else:
        young = mater.getValueReal(key, "E")
        poisson = mater.getValueReal(key, "NU")
...
def post_rupture_ops(self, TABLE, OPERATION, **args):
    ...
    if OPERATION == "ABSC_CURV_NORM":
        ...
        if "NUME_FOND" in tabin.para:
            ...
            tabout = CALC_TABLE(
                TABLE=TABIN,
                reuse=TABIN,
                ACTION=_F(OPERATION="OPER", FORMULE=__formul, NOM_PARA=args["NOM_PARA"]),
            )
        else:
            # récupération de l'abscisse maximale et ajout dans l'environnement
            smax = max(tabin.ABSC_CURV.values())

            __NORM_ABS = FORMULE(NOM_PARA=("ABSC_CURV"), VALE="ABSC_CURV/smax", smax=smax)

            tabout = CALC_TABLE(
                TABLE=TABIN,
                reuse=TABIN,
                ACTION=_F(OPERATION="OPER", FORMULE=__NORM_ABS, NOM_PARA=args["NOM_PARA"]),
            )
    ...
    if OPERATION == "ANGLE_BIFURCATION":
        ...
        if crit_ang in ("SITT_MAX", "K1_MAX", "K2_NUL"):
            ...
            if crit_ang == "SITT_MAX":
                ...
                # dans les commandes
                __angle_deg = FORMULE(
                    NOM_PARA=("K1", "K2"), VALE="sittmax(K1,K2)*180./pi", sittmax=sittmax
                )
            ...
            tabout = CALC_TABLE(
                TABLE=TABIN,
                reuse=TABIN,
                ACTION=_F(OPERATION="OPER", FORMULE=__angle_deg, NOM_PARA=args["NOM_PARA"]),
            )
            ...
            tabout = CALC_TABLE(
                TABLE=TABIN,
                reuse=TABIN,
                ACTION=_F(OPERATION="AJOUT_COLONNE", VALE=0.0, NOM_PARA=args["NOM_PARA"]),
            )
            ...
            __TAB = CALC_TABLE(
                TABLE=TABIN, reuse=TABIN, ACTION=_F(OPERATION="AJOUT_COLONNE", VALE=p, NOM_PARA="P")
            )

            #        On calcule des membres intermédiaires
            #          membre 'MODE3'
            __puissance = FORMULE(
                NOM_PARA=("K1", "K2", "K3", "P"), VALE="puissance(K1,K2,K3,P)", puissance=puissance
            )
            __TAB = CALC_TABLE(
                TABLE=__TAB,
                reuse=__TAB,
                ACTION=_F(OPERATION="OPER", FORMULE=__puissance, NOM_PARA="MODE3"),
            )
            #          membre 'TERME_GE'
            __terme_general = FORMULE(
                NOM_PARA=("K1", "K2", "K3", "MODE3"),
            ...
            __TAB = CALC_TABLE(
                TABLE=__TAB,
                reuse=__TAB,
                ACTION=_F(OPERATION="OPER", FORMULE=__terme_general, NOM_PARA="TERME_GE"),
            )
            #          membre 'RACINE'
            __racine = FORMULE(
                NOM_PARA=("K2", "TERME_GE"), VALE="racine(K2,TERME_GE)", racine=racine
            )
            __TAB = CALC_TABLE(
                TABLE=__TAB,
                reuse=__TAB,
                ACTION=_F(OPERATION="OPER", FORMULE=__racine, NOM_PARA="RACINE"),
            )
            #             ANGLE_BETA
            __trigo = FORMULE(NOM_PARA=("RACINE"), VALE="trigo(RACINE)*180./pi", trigo=trigo)
            __TAB = CALC_TABLE(
                TABLE=__TAB,
                reuse=__TAB,
                ACTION=_F(OPERATION="OPER", FORMULE=__trigo, NOM_PARA="ANGLE_BETA"),
            )
            ...
            __TAB = CALC_TABLE(
                TABLE=__TAB,
                reuse=__TAB,
                ACTION=_F(OPERATION="AJOUT_COLONNE", VALE=poisson, NOM_PARA="POISSON"),
            )
            #             membre 'MEMBRE_1'
            __fmembre_1 = FORMULE(
                NOM_PARA=("K1", "ANGLE_BETA", "POISSON"),
            ...
            __TAB = CALC_TABLE(
                TABLE=__TAB,
                reuse=__TAB,
                ACTION=_F(OPERATION="OPER", FORMULE=__fmembre_1, NOM_PARA="MEMBRE_1"),
            )
            #          membre 'MEMBRE_2'
            __fmembre_2 = FORMULE(
                NOM_PARA=("K2", "ANGLE_BETA", "POISSON"),
            ...
            __TAB = CALC_TABLE(
                TABLE=__TAB,
                reuse=__TAB,
                ACTION=_F(OPERATION="OPER", FORMULE=__fmembre_2, NOM_PARA="MEMBRE_2"),
            )
            #          membre 'MEMBRE_3'
            __fmembre_3 = FORMULE(
                NOM_PARA=("K3", "ANGLE_BETA"), VALE="__membre_3(K3,ANGLE_BETA)", __membre_3=membre_3
            )
            __TAB = CALC_TABLE(
                TABLE=__TAB,
                reuse=__TAB,
                ACTION=_F(OPERATION="OPER", FORMULE=__fmembre_3, NOM_PARA="MEMBRE_3"),
            )
            #          ANGLE_GAMMA
            __fdivision = FORMULE(
                NOM_PARA=("MEMBRE_1", "MEMBRE_2", "MEMBRE_3"),
            ...
            __TAB = CALC_TABLE(
                TABLE=__TAB,
                reuse=__TAB,
                ACTION=_F(OPERATION="OPER", FORMULE=__fdivision, NOM_PARA="ANGLE_GAMMA"),
            )
    ...
    if OPERATION[-4:] == "K_EQ":
        ...
        if cumul == "CUMUL_G":
            ...
            __cumul = FORMULE(NOM_PARA="G", VALE="sqrt(G*E/(1-nu**2))", **const_context)
        ...
        tabout = CALC_TABLE(
            TABLE=TABIN,
            reuse=TABIN,
            ACTION=_F(OPERATION="OPER", FORMULE=__cumul, NOM_PARA=args["NOM_PARA"]),
        )
    ...
    if OPERATION == "COMPTAGE_CYCLES":
        ...
        if COMPTAGE == "UNITAIRE":
            ...
            for i, q in enumerate(list_q):
                ...
                __delta_unit = FORMULE(
                    NOM_PARA=q, VALE=q + "*(COEF_MULT_MAXI-COEF_MULT_MINI)", **const_context
                )
        ...
        # vrai comptage des cycles avec POST_FATIGUE
            ...
            __COPIE_TABIN = CALC_TABLE(
                TABLE=TABIN, ACTION=_F(OPERATION="EXTR", NOM_PARA=tabin.para)
            )

            if "NUME_FOND" not in tabin.para:
                __COPIE_TABIN = CALC_TABLE(
                    TABLE=__COPIE_TABIN,
                    ACTION=_F(OPERATION="AJOUT_COLONNE", VALE=1, NOM_PARA="NUME_FOND"),
                )
            if "NUM_PT" not in tabin.para:
                __COPIE_TABIN = CALC_TABLE(
                    TABLE=__COPIE_TABIN,
                    ACTION=_F(OPERATION="AJOUT_COLONNE", VALE=1, NOM_PARA="NUM_PT"),
                )
                ...
                for ipt in range(nbpt):
                    ...
                    for i, q in enumerate(list_q):
                        ...
                        __EVOLQ = RECU_FONCTION(
                            ...
                            PARA_Y=q,
                            FILTRE=(
                                _F(NOM_PARA="NUM_PT", CRIT_COMP="EQ", VALE_I=numpt),
                                _F(NOM_PARA="NUME_FOND", CRIT_COMP="EQ", VALE_I=fond_j),
                            ),
                        )

                        __TABC[i] = POST_FATIGUE(
                            CHARGEMENT="UNIAXIAL",
                        ...
                        __TABC[i] = CALC_TABLE(
                            ...
                            reuse=__TABC[i],
                            ACTION=(
                                _F(OPERATION="OPER", FORMULE=__delta, NOM_PARA="DELTA_" + q),
                    ...
                    if "NUM_PT" in tabin.para:
                        __TABC[0] = CALC_TABLE(
                            reuse=__TABC[0],
                            TABLE=__TABC[0],
                            ACTION=_F(OPERATION="AJOUT_COLONNE", VALE=numpt, NOM_PARA="NUM_PT"),
                        )
                    ...
                    if "NUME_FOND" in tabin.para:
                        __TABC[0] = CALC_TABLE(
                            reuse=__TABC[0],
                            TABLE=__TABC[0],
                            ACTION=_F(OPERATION="AJOUT_COLONNE", VALE=fond_j, NOM_PARA="NUME_FOND"),
                        )
                    ...
                    for para in l_para_aux:
                        ...
                        __TABC[0] = CALC_TABLE(
                            reuse=__TABC[0],
                            TABLE=__TABC[0],
                            ACTION=_F(OPERATION="AJOUT_COLONNE", VALE=vale, NOM_PARA=para),
                        )

                    # on concatene la table avec la table sortie
                    tabout = CALC_TABLE(
                        reuse=tabout, TABLE=tabout, ACTION=_F(TABLE=__TABC[0], OPERATION="COMB")
                    )
    ...
    if OPERATION == "LOI_PROPA":
        ...
        tabout = CALC_TABLE(
            TABLE=TABIN,
            reuse=TABIN,
            ACTION=_F(OPERATION="OPER", FORMULE=__da, NOM_PARA=args["NOM_PARA"]),
        )
        ...
        for i, fond_i in enumerate(nume_fond):
            ...
            for ipt in range(nbpt):
                ...
                tabout = CALC_TABLE(
                    reuse=tabout,
                    TABLE=tabout,
                    ACTION=_F(OPERATION="AJOUT_LIGNE", NOM_PARA=l_para, VALE=l_vale),
                )
    ...
    if OPERATION == "PILO_PROPA":
        ...
        for TABLE_i in TABLE:
            ...
            tabout = CALC_TABLE(
                reuse=tabout, TABLE=tabout, ACTION=_F(TABLE=__tabtmp, OPERATION="COMB")
            )
    ...
    if OPERATION == "K1_NEGATIF":
        ...
        if change == True and ("G" or "G_IRWIN" in tabin.para):
            ...
            __tabtmp = CALC_TABLE(
                TABLE=TABIN, ACTION=_F(OPERATION="FILTRE", NOM_PARA="K1", CRIT_COMP="GE", VALE=0.0)
            )
            ...
            tabout = CALC_TABLE(
                TABLE=TABIN,
                reuse=TABIN,
                ACTION=_F(OPERATION="FILTRE", NOM_PARA="K1", CRIT_COMP="LT", VALE=0.0),
            )
            ...
            tabout = CALC_TABLE(
                ...
                reuse=tabout,
                ACTION=(
                    _F(OPERATION="SUPPRIME", NOM_PARA="K1"),
                    _F(OPERATION="AJOUT_COLONNE", VALE=0.0, NOM_PARA="K1"),
                ),
            )
            ...
            for param_G in list_G:
                tabout = CALC_TABLE(
                    ...
                    reuse=tabout,
                    ACTION=(
                        _F(OPERATION="SUPPRIME", NOM_PARA=param_G),
                        _F(OPERATION="OPER", FORMULE=__formul_G, NOM_PARA=param_G),
                    ),
                )
            # association des tableaux 1 et 2
            tabout = CALC_TABLE(
                TABLE=tabout, reuse=tabout, ACTION=_F(TABLE=__tabtmp, OPERATION="COMB")
            )
            ...
            if "NUME_FOND" or "INST" or "NUM_PT" in tabin.para:
                ...
                tabout = CALC_TABLE(
                    TABLE=tabout,
                    reuse=tabout,
                    ACTION=_F(OPERATION="TRI", NOM_PARA=list_para, ORDRE="CROISSANT"),
                )
        ...
        # si on a trouve au moins une valeur negative de K1 et G non present
            ...
            tabout = CALC_TABLE(
                ...
                reuse=TABIN,
                ACTION=(
                    _F(OPERATION="OPER", FORMULE=__formul, NOM_PARA="K1_bis"),
                    _F(OPERATION="SUPPRIME", NOM_PARA="K1"),
                    _F(OPERATION="RENOMME", NOM_PARA=("K1_bis", "K1")),
                ),
            )
        else:
