...
def calc_comb_modes(__tbresul, myintitule_by_num, comb_modes, resu, b_extrac, resu_nume_ordre):
    ...
    __tbmodpar = RECU_TABLE(
        CO=__nresu, NOM_PARA=("FREQ", "FACT_PARTICI_DX", "FACT_PARTICI_DY", "FACT_PARTICI_DZ")
    )
    ...
    for comb_num, comb in enumerate(comb_modes, 1):
        ...
        for lign_num in myintitule_by_num.keys():
            l_ri_by_comp = (
                (pytbcoup.INTITULE == myintitule_by_num[lign_num]) & (pytbcoup.TYPE == "RESULTANTE")
            ).values()  # INTITULE must be unique and not empty here
            l_Ri_by_inte_by_space = {
                c: {inte_comp: [] for inte_comp in inte_by_efge.keys()} for c in space_dirs
            }
            l_fact_by_spacedir = {
                "X": resu_partab["FACT_PARTICI_DX"],
                "Y": resu_partab["FACT_PARTICI_DY"],
                "Z": resu_partab["FACT_PARTICI_DZ"],
            }
            for nume_ordre in resu_nume_ordre:
                ...
                for spacedir_index, space_dir in enumerate(space_dirs):
                    paras = ["INTITULE", "NOM_CHAM", "COMB", "TYPE", "NUME_ORDRE"]
                    vales = [
                        myintitule_by_num[lign_num],
                        "EFGE_NOEU",
                        comb_intitule,
                        "RI" + space_dir,
                        nume_ordre,
                    ...
                    for inte_comp in inte_by_efge.keys():
                        ...
                        vales.append(R_i)
                    __tbresul = CALC_TABLE(
                        reuse=__tbresul,
                        TABLE=__tbresul,
                        ACTION=_F(OPERATION="AJOUT_LIGNE", NOM_PARA=paras, VALE=vales),
                    )

            Rm_by_inte_by_spacedir = {
                space_dir: {inte_comp: 0.0 for inte_comp in inte_by_efge.keys()}
            ...
            for spacedir_index, space_dir in enumerate(space_dirs):
                ...
                for inte_comp in inte_by_efge.keys():
                    ...
                    Rm_by_inte_by_spacedir[space_dir][inte_comp] = signR_k * math.sqrt(
                        Rm_by_inte_by_spacedir[space_dir][inte_comp]
                    )
                    paras.append(inte_comp)
            ...
            for permsign in itertools.product((-1, 1), (-1, 1), (-1, 1)):
                for first_spacecomp_index, first_spacecomp in enumerate(space_dirs):
                    ...
                    paras = ["INTITULE", "NOM_CHAM", "COMB", "TYPE"]
                    permspace_labels = [
                        "%.1f" % comb_coefs[idx] + c if comb_coefs[idx] != 1 else c
                        for idx, c in enumerate(permspace_comps)
                    ]
                    ...
                    vales = [
                        ...
                        "EFGE_NOEU",
                        comb_intitule,
                        "NEWMARK" + "".join(["".join(x) for x in zippermspace]),
                    ]
                    for inte_comp in inte_by_efge.keys():
                        Ei = 0.0
                        for spacedir_index, space_dir in enumerate(permspace_comps):
                            Ei = (
                                Ei
                                + permsign[spacedir_index]
                                * comb_coefs[spacedir_index]
                                * Rm_by_inte_by_spacedir[space_dir][inte_comp]
                            )
...
def calc_coupure_ops(self, **args):
    ...
    if "NUME_ORDRE" in args and args["NUME_ORDRE"] is not None:
        resu_nume_ordre = args["NUME_ORDRE"]
    elif "NOM_CAS" in args and args["NOM_CAS"] is not None:
        index = -1.0
        for i, nom_cas in enumerate(resu.LIST_VARI_ACCES()["NOM_CAS"]):
            if args["NOM_CAS"][0] in nom_cas:
                index = i
        if index == -1.0:
            resu_nume_ordre = resu.LIST_VARI_ACCES()["NUME_ORDRE"]
        else:
            resu_nume_ordre = []
            resu_nume_ordre.append(resu.LIST_VARI_ACCES()["NUME_ORDRE"][index])
    else:
        resu_nume_ordre = resu.LIST_VARI_ACCES()["NUME_ORDRE"]

    macr_lign_extrac = {k: v for k, v in b_extrac.items() if k not in ("TOUT_ORDRE", "NOM_CAS")}
            ...
            if __tbresul is None:
                ...
                __tbresul = CALC_TABLE(
                    ...
                    ACTION=(
                        _F(
                            OPERATION="COMB",
                            NOM_PARA=("INTITULE", "TYPE", "NUME_ORDRE"),
                            TABLE=__tbextr,
                        ),
                    ),
                )
        else:
            __tbresul = calc_resultante(
                lign_num, __tbresul, __tbextr, myintitule_by_num, resu_nume_ordre
            )

    # Combinaison modale
    if len(comb_modes) >= 1:
        __tbresul = calc_comb_modes(
            __tbresul, myintitule_by_num, comb_modes, resu, b_extrac, resu_nume_ordre
        )
    ...
    for lign_num, _ in enumerate(lign_coupe, 1):
        ...
        if lign_num == 1:
            ...
            __tbcoup2 = CALC_TABLE(
                TABLE=__tbcoup2, reuse=__tbcoup2, ACTION=_F(OPERATION="COMB", TABLE=__tblign)
            )
