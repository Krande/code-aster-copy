...
def calc_spec_ops(self, **args):
    ...
    if len(l_f) > 0:
        ...
        l_ech_t = [
            l_t[0][1]["LONGUEUR_DUREE"],
            l_t[0][1]["LONGUEUR_POURCENT"],
            l_t[0][1]["LONGUEUR_NB_PTS"],
        ]
        recouvr_t = [
            l_t[0][1]["RECOUVREMENT_DUREE"],
            l_t[0][1]["RECOUVREMENT_POURCENT"],
            l_t[0][1]["RECOUVREMENT_NB_PTS"],
        ]
        ...
        if l_ech > len(temp) // 2:
            raise_function_error(
                "Vous devez specifier une longueur d"
                + "'"
        ...
        if recouvr > l_ech:
            raise_function_error("La longueur de recouvrement ne peut exceder la longueur ")
    ...
    for occ in l_G + l_H:
        if occ[1]["FENETRE"] == "RECT":
            ...
            if len(para) != 2:
                raise_function_error(
                    "Erreur de taille dans DEFI_FENE : "
                    + "la fenetre exponentielle est definie par exactement deux valeurs"
                )
            fene = [1.0] * int(para[0]) + [
                numpy.exp(para[1] * (i1 - int(para[0] - 1)) * dt)
    ...
    if TAB_ECHANT:  # Cas TAB_ECHANT
        ...
        for i1 in range(len(fonc_py)):
            ...
            crit.sort()
            dt.append(crit[-1])
            if abs((crit[-1] - crit[0]) / crit[-1]) > 1.0e-5:
                raise_function_error("L" + "'" + "echantillonage doit etre fait a pas constant")
        ...
        for j1 in range(N_fen):
            for i1 in range(len(fonc_py)):
                fft.append(
                    FFT.fft(
                        numpy.multiply(
                            sig[i1][j1 * (l_ech - recouvr) : (j1 * (l_ech - recouvr) + l_ech)], fene
                        )
                    )
                )
                if j1 == 0:
                    df.append(1.0 / (dt[i1]) / l_ech)
        ...
        if len(df) > 1:
            ...
            test_df.sort()
            if abs(test_df[-1]) > 1.0e-5:
                raise_function_error(
                    "Toutes les fonctions doivent etre definies "
                    + "avec la meme frequence d"
                    + "'"
                    + "echantillonage"
                )
    ...
    if ECHANT:
        for occ in l_f:
            ...
            if len(vale_sig[0]) != len(vale_sig[1]):
                raise_function_error(
                    "Les vecteurs associes au temps "
            ...
            df.append(1.0 / (crit[-1]) / len(vale_sig[0]))
        ...
        if (test_long[-1] - test_long[0]) != 0:
            raise_function_error(
                "Toutes les fonctions doivent etre definies avec le meme nombre de points"
            )
    ...
    if INTERSPE:
        ...
        for i in range(nb_ord * (nb_ord + 1) // 2):
            mcfact.append(_F(NUME_ORDRE_I=nume_i1[i], NUME_ORDRE_J=nume_j1[i], FONCTION=l_fc[i]))
        inte_out = DEFI_INTE_SPEC(PAR_FONCTION=mcfact, TITRE="DSP")
    ...
    if TRANSFERT:
            ...
            for i1 in range(len(refer)):
                for j1 in range(len(list_ord)):
                    if refer[i1] != list_ord[j1]:
                        mes_i1 = [
                            num_mes[k1] for k1 in ind_ord[ind_refer[i1]]
                        ]  # -- mesures des efforts
        ...
        for i in range(len(nume_i1)):
            mcfact.append(_F(NUME_ORDRE_I=nume_i1[i], NUME_ORDRE_J=nume_j1[i], FONCTION=l_fc[i]))
        inte_out = DEFI_INTE_SPEC(PAR_FONCTION=mcfact, TITRE=nom_frf)
    return inte_out
