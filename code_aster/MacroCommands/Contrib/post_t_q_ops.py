...
def expand_values(self, tabout, liste_noeu_a_extr, titre, type_para):
    ...
    if "FISSURE" in extrtabout.values().keys():
        tabout = CREA_TABLE(
            TITRE=titre,
            LISTE=(
                _F(LISTE_K=extrtabout.values()["FISSURE"], PARA="FISSURE"),
                _F(LISTE_I=extrtabout.values()["NUME_FOND"], PARA="NUME_FOND"),
                _F(LISTE_R=extrtabout.values()[type_para], PARA=type_para),
                _F(LISTE_I=extrtabout.values()["NUM_PT"], PARA="NUM_PT"),
                _F(LISTE_R=extrtabout.values()["ABSC_CURV"], PARA="ABSC_CURV"),
                _F(LISTE_R=Td, PARA="Td"),
                _F(LISTE_R=ERR_Td, PARA="ERR_Td"),
            ),
        )
    else:
        tabout = CREA_TABLE(
            TITRE=titre,
            LISTE=(
                _F(LISTE_K=extrtabout.values()["FOND_FISS"], PARA="FOND_FISS"),
                _F(LISTE_I=extrtabout.values()["NUME_FOND"], PARA="NUME_FOND"),
                _F(LISTE_R=extrtabout.values()[type_para], PARA=type_para),
                _F(LISTE_K=extrtabout.values()["NOEUD_FOND"], PARA="NOEUD_FOND"),
                _F(LISTE_I=extrtabout.values()["NUM_PT"], PARA="NUM_PT"),
                _F(LISTE_R=extrtabout.values()["ABSC_CURV"], PARA="ABSC_CURV"),
                _F(LISTE_R=Td, PARA="Td"),
                _F(LISTE_R=ERR_Td, PARA="ERR_Td"),
...
def verif_config_init(FOND_FISS):
    ...
    iret, ibid, config = aster.dismoi("CONFIG_INIT", FOND_FISS.getName(), "FOND_FISS", "F")
    if config != "COLLEE":
...
def get_direction(Nnoff, ndim, Lnoff, FOND_FISS, MAILLAGE):
    ...
    if ndim == 2:
        VNOR[0] = NP.array([Basefo[0], Basefo[1], 0.0])
        VDIR[0] = NP.array([Basefo[2], Basefo[3], 0.0])
...
def get_tab_dep(
    ...
    for i in range(Nnocal):
        ...
        Pextr = Porig - (1.0 * hmax * dicVDIR[Lnocal[i]])
        PextrH = Porig + (1.0 * hmax * dicVDIR[Lnocal[i]])
        PextrV = Porig + (1.0 * hmax * dicVNOR[Lnocal[i]])
        PextrQ = Porig + (5.0 * hmax * dicVDIR[Lnocal[i]])
    ...
    __TlibHd = MACR_LIGN_COUPE(RESULTAT=RESULTAT, NOM_CHAM="DEPL", MODELE=MODEL, LIGN_COUPE=mcfactH)

    __TlibHs = MACR_LIGN_COUPE(
        RESULTAT=RESULTAT, NOM_CHAM="SIGM_NOEU", MODELE=MODEL, LIGN_COUPE=mcfactH
    )
    ...
    __TlibVd = MACR_LIGN_COUPE(RESULTAT=RESULTAT, NOM_CHAM="DEPL", MODELE=MODEL, LIGN_COUPE=mcfactV)

    __TlibVs = MACR_LIGN_COUPE(
        RESULTAT=RESULTAT, NOM_CHAM="SIGM_NOEU", MODELE=MODEL, LIGN_COUPE=mcfactV
    )
    ...
    __TlibQd = MACR_LIGN_COUPE(RESULTAT=RESULTAT, NOM_CHAM="DEPL", MODELE=MODEL, LIGN_COUPE=mcfactQ)

    __TlibQs = MACR_LIGN_COUPE(
        RESULTAT=RESULTAT, NOM_CHAM="SIGM_NOEU", MODELE=MODEL, LIGN_COUPE=mcfactQ
    )
...
def get_saut(self, pgl, ds, Ss, di, Si, Sh, Sv, Sq, INFO, FISSURE, syme_char, abscs, ndim):
    ...
    if ndim == 3:
        sauttt[2] = NP.array(
            [[Spls[2][j] / ((Spls[1][j]) + (Spls[0][j])) for j in range(len(Spls[2]))]]
        )
    ...
    if FISSURE:
        ...
        Spli = [NP.multiply(Spls[0], -1.0), Spls[1], Spls[2]]
...
def get_kgsig(saut, sautt, sauttt, nbval, coetd):
    ...
    saut2 = saut * NP.array([[coetd] * nbval, [coetd] * nbval, [coetd] * nbval])

    sautt2 = sautt * NP.array([[1] * nbval, [1] * nbval, [1] * nbval])

    sauttt2 = sauttt * NP.array([[1] * nbval, [1] * nbval, [1] * nbval])
...
def get_erreur(self, ndim, __tabi, type_para):
    ...
    # rename k parameters
    __tabi = CALC_TABLE(
        TABLE=__tabi, reuse=__tabi, ACTION=(_F(OPERATION="RENOMME", NOM_PARA=("Td_MAX", "Td")),)
    )
    # create error
    if ndim != 3:
        __tini = CREA_TABLE(
            LISTE=(
                _F(LISTE_R=(tuple(__tabi.EXTR_TABLE().values()["Td"])), PARA="Td"),
                _F(LISTE_R=(tuple(err[0].tolist())), PARA="ERR_Td"),
            )
        )
    else:
        __tini = CREA_TABLE(
    ...
    __tabi = CALC_TABLE(
        TABLE=__tabi,
        reuse=__tabi,
        ACTION=(_F(OPERATION="COMB", NOM_PARA="Td", TABLE=__tini)),
        INFO=1,
    )
...
def post_t_q_ops(self, **args):
    ...
    if "EVOL_THER" in args:
        if args.get("EVOL_THER") is not None:
    ...
    if FOND_FISS:
        ...
        iret, ibid, syme_char = aster.dismoi("SYME", FOND_FISS.getName(), "FOND_FISS", "F")
        ...
        if TYPE_MAILLAGE == "LIBRE" or TYPE_MAILLAGE == "REGLE":
            ...
            (__TlibS, __TlibI, __TlibH, __TlibV, __TlibQ) = get_tab_dep(
                self,
                Lnocal,
                Nnocal,
                d_coorf,
    ...
    if Tempe3D:
        ...
        __TEMP = POST_RELEVE_T(
            ACTION=_F(
                INTITULE="Temperature fond de fissure",
                NOEUD=Lnofon,
                TOUT_CMP="OUI",
                RESULTAT=Rth,
                NOM_CHAM="TEMP",
    ...
    for ino in range(0, Nbnofo):
        ...
        for iord, inst in enumerate(l_inst):
            ...
            (abscs, ds, Ss) = get_depl_sup(
                FOND_FISS, tabsupi, ndim, Lnofon, d_coor, ino, TYPE_MAILLAGE
            )

            (absci, di, Si) = get_depl_inf(
                FOND_FISS, tabinfi, ndim, Lnofon, syme_char, d_coor, ino, TYPE_MAILLAGE
            )

            (absch, dh, Sh) = get_depl_H(
                FOND_FISS, tabHi, ndim, Lnofon, syme_char, d_coor, ino, TYPE_MAILLAGE
            )

            (abscv, dv, Sv) = get_depl_V(
                FOND_FISS, tabVi, ndim, Lnofon, syme_char, d_coor, ino, TYPE_MAILLAGE
            )

            (abscq, dq, Sq) = get_depl_Q(
                FOND_FISS, tabQi, ndim, Lnofon, syme_char, d_coor, ino, TYPE_MAILLAGE
            )
            ...
            if nbval < 3:
                ...
                #           CALCUL T, Tz, Q
                (isig, isigt, isigtt, tgsig, ttgsig, tttgsig, saut2, sautt2, sauttt2) = get_kgsig(
                    saut, sautt, sauttt, nbval, coetd
                )
