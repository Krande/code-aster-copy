...
CALC_IFS_DNL_CATA = MACRO(
    ...
    fr=tr(
        "Calcul de l'évolution dynamique d'une structure couplée à un "
    ...
    EXCIT=FACT(
        ...
        # <=
        # IFS
        regles=(PRESENT_ABSENT("FONC_MULT", "ACCE"), PRESENT_PRESENT("ACCE", "VITE", "DEPL")),
        TYPE_CHARGE=SIMP(
    ...
    b_reuse=BLOC(
        ...
        fr=tr("en mode concept reentrant : ETAT_INIT obligatoire"),
        ETAT_INIT=C_ETAT_INIT("DYNA_NON_LINE", "o"),
    ),
    b_notreuse=BLOC(
        condition="""not exists("reuse")""",
        fr=tr("en mode concept reentrant : ETAT_INIT facultatif"),
    ...
    SCHEMA_TEMPS=FACT(
        ...
        b_theta=BLOC(
            ...
            THETA=SIMP(statut="f", typ="R", defaut=1.0, val_min=0.5, val_max=100.0),
        ),
        b_krenk=BLOC(
            condition="SCHEMA=='KRENK'",
            KAPPA=SIMP(statut="f", typ="R", defaut=1.0, val_min=1.0, val_max=100.0),
        ),
        b_explicit=BLOC(
        ...
        b_implicit=BLOC(
            ...
            FORMULATION=SIMP(
                statut="o", max=1, typ="TXM", into=("DEPLACEMENT", "VITESSE", "ACCELERATION")
            ),
        ),
    ),
    # -------------------------------------------------------------------
    ...
    b_info=BLOC(
        ...
        INFO_DBG=SIMP(
            ...
            max="**",
            validators=NoRepeat(),
            into=("CONTACT", "MECA_NON_LINE", "PILOTAGE", "FACTORISATION", "APPARIEMENT"),
        ),
    ),
