...
def change_test_resu():
    ...
    reval = re.compile(
        "^ *(OK|NOOK|SKIP) +NON_REGRESSION +(?P<leg>.+?) +"
        "(?P<refe>.+?) +(?P<calc>.+?) +(?P<err>.+?) +(?P<tole>.+?) *$",
        re.M,
    )
...
def read_keyword_value(kw, txt):
    ...
    re_vale = re.compile(r"(?P<key>%s) *= *(?P<val>[^, \)]+)" % kw, re.M)
    found = []
...
def get_dest_filename(fname, nb):
    ...
    lexp = glob(osp.join("astest", root + ".export")) + glob(
        osp.join("../validation/astest", root + ".export")
    )
    if lexp:
