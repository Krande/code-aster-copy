    ...
    **args
    ...
    if ETAT_INIT is not None:
        ...
        try:
            ...
            if (
                eval(errText)[0][0] == "MED_32"
            ):  # exception MED_32 (field does not exist in MED file)
                aster.affiche("MESSAGE", "")
                aster.affiche(
                    "MESSAGE", "==> Initialisation avec un état nul (endommagement absent)"
                )
                aster.affiche("MESSAGE", "")
            else:
                raise err
    ...
    if initial:
        ...
        cvg["RESI_REFE_RELA"] = min(1.0e-6, cvg["RESI_REFE_RELA"])
        ...
        __UNLOAD_DEPL = CREA_CHAMP(
            OPERATION="EXTR", TYPE_CHAM="NOEU_DEPL_R", RESULTAT=__UNLOAD, INST=-1, NOM_CHAM="DEPL"
        )
        __UNLOAD_SIGM = CREA_CHAMP(
            OPERATION="EXTR",
        ...
        asse = Extract(
            ...
            __UNLOAD_VARI,
            __PROV_VARI,
            ENDO_FISS_EXP=("EPSEXX", "EPSEYY", "EPSEZZ", "EPSEXY", "EPSEXZ", "EPSEYZ"),
        )
        __UNLOAD_VARI_MOD = CREA_CHAMP(
            TYPE_CHAM="ELGA_VARI_R", MODELE=MODELE, OPERATION="ASSE", ASSE=asse
        )
        ...
        para = dict(
            ...
            NEWTON=_F(
                MATRICE="TANGENTE", REAC_ITER=1, PAS_MINI_ELAS=2, REAC_ITER_ELAS=0
            ),  # secant matrix
            ARCHIVAGE=_F(INST=0),
        )
        ...
        if iteglob <= 5:
            ...
            __RAMPE = DEFI_FONCTION(
                NOM_PARA="INST", VALE=(-1, 0, 0, eta * 0.99)  # some margin in case of instability
            )
            ...
            para = dict(
                ETAT_INIT=_F(
                    DEPL=__UNLOAD_DEPL, SIGM=__UNLOAD_SIGM, VARI=__UNLOAD_VARI_MOD, INST=-1
                ),
                MODELE=MODELE,
                CHAM_MATER=CHAM_MATER,
                EXCIT=loading_1,
    ...
    __SUB = DEFI_LIST_INST(
        DEFI_LIST=_F(LIST_INST=__SEQU), ECHEC=_F(SUBD_PAS=4, SUBD_NIVEAU=level + 2)
    )
...
def Extract(COMPORTEMENT, baseField, cplField=None, **components):
    ...
    for (compor, law) in zip(COMPORTEMENT, laws):
        ...
        if cplField is not None and lawName in list(components.keys()):
