...
POST_T_Q_CATA = MACRO(
    ...
    b_fond_fiss=BLOC(
        condition="FOND_FISS is not None",
        b_no_mod=BLOC(
            condition="AsType(RESULTAT)!= mode_meca",
            EVOL_THER=SIMP(
                statut="f", typ=(evol_ther), fr=tr("Température sur le fond de fissure")
            ),
            b_ref_3D=BLOC(
                condition="MODELISATION=='3D' ",
                TYPE_MAILLAGE=SIMP(statut="f", typ="TXM", into=("LIBRE", "REGLE"), defaut="REGLE"),
            ),
        ),
        b_mod=BLOC(
            condition="AsType(RESULTAT)== mode_meca and MODELISATION=='3D'",
    ...
    MATER=SIMP(
        ...
        typ=mater_sdaster,
        fr=tr(
            "Matériau homogène et isotrope cohérent avec celui utilisé pour le calcul des déplacements"
        ),
    ),
    ...
    b_mod_meca=BLOC(
        ...
        b_acce_reel=BLOC(
            condition="(FREQ is not None) or (LIST_FREQ is not None)",
    ...
    b_no_mod_meca=BLOC(
        ...
        b_acce_reel=BLOC(
            condition="(INST is not None)or(LIST_INST is not None)",
            CRITERE=SIMP(statut="f", typ="TXM", defaut="RELATIF", into=("RELATIF", "ABSOLU")),
            b_prec_rela=BLOC(
                condition="(CRITERE=='RELATIF')", PRECISION=SIMP(statut="f", typ="R", defaut=1.0e-6)
            ),
...
POST_T_Q = UserMacro("POST_T_Q", POST_T_Q_CATA, post_t_q_ops)
