...
from .recu_para_yacs import RECU_PARA_YACS


def calc_ifs_dnl_ops(
    self,
    GROUP_MA_IFS,
    NOM_CMP_IFS,
    UNITE_NOEUD,
    ...
    **args
    ...
    _strucMesh = DEFI_GROUP(reuse=_strucMesh, MAILLAGE=_strucMesh, **grpRefeMocle)
    ...
    _fluidNodeMesh = DEFI_GROUP(reuse=_fluidNodeMesh, MAILLAGE=_fluidNodeMesh, **grpProjMocle)
    ...
    # print "grpProjMocle=",grpProjMocle
    _fluidElemMesh = DEFI_GROUP(reuse=_fluidElemMesh, MAILLAGE=_fluidElemMesh, **grpProjMocle)
    ...
    _ifsCharMeca = AFFE_CHAR_MECA(MODELE=MODELE, FORCE_NODALE=_F(**ifsCharMocle))
    dExcit = []
    ...
    if _numpas <= _nbTimeStep:
        ...
        _tStart = _tInitial
        _pastps = RECU_PARA_YACS(
            DONNEES="PAS", NUME_ORDRE_YACS=_numpas, INST=_tStart, PAS=_timeStepAster
        )
        ...
        _liste = DEFI_LIST_REEL(DEBUT=_tStart, INTERVALLE=_F(JUSQU_A=_tEnd, NOMBRE=1))
        #    _iter = 1
        _SousIterations = 0
        icv = 0
        while _SousIterations < _nbSsIterMax:
            ...
            # test de convergence
            # ~~~~~~~~~~~~~~~~~~~
            _ticv = RECU_PARA_YACS(
                DONNEES="CONVERGENCE", NUME_ORDRE_YACS=_ntcast, INST=_tEnd, PAS=_timeStep
            )
            ...
            if icv == 1 or _SousIterations == _nbSsIterMax:
                ...
                print("EXTR_RESU")
                resdnl = EXTR_RESU(RESULTAT=__rescur, ARCHIVAGE=_F(NUME_ORDRE=(0, 1)))
                # resdnl = __rescur
    ...
    while _numpas <= _nbTimeStep:
        ...
        _pastps = RECU_PARA_YACS(
            DONNEES="PAS", NUME_ORDRE_YACS=_numpas, INST=_tyacs, PAS=_timeStepAster
        )
        _pastps0 = _pastps.getValues()
        print("_pastps0[0]=", _pastps0[0])
        _timeStep = _pastps0[0]
        ...
        while _SousIterations < _nbSsIterMax:
            ...
            if icv == 1 or _SousIterations == _nbSsIterMax:
                ...
                ENV_CINE_YACS(
                    RESULTAT=_F(RESU=resdnl, NUME_ORDRE=_numpas),
                    MATR_PROJECTION=_fluidNodePoids,
                    NUME_ORDRE_YACS=_ntcast,
                    INST=_tEnd,
                    PAS=_timeStep,
                ...
                _list2 = DEFI_LIST_ENTI(DEBUT=0, INTERVALLE=_F(JUSQU_A=_numpas - 1, PAS=1))
                resdnl = EXTR_RESU(RESULTAT=resdnl, ARCHIVAGE=_F(LIST_ORDRE=_list2))
    ...
    if _impMed == 1:
        ...
        _listImp = DEFI_LIST_ENTI(DEBUT=_StartImp, **impEnsiMocle)
        print("Liste impression=", _listImp)
        IMPR_RESU(FORMAT="MED", RESU=_F(MAILLAGE=_strucMesh, RESULTAT=resdnl, LIST_ORDRE=_listImp))
