    ...
    **args
    ...
    if IOCFLU == 1:
        ...
        __NOMCMA = AFFE_MATERIAU(
            MAILLAGE=MAILLAGE, AFFE=_F(GROUP_MA=(GROUP_MA_FLUIDE, GROUP_MA_INTERF), MATER=__NOMMAT)
        )
    ...
    __NOMFLU = AFFE_MODELE(
        MAILLAGE=MAILLAGE,
        AFFE=_F(
            GROUP_MA=(GROUP_MA_FLUIDE, GROUP_MA_INTERF),
    ...
    __NOMINT = AFFE_MODELE(
        MAILLAGE=MAILLAGE,
        AFFE=_F(GROUP_MA=GROUP_MA_INTERF, MODELISATION=MODELISATION, PHENOMENE="THERMIQUE"),
    )
    ...
    if (MATR_AMOR_AJOU is not None) or (MATR_RIGI_AJOU is not None):
        ...
        __NOFLUI = AFFE_MODELE(
            MAILLAGE=MAILLAGE,
            AFFE=_F(GROUP_MA=grma, MODELISATION=MODELISATION, PHENOMENE="THERMIQUE"),
        )
        affimp = []
        for DDL in DDL_IMPO:
            if DDL["PRES_SORTIE"] is not None:
                ...
                if DDL["GROUP_NO"] is not None:
                    mfact = _F(GROUP_NO=DDL["GROUP_NO"], TEMP=DDL["PRES_SORTIE"])
                affimp.append(mfact)
        ...
        for ECL in ECOULEMENT:
            ...
            affecl.append(mfact)
        __CHARG2 = AFFE_CHAR_THER(MODELE=__NOFLUI, TEMP_IMPO=affimp, FLUX_REP=affecl)

        __DLZ = DEFI_LIST_REEL(VALE=(0.0))
        __POTEN = THER_LINEAIRE(
            MODELE=__NOFLUI,
            CHAM_MATER=__NOMCMA,
    ...
    if FORC_AJOU is not None:
        for FORCAJ in FORC_AJOU:
            ...
            VECTAJ = CALC_FORC_AJOU(
                DIRECTION=FORCAJ["DIRECTION"],
                MODELE_FLUIDE=__NOMFLU,
                MODELE_INTERFACE=__NOMINT,
                CHARGE=__CHARGE,
                CHAM_MATER=__NOMCMA,
