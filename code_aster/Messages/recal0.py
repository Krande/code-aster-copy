...
"""
    ...
    3: _(
        """Le logiciel GNUPLOT ou le module python Gnuplot n'est pas disponible.
On désactive l'affichage des courbes par Gnuplot.
"""
    ),
    4: _(
        """Il n'y a pas de fichier .export dans le répertoire de travail !
"""
    ),
    5: _(
        """Il y a plus d'un fichier .export dans le répertoire de travail !
"""
    ),
    6: _(
        """Pour les calculs DISTRIBUES en mode INTERACTIF, il faut spécifier une valeur pour mem_aster
(menu Option de ASTK) pour limiter la mémoire allouée au calcul maître.
"""
    ),
    7: _(
        """Pour pouvoir lancer les calculs esclaves en MPI, le calcul maître doit être lancé
en MPI sur un processeur."""
    ),
    8: _(
        """Vérifier les valeurs des paramètres mem_aster et memjeveux.
"""
    ),
    # 9 : _(u""" """),
    10: _(
        """Pour l'algorithme %(k1)s, on ne peut tracer qu'à la dernière itération.
"""
    ),
    11: _(
        """Pour l'algorithme %(k1)s, on ne tient pas compte des bornes sur les paramètres.
"""
    ),
    12: _(
        """Recalage :
...
"""
    ...
    13: _(
        """Lancement de l'optimisation avec la méthode : %(k1)s.
"""
    ),
    14: _(
        """Les dérivées sont calculées par Aster.
"""
    ),
    15: _(
        """Les dérivées sont calculées par l'algorithme.
"""
    ),
    16: _(
        """
...
"""
    ...
    18: _(
        """Erreur dans l'algorithme de bornes de MACR_RECAL.
"""
    ),
    19: _(
        """Erreur dans le test de convergence de MACR_RECAL.
"""
    ),
    24: _(
        """Impossible de récupérer les résultats de calcul esclave (lecture des tables) !
Message d'erreur :
   %(k1)s
"""
    ),
...
"""
    ...
    37: _(
        """=> Variation des paramètres (norme L2)  = %(r1)f
"""
    ),
    38: _(
        """=> Variation de la fonctionnelle        = %(r1)f
"""
    ),
    39: _(
        """=> Nombre d'évaluation de la fonction   = %(k1)s
"""
    ),
    # 40 : _(u""" """),
    41: _(
        """Tracé des courbes dans le fichier : %(k1)s
"""
    ),
    42: _(
        """Problème lors de l'affichage des courbes. On ignore et on continue.
Erreur :
   %(k1)s
"""
    ),
...
"""
    ...
    45: _(
        """Problème de division par zéro dans le calcul de la matrice de sensibilité.
Le paramètre %(k1)s est nul ou plus petit que la précision machine.
"""
    ),
    46: _(
        """Le paramètre %(k1)s est en butée sur un bord du domaine admissible.
"""
    ),
    47: _(
        """Les paramètres %(k1)s sont en butée sur un bord du domaine admissible.
"""
    ),
    48: _(
        """Problème lors de l'interpolation du calcul dérivé sur les données expérimentale !
...
"""
    ...
    82: _(
        """Impossible de créer le répertoire temporaire : %(k1)s
"""
    ),
    83: _(
        """
