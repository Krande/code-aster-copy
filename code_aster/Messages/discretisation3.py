...
cata_msg = {
    1: _("""Gestion manuelle de la liste d'instants."""),
    2: _("""Gestion automatique de la liste d'instants."""),
    3: _(
...
"""
    ...
    4: _(
        """La liste d'instants contient %(i1)d pas de temps et le pas de temps minimum vaut %(r1)13.6G."""
    ),
    5: _("""Il y a %(i1)d événements."""),
    ...
    16: _("""L'événement %(i1)d est pour détecter quand le résidu dépasse une valeur donnée."""),
    21: _(
        """Avec ce critère pour la variation d'une grandeur: ( Champ: %(k1)s , Composante: %(k2)s ) %(k3)s %(r1)13.6G ."""
    ),
    22: _("""La pénétration maximale est de %(r1)13.6G ."""),
    ...
    62: _(
        """On découpe le pas de temps en %(i1)d incréments jusqu'à ce que le niveau atteigne %(i2)d."""
    ),
    63: _(
        """Le seuil est franchi quand, %(i1)d fois de suite, on fait exactement ou moins de %(i2)d itérations de Newton."""
    ),
    64: _(
        """Le seuil est franchi quand, %(i1)d fois de suite, on fait strictement moins de %(i2)d itérations de Newton."""
    ),
    65: _(
        """Le seuil est franchi quand, %(i1)d fois de suite, on fait exactement ou plus de %(i2)d itérations de Newton."""
    ),
    66: _(
        """Le seuil est franchi quand, %(i1)d fois de suite, on fait strictement plus de %(i2)d itérations de Newton."""
    ),
    70: _(
        """La découpe du pas de temps est automatique et utilise la méthode d'extrapolation à partir du résidu."""
    ),
    71: _(
        """La découpe du pas de temps est automatique et utilise la méthode d'estimation pour la gestion de la collision."""
    ),
    72: _(
        """La gestion du pas de temps après collision se fait à l'instant %(r1)13.6G et pour une durée de %(r2)13.6G ."""
    ),
    80: _("""Le mode de calcul de l'instant suivant est fixe."""),
    81: _("""Le mode de calcul de l'instant suivant utilise la variation d'une grandeur."""),
    82: _(
        """Le mode de calcul de l'instant suivant utilise la variation du  nombre d'itérations de Newton."""
    ),
    83: _("""Le mode de calcul de l'instant suivant utilise un schéma spécifique pour IMPLEX."""),
    84: _("""Le pas de temps suivant sera modifié de %(r1).2f %%."""),
    85: _(
