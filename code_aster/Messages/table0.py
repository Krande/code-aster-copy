...
"""
    ...
    19: _(
        """La formule dépend de %(i1)d paramètre(s).
Vous devez fournir autant de nom de colonne dans NOM_COLONNE."""
    ),
    20: _(
        """Erreur lors de la construction des n-uplets
"""
    ),
    21: _(
        """La table doit avoir exactement deux paramètres pour une impression au format XMGRACE.
"""
    ),
    22: _(
        """Les cellules ne doivent contenir que des nombres réels
"""
    ),
    23: _(
        """Le paramètre %(k1)s est en double.
"""
    ),
    24: _(
        """Le paramètre %(k1)s existe déjà.
"""
    ),
    25: _(
        """'%(k1)s' n'a pas d'attribut '%(k2)s'.
"""
    ),
    27: _(
        """Les paramètres n'existent pas dans la table : %(k1)s
"""
    ),
    28: _(
        """L'argument '%(k1)s' doit être de type '%(k2)s'.
"""
    ),
    29: _(
        """Valeur incorrecte pour ORDRE : %(k1)s
"""
    ),
    30: _(
        """Les paramètres doivent être les mêmes dans les deux tables pour
faire l'intersection  ou l'union (opérateurs &, |).
"""
    ),
    31: _(
        """Type du paramètre '%(k1)s' non défini.
"""
    ),
    32: _(
        """Type du paramètre '%(k1)s' forcé à '%(k2)s'.
"""
    ),
    33: _(
        """Erreur pour le paramètre '%(k1)s' :
   %(k2)s
"""
    ),
    34: _(
        """La colonne '%(k1)s' est vide.
"""
    ),
    35: _(
        """La table est vide !
"""
    ...
    36: _(
        """La table doit avoir exactement trois paramètres.
"""
    ),
    37: _(
        """
...
"""
    ...
    38: _(
        """Champ %(k1)s inexistant à l'ordre %(i1)d .
"""
    ),
    39: _(
        """
