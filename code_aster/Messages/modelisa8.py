...
"""
    ...
    10: _(
        """Le noeud %(k1)s connecte plus de 200 mailles.
"""
    ),
    11: _(
        """Le noeud %(k1)s est orphelin.
"""
    ),
    12: _(
        """La maille %(k1)s est topologiquement dégénérée : noeud répété dans la connectivité.
    Type de maille : %(k2)s
"""
    ),
    13: _(
        """Les mailles %(k1)s et %(k2)s sont doubles (même noeuds en support).
    Type de maille : %(k3)s
"""
    ),
    14: _(
        """La maille POI1 %(k1)s est incluse dans une autre.
"""
    ),
    15: _(
        """La maille %(k1)s possède des noeuds confondus géométriquement.
