...
class ERREUR:
    ...
    def veri_appartient_liste(self, code, element, liste):
        try:
            ...
            self.mess(code, str(element) + " n'appartient pas a la liste: " + str(liste))
    ...
    def veri_pas_doublon_lpara(self, code, liste):
        ...
        if len(lpara) != len(liste):
            self.mess(
                code,
                "Le paramètre " + param + " apparait plusieurs fois dans la liste: " + str(liste),
            )
    ...
    def veri_long_chaine(self, code, chaine, n):
        if len(chaine) > n:
            self.mess(
                code,
                chaine + " est une chaine de caractères trop longue (>" + str(n) + ").\n"
...
class JV_COLLEC:
    ...
    def cree_oc(self, nom, long):
        ...
        if num > 0:
            ERR.mess("F", "Erreur : nom existant deja: " + nom + " dans: " + self.nom)
        if self.longv > 0 and long != self.longv:
            ERR.mess("F", "Erreur : longueur incorrecte: " + str(long) + " pour: " + self.nom)
        self.objs.append(oc1)

    def ecri_co(self, nom, indice, valeur):
        num = self.pn.jenonu(nom)
        if num < 0:
            ERR.mess("F", "Erreur : nom inexistant: " + nom + " dans: " + self.nom)
        oc1 = self.objs[num - 1]
    ...
    def impr(self, file):
        ...
        # TODO nutioc == nmaxoc ?
        lines.append(
            "|NOM={self.nom:24}|TYPE={self.tsca:3}|NMAXOC={nmaxoc:>12}"
            "|NUTIOC={nmaxoc:>12}|ACCES={self.acces:2}|STOCKAGE={self.contig:8}"
            "|MODELONG={modlon:8}|LONMAX={self.longv:>12}|LONT={lont:>12}".format(
                self=self, nmaxoc=nmaxoc, modlon=modlon, lont=lont
            )
        )
        ...
        for oc1 in self.objs:
            if self.acces == "NO":
                ...
                # acces K8
                lines.append(
                    "|NOM={nom:8}|LONMAX={len:>12}".format(nom=oc1.nom, len=len(oc1.valeurs))
                )
            else:
...
class JV_SIMPLE:
    ...
    def impr(self, file):
        ...
        lines.append(
            "|NOM={self.nom:24}|TYPE={self.tsca:3}|LONMAX={self.long:>12}".format(self=self)
        )
        if self.tsca[0] == "K":
...
class JV_PNOM:
    ...
    def jenonu(self, nom, stop="PAS_COOL"):
        ...
        if num is not None:
            ...
            if stop != "COOL":
                ERR.mess("F", "Erreur: nom <" + nom + "> inexistant dans: " + self.nom)
    ...
    def ajout_nom(self, nom):
        ...
        if self.dico.get(nom) is not None:
            ERR.mess("F", "Erreur: le nom: " + nom + " existe deja dans: " + self.nom)
        else:
            indice = self.nomuti + 1
            self.nomuti = indice
            ERR.veri_long_chaine("E", nom, int(self.tsca[1:3]))
    ...
    def impr(self, file):
        ...
        lines.append(
            "|NOM={self.nom:24}|TYPE={self.tsca:3}|NOMMAX={self.nomuti:>12}".format(self=self)
        )
