...
# --------------------------------------------------------------------------------------------------
class THER3DQU9_HHO0_F(Element):
    """Thermics - Skin element HHO_CSTE - 3D - QUAD"""

    meshType = MT.QUAD9
    nodes = (SetOfNodes("EN1", (9,)), SetOfNodes("EN2", (1, 2, 3, 4, 5, 6, 7, 8)))
    attrs = ((AT.BORD_ISO, "OUI"),)
    elrefe = (ElrefeLoc(MT.QU9, gauss=("RIGI=FPG1",), mater=("RIGI",)),)
    calculs = (
        OP.CHAR_THER_FLUN_F(
            te=461,
            para_in=(
...
# --------------------------------------------------------------------------------------------------
class THER3DTR7_HHO0_F(THER3DQU9_HHO0_F):
    """Thermics - Skin element 3D_HHO_CSTE - TRIA"""

    meshType = MT.TRIA7
    nodes = (SetOfNodes("EN1", (7,)), SetOfNodes("EN2", (1, 2, 3, 4, 5, 6)))
    attrs = ((AT.BORD_ISO, "OUI"),)
    elrefe = (ElrefeLoc(MT.TR7, gauss=("RIGI=FPG1",), mater=("RIGI",)),)
