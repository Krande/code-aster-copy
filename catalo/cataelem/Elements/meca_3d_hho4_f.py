...
DDL_MECA = LocatedComponents(
    ...
    type="ELNO",
    diff=True,
    components=(("EN1", ("HHO_FX[15]", "HHO_FY[15]", "HHO_FZ[15]")), ("EN2", ())),
)

CHHOBS = LocatedComponents(
    phys=PHY.N3600R, type="ELNO", diff=True, components=(("EN1", ("X[120]",)), ("EN2", ()))
)
...
CFORCER = LocatedComponents(phys=PHY.FORC_R, type="ELEM", components=("FX", "FY", "FZ"))
...
# ------------------------------------------------------------
class MECA3DQU9_HHO4_F(Element):
    """Please document this element"""

    meshType = MT.QUAD9
    nodes = (SetOfNodes("EN1", (9,)), SetOfNodes("EN2", (1, 2, 3, 4, 5, 6, 7, 8)))
    attrs = ((AT.BORD_ISO, "OUI"),)
    elrefe = (ElrefeLoc(MT.QU9, gauss=("RIGI=FPG25",), mater=("RIGI",)),)
    calculs = (
        OP.CHAR_MECA_PRES_F(
            te=459,
            para_in=(
        ...
        OP.CHAR_MECA_FF2D3D(
            ...
            para_in=(
                ...
                (SP.PGEOMER, NGEOMER),
                (SP.PINSTR, CTEMPSR),
                (OP.CHAR_MECA_FF2D3D.PCHHOBS, CHHOBS),
            ),
            para_out=((SP.PVECTUR, MVECTUR),),
        ),
        OP.CHAR_MECA_FR2D3D(
            ...
            para_in=(
                (SP.PFR2D3D, NFORCER),
                (SP.PGEOMER, NGEOMER),
                (OP.CHAR_MECA_FR2D3D.PCHHOBS, CHHOBS),
            ),
            para_out=((SP.PVECTUR, MVECTUR),),
        ),
        OP.COOR_ELGA(
...
# ------------------------------------------------------------
class MECA3DTR7_HHO4_F(MECA3DQU9_HHO4_F):
    """Please document this element"""

    meshType = MT.TRIA7
    nodes = (SetOfNodes("EN1", (7,)), SetOfNodes("EN2", (1, 2, 3, 4, 5, 6)))
    attrs = ((AT.BORD_ISO, "OUI"),)
    elrefe = (ElrefeLoc(MT.TR7, gauss=("RIGI=FPG16",), mater=("RIGI",)),)
