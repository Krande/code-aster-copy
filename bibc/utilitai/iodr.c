...
long ind_fac( char *nom ) {
    ...
    res = -1;
    i = 0;
    while ( ( i < nbFAC ) && ( strcmp( nom, nomFAC[i] ) != 0 ) )
        i++;
    if ( i < nbFAC )
        res = i;
...
void DEFSPP( OPENDR, opendr, char *dfname, STRING_SIZE len_dfname, ASTERINTEGER *mode,
             ASTERINTEGER *ierr ) {
    /*
        mode = 0 : ro read only
    ...
    *ierr = 0;
    ...
    if ( fpfile[iu] != NULL ) {
        ...
        if ( imode == 2 ) {
            ...
            CALL_UTMESS_CORE( "I", "JEVEUX_45", &n1, valk, &n0, &ibid, &n0, &rbid, &n0, " " );
            nenr[iu] = -1;
            *ierr = 0;
        } else {
            DEBUG_IODR( "open in %s mode: %s\n", "read", fname );
            CALL_UTMESS_CORE( "I", "JEVEUX_44", &n1, valk, &n0, &ibid, &n0, &rbid, &n0, " " );
