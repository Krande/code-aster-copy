...
static char putvectjev_doc[] = "putvectjev(nomsd, nbval, indices, reel, imag, num)\n\
\n\
...
static char putcolljev_doc[] = "putcolljev(nomsd)->valsd      \n\
\n\
...
static PyObject *aster_fclose( PyObject *self, PyObject *args ) {
    ...
    if ( !PyArg_ParseTuple( args, "i:fort_fclose", &iunit ) )
        return NULL;
    unit = (ASTERINTEGER)iunit;
    CALL_FCLOSE( &unit );
...
static PyMethodDef aster_methods[] = {
    ...
    { "fort_fclose", aster_fclose, METH_VARARGS },
