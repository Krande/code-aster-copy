...
void calc_SPEC_OSCI( int nbpts, double *vale_x, double *vale_y, int len_f, double *l_freq,
                     int len_a, double *l_amor, double *spectr ) {
    int i, a, f;
        ...
        for ( a = 0; a < len_a; a++ ) {
            ...
            for ( f = 0; f < len_f; f++ ) {
                ...
                for ( i = 1; i < nbpts; i++ ) {
                    F2 = vale_y[i];
                    FSDT = ( F2 - F1 ) / DELTAT;
                    D3 = EXPSA * ( ( A * COSWDT + D * SINWDT ) * D2 + SINWDT * V2 ) +
                         UNSW2 * ( EXPXWT * ( FSDT * ( B * COSWDT + C * SINWDT ) - F1 * F ) + F2 -
                                   B * FSDT );
