...
void DEFSSSSP( MFRONT_SET_DOUBLE_PARAMETER, mfront_set_double_parameter, char *nomlib,
               STRING_SIZE lnomlib, char *nomsub, STRING_SIZE lnomsub, char *nommod,
               STRING_SIZE lnommod, char *nomparam, STRING_SIZE lnomparam, ASTERDOUBLE *value ) {
#ifdef ASTER_PLATFORM_POSIX
...
void DEFSSSSP( MFRONT_SET_INTEGER_PARAMETER, mfront_set_integer_parameter, char *nomlib,
               STRING_SIZE lnomlib, char *nomsub, STRING_SIZE lnomsub, char *nommod,
               STRING_SIZE lnommod, char *nomparam, STRING_SIZE lnomparam, ASTERINTEGER *value ) {
#ifdef ASTER_PLATFORM_POSIX
    /* MFRONT Wrapper : wrapper to the MFRONT set function through the function pointer
     * Load the library if necessary (at the first call).
     */
...
void DEFPPSP( MFRONT_GET_EXTERNAL_STATE_VARIABLE, mfront_get_external_state_variable,
              ASTERINTEGER *pliesv, ASTERINTEGER *pnbesv, char *txval, STRING_SIZE ltx,
              ASTERINTEGER *nbvarc ) {
#ifdef ASTER_PLATFORM_POSIX
    /* MFRONT Wrapper
     */
...
int mfront_get_number_of_internal_state_variables( char *nomlib, STRING_SIZE lnomlib, char *nomsub,
                                                   STRING_SIZE lnomsub, char *nommod,
                                                   STRING_SIZE lnommod ) {
#ifdef ASTER_PLATFORM_POSIX
    /* MFRONT Wrapper
     */
    ...
    if ( symbname == NULL ) {
        name1 = (char *)malloc( strlen( symbol ) + strlen( "_nInternalStateVariables" ) + 1 );
        strcpy( name1, symbol );
...
int mfront_get_strain_model( char *nomlib, STRING_SIZE lnomlib, char *nomsub, STRING_SIZE lnomsub,
...
void DEFSSSP( MFRONT_GET_NUMBER_OF_INTERNAL_STATE_VARIABLES,
              mfront_get_number_of_internal_state_variables, char *nomlib, STRING_SIZE lnomlib,
              char *nomsub, STRING_SIZE lnomsub, char *nommod, STRING_SIZE lnommod,
              ASTERINTEGER *nbvari ) {
#ifdef ASTER_PLATFORM_POSIX
    /* MFRONT Wrapper
     */
    int nbvari2 = mfront_get_number_of_internal_state_variables( nomlib, lnomlib, nomsub, lnomsub,
                                                                 nommod, lnommod );
    *nbvari = nbvari2;
#else
    printf( "Not available under Windows.\n" );
...
void DEFSSSP( MFRONT_GET_STRAIN_MODEL, mfront_get_strain_model, char *nomlib, STRING_SIZE lnomlib,
              char *nomsub, STRING_SIZE lnomsub, char *nommod, STRING_SIZE lnommod,
     ...
     */
    int strain_model2 =
        mfront_get_strain_model( nomlib, lnomlib, nomsub, lnomsub, nommod, lnommod );
...
void DEFSSSS( MFRONT_GET_INTERNAL_STATE_VARIABLES_TYPES, mfront_get_internal_state_variables_types,
              char *nomlib, STRING_SIZE lnomlib, char *nomsub, STRING_SIZE lnomsub, char *nommod,
              STRING_SIZE lnommod, char *txval, STRING_SIZE ltx ) {
#ifdef ASTER_PLATFORM_POSIX
    /* MFRONT Wrapper
     */
    char *libname, *symbol, *model, *symbname = NULL;
    ...
    unsigned short *nb_int_var =
        (unsigned short *)libsymb_get_symbol( DLL_DICT, libname, symbname );
...
void DEFSSSSP( MFRONT_GET_INTERNAL_STATE_VARIABLES, mfront_get_internal_state_variables,
               char *nomlib, STRING_SIZE lnomlib, char *nomsub, STRING_SIZE lnomsub, char *nommod,
               STRING_SIZE lnommod, char *txval, STRING_SIZE ltx, ASTERINTEGER *nbintvar ) {
#ifdef ASTER_PLATFORM_POSIX
#define MIN( A, B ) ( ( A ) < ( B ) ? ( A ) : ( B ) )
    ...
    int nbcheck = mfront_get_number_of_internal_state_variables( nomlib, lnomlib, nomsub, lnomsub,
                                                                 nommod, lnommod );
    ...
    if ( symbname == NULL ) {
        name1 = (char *)malloc( strlen( symbol ) + strlen( "_InternalStateVariables" ) + 1 );
        strcpy( name1, symbol );
...
void DEFSSSPPPPP( MFRONT_GET_POINTERS, mfront_get_pointers, char *nomlib, STRING_SIZE lnomlib,
                  char *nomsub, STRING_SIZE lnomsub, char *nommod, STRING_SIZE lnommod,
...
void DEFSSSP( MFRONT_SET_OUTOFBOUNDS_POLICY, mfront_set_outofbounds_policy, char *nomlib,
              STRING_SIZE lnomlib, char *nomsub, STRING_SIZE lnomsub, char *nommod,
              STRING_SIZE lnommod, ASTERINTEGER *value ) {
#ifdef ASTER_PLATFORM_POSIX
    ...
    f_mfront =
        (FUNC_MFRONT_SET_OUTOFBOUNDS_POLICY())libsymb_get_symbol( DLL_DICT, libname, symbname );
...
void DEFSSSPP( MFRONT_GET_NBVARI, mfront_get_nbvari, char *nomlib, STRING_SIZE lnomlib,
               char *nomsub, STRING_SIZE lnomsub, char *nommod, STRING_SIZE lnommod,
...
void DEFPPPPPPPPPPPPPPPPPP( MFRONT_BEHAVIOUR, mfront_behaviour, ASTERINTEGER *pfcmfr,
                            ASTERDOUBLE *stress, ASTERDOUBLE *statev, ASTERDOUBLE *ddsdde,
                            ASTERDOUBLE *stran, ASTERDOUBLE *dstran, ASTERDOUBLE *dtime,
                            ASTERDOUBLE *temp, ASTERDOUBLE *dtemp, ASTERDOUBLE *predef,
                            ASTERDOUBLE *dpred, ASTERINTEGER *ntens, ASTERINTEGER *nstatv,
                            ASTERDOUBLE *props, ASTERINTEGER *nprops, ASTERDOUBLE *drot,
                            ASTERDOUBLE *pnewdt, ASTERINTEGER *nummod ) {
#ifdef ASTER_PLATFORM_POSIX
    ...
    CALLMFRONTBEHAVIOUR( *f_mfront, stress, statev, ddsdde, stran, dstran, dtime, temp, dtemp,
 ...
 */
void DEFSPS( MFRONT_GET_MATER_PROP, mfront_get_mater_prop, _IN char *rela, STRING_SIZE lrela,
             _OUT ASTERINTEGER *nbval, _OUT char *txval, STRING_SIZE ltx ) {
#ifdef ASTER_HAVE_MFRONT
    /* MFRONT Wrapper
     */
    char *crela;
...
int load_mfront_lib( const char *libname, const char *symbol ) {
    ...
    *(void **)( &f_mfront ) = dlsym( mfront_handle, symbol );
    ...
    if ( libsymb_register( DLL_DICT, libname, symbol, mfront_handle, (FUNC_PTR)f_mfront ) ) {
