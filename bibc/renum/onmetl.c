...
int ComputeFillInL( graph_t *graph, idx_t *iperm, int *parent, int *supnd, int *neq, int *nbsn,
     ...
     */
    int errs;
    errs = smbfctl( nvtxs, xadj, adjncy, perm, iperm, xlnz, maxlnz, xnzsub, nzsub, &maxsub, supnd,
                    parent, nbsn, &lgind, opc );
    if ( errs ) {
        int errs2;
        free( nzsub );
...
*
*******************************************************************/
int smbfctl( int neqns, idx_t *xadj, idx_t *adjncy, idx_t *perm, idx_t *invp, idx_t *xlnz,
             int *maxlnz, idx_t *xnzsub, idx_t *nzsub, int *maxsub, int *supnd, int *parsnode,
             int *nbsn, int *lgin, double *opc ) {
    /* Local variables */
    ...
    while ( i < neqns + 1 ) {
        ...
        if ( nbip1 != ( nbi - 1 ) ) {
            ...
            supnd[snode] = i;
            lgind += nbip1;
        } else { /* verification que dans le  sn courant (snode) le noeud i est bien voisin du nd
                i-1 sinon on crée un nouveau SN correction des fiches 12345 et 12503  */
