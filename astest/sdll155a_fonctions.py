...
class PostRocheAnalytic(object):
    ...
    _Z = np.array([2.32973E-09, 2.32973E-09, 4.60194E-10, 4.60194E-10]) / np.array([0.0075, 0.0075, 0.005, 0.005])
    ...
    _D_23 = _D_22
    def __init__(self, mfy_poids_propre, mt_deplacement_impose, mfy_sismique_dyn, mfz_sismique_dyn, mfy_sismique_qs, mfz_sismique_qs,  mfy_sismique, mfz_sismique, pression = 0,):
        ep = 0.005
        rayon=0.01
        ep2 = 0.0025
        rayon2 = 0.005
        self._sigpres = 0.87*pression*np.array([(rayon-ep)/ep,(rayon-ep)/ep,(rayon2-ep2)/ep2,(rayon2-ep2)/ep2])
    ...
    def calcul_ressort(self):
        ...
        if (sum(self._sigma_deplacement_ref[self._t!=0]**2)==0):
            ...
            self._T = sum(self._A[self._t!=0]*self._sigma_deplacement_ref[self._t!=0]**2) / sum(self._A[self._t!=0]*self._sigma_deplacement_ref[self._t!=0]**2/self._t[self._t!=0])
        if sum(self._sigma_sismique_ref[self._t_s!=0]**2)==0:
            self._T_s = 0.
        else:
            self._T_s = sum(self._A[self._t_s!=0]*self._sigma_sismique_ref[self._t_s!=0]**2) / sum(self._A[self._t_s!=0]*self._sigma_sismique_ref[self._t_s!=0]**2/self._t_s[self._t_s!=0])
        # ressorts
    ...
    def calcul_abattement(self):
        ...
        # abbatements g
        self._g = []
        for sigma_ref, sigma_vrai, sigma_pres in zip(self._sigma_deplacement_ref, self._sigma_deplacement, self._sigpres):
        ...
        # abbatements gopt        
        self._gopt = []
        for sigma_ref, sigma_vrai, sigma_pres in zip(self._sigma_deplacement_ref, self._sigma_deplacement_opt, self._sigpres):
            if sigma_vrai < sigma_pres or sigma_vrai ==0:
                g = 1.
            else:
                g = (sigma_vrai-sigma_pres) / (sigma_ref-sigma_pres)
        ...
        # abbatements gs        
        self._g_s = []
        for sigma_ref, sigma_vrai, sigma_pres in zip(self._sigma_sismique_ref, self._sigma_sismique, self._sigpres):
            if sigma_vrai < sigma_pres or sigma_vrai ==0:
                g = 1.
            else:
                g = (sigma_vrai-sigma_pres) / (sigma_ref-sigma_pres)
        ...
        # abbatements gsopt       
        self._g_sopt = []
        for sigma_ref, sigma_vrai, sigma_pres in zip(self._sigma_sismique_ref, self._sigma_sismique_opt, self._sigpres):
            if sigma_vrai < sigma_pres or sigma_vrai ==0:
                g = 1.
            else:
                g = (sigma_vrai-sigma_pres) / (sigma_ref-sigma_pres)
    ...
    def calcul_sigma_eq(self):
        ...
        self._sigma_eq_opt = 1/self._Z * (self._D_21**2*(self._gopt*self._mt_deplacement_impose)**2 + \
                      self._D_22**2*(abs(self._mfy_poids_propre)+abs(self._mfy_sismique_qs)+self._g_sopt*abs(self._mfy_sismique_dyn))**2 \
                + self._D_23**2*(abs(self._mfz_sismique_qs)+self._g_sopt*abs(self._mfz_sismique_dyn))**2  )**0.5
