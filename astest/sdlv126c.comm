...
# viscoélastique 3D isotrope transverse.
...
E1 = G1*(2*(1 + NU1))
materiau_1 = DEFI_MATERIAU(ELAS_ISTR=_F(E_L=E1, E_N=E1, G_LN=G1,
                                        NU_LT=NU1, NU_LN=NU1))
...
E2 = G2*(2*(1 + NU2))
materiau_2 = DEFI_MATERIAU(ELAS_ISTR=_F(E_L=E2, E_N=E2, G_LN=G2,
                                        NU_LT=NU2, NU_LN=NU2))
...
NU = (3*K - 2*G)/(2*(3*K + G))
E = 2*G*(1+NU)
materiau_v = DEFI_MATERIAU(ELAS_VISCO_ISTR=_F(E_L=E, E_N=E, G_LN=G,
                                              NU_LT=NU, NU_LN=NU))
