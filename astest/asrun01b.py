...
class TestUtils(unittest.TestCase):
    ...
    def test_timer(self):
        ...
        timer.stop()
        time.sleep(0.1)
        self.assertEqual(len(timer._started), 2)
        report = timer.report()
        timer.save("test_timer.asrun01b")
        self.assertEqual(len(timer._started), 0)
        ...
        second = Timer("Global")
        second.load("test_timer.asrun01b")
        second.start("key1")
        time.sleep(0.1)
        second.stop()
        self.assertGreaterEqual(second._measures["key1"].elapsed, 0.29)
