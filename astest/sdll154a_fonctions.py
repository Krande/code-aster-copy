...
class PostRocheAnalytic(object):
    ...
    _D_23 = _D_22
    def __init__(self, mfy_poids_propre, mt_deplacement_impose, mfy_sismique_dyn, mfz_sismique_dyn, mfy_sismique_qs, mfz_sismique_qs,  mfy_sismique, mfz_sismique, pression = 0, coude=False, RCCM=False):
        ep = 0.005
        rayon=0.01
        ...
        if coude:
            ...
            self._B_2 =max(1.0, 1.3/ff**(2./3))
            self._D_22 = max(1.07*pow(np.pi/angle_rad,-0.4)*pow(ff,-2/3)/(1+0.142*pression*(rayon-ep)/ep/rp02min/pow(ff,1.45)),1.02)
            self._D_23 = max(0.809*pow(ff,-0.44)/(1+0.142*pression*(rayon-ep)/ep/rp02min/pow(ff,1.45)),1.02)
    ...
    def calcul_sigma_eq(self):
        ...
        self._sigma_eq = (self._sigpres2**2 + 1/self._Z**2*(self._D_21**2*(self._g*self._mt_deplacement_impose)**2 + \
        ...
        self._sigma_eq_opt = (self._sigpres2**2 + 1/self._Z**2 * (self._D_21**2*(self._gopt*self._mt_deplacement_impose)**2 + \
