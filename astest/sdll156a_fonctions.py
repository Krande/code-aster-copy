...
class PostRocheAnalytic(object):
    ...
    def __init__(self, mfy_poids_propre, mt_deplacement_impose, mfy_sismique_dyn, mfz_sismique_dyn, mfy_sismique_qs, mfz_sismique_qs,\
                        mfy_dilatation_thermique, mt_sismique_dyn, mt_sismique_qs, mt_sismique, mfy_sismique, mfz_sismique, pression=0.):
    ...
    def calcul_ressort(self):
        ...
        if np.count_nonzero(self._t_s) == 0:
            ...
            self._T_s = np.array([T_s_1]*4+[T_s_2]*2)
            # ressorts
            self._r_s = np.array([T/t-1 if t!=0. else 0. for t, T in zip(self._t_s, self._T_s)])
    ...
    def calcul_sigma_eq(self):
        ...
        self._sigma_eq_opt = 1/self._Z * (self._D_21**2*(abs(self._mt_sismique_qs)+self._gopt*abs(self._mt_deplacement_impose)+self._g_sopt*abs(self._mt_sismique_dyn))**2 + \
                      self._D_22**2*(abs(self._mfy_poids_propre)+abs(self._mfy_sismique_qs)+self._gopt*abs(self._mfy_dilatation_thermique)+self._g_sopt*abs(self._mfy_sismique_dyn))**2 + \
