...
def test_init_ops(self, TINI, TFIN, TMIL, MATER, LIST_INST, **args):
    ...
    __coef1b = DEFI_FONCTION(
        NOM_PARA="INST", VALE=(0, 0, 10, -1.0e4, 25, 1.0e4, 40, -1.0e4), PROL_DROITE="CONSTANT"
    )

    __coef2b = DEFI_FONCTION(NOM_PARA="INST", VALE=(0, -5.0e4, 50, -5.0e4), PROL_DROITE="CONSTANT")
    ...
    __u2 = SIMU_POINT_MAT(
        ...
        MATER=MATER,
        INCREMENT=_F(LIST_INST=LIST_INST, INST_INIT=TINI, INST_FIN=TMIL),
        NEWTON=_F(MATRICE="TANGENTE", REAC_ITER=1),
        CONVERGENCE=_F(RESI_GLOB_RELA=1.0e-6, ITER_GLOB_MAXI=10),
        ARCHIVAGE=_F(LIST_INST=LIST_INST),
        SIGM_IMPOSE=_F(SIXX=__coef2b, SIYY=__coef2b, SIZZ=__coef2b, SIXY=__coef1b),
        SIGM_INIT=_F(SIXX=-5.0e4, SIYY=-5.0e4, SIZZ=-5.0e4),
    ...
    SPM = SIMU_POINT_MAT(
        ...
        MATER=MATER,
        INCREMENT=_F(LIST_INST=LIST_INST, INST_INIT=TMIL, INST_FIN=TFIN),
        NEWTON=_F(MATRICE="TANGENTE", REAC_ITER=1),
        CONVERGENCE=_F(RESI_GLOB_RELA=1.0e-6, ITER_GLOB_MAXI=10),
        ARCHIVAGE=_F(LIST_INST=LIST_INST),
        SIGM_IMPOSE=_F(SIXX=__coef2b, SIYY=__coef2b, SIZZ=__coef2b, SIXY=__coef1b),
        ...
        SIGM_INIT=_F(
            SIXX=SIXX_INI, SIYY=SIYY_INI, SIZZ=SIZZ_INI, SIXY=SIXY_INI, SIYZ=SIYZ_INI, SIXZ=SIXZ_INI
        ),
        EPSI_INIT=_F(
            EPXX=EPXX_INI, EPYY=EPYY_INI, EPZZ=EPZZ_INI, EPXY=EPXY_INI, EPXZ=EPXZ_INI, EPYZ=EPYZ_INI
        ),
    )
    return SPM
