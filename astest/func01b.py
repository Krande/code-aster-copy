...
tps = DEFI_LIST_REEL(DEBUT=0.0, INTERVALLE=_F(JUSQU_A=1.0, NOMBRE=10))
test.assertEqual(tps.size, 11)
test.assertAlmostEqual(tps.max(), 1.0)


tps = DEFI_LIST_REEL(DEBUT=0.0, INTERVALLE=_F(JUSQU_A=10.0, PAS=0.7))
test.assertEqual(tps.size, 16)
test.assertAlmostEqual(tps.max(), 10.0)

tps = DEFI_LIST_REEL(DEBUT=0.0, INTERVALLE=_F(JUSQU_A=10.0, PAS=0.9999))
test.assertEqual(tps.size, 12)
test.assertAlmostEqual(tps.max(), 10.0)

# check last step
tps = DEFI_LIST_REEL(DEBUT=0.0, INTERVALLE=_F(JUSQU_A=10.0, PAS=0.999999))
test.assertEqual(tps.size, 11)
test.assertAlmostEqual(tps.max(), 10.0)

tps = DEFI_LIST_REEL(DEBUT=0.0, INTERVALLE=_F(JUSQU_A=10.0, PAS=1.01))
test.assertEqual(tps.size, 11)
test.assertAlmostEqual(tps.max(), 10.0)

tps = DEFI_LIST_REEL(DEBUT=0.0, INTERVALLE=_F(JUSQU_A=10.0, PAS=1.000001))
test.assertEqual(tps.size, 11)
test.assertAlmostEqual(tps.max(), 10.0)
...
fsin = DEFI_FONCTION(
    NOM_PARA="INST", NOM_RESU="TEMP", ABSCISSE=valx, ORDONNEE=valy, INTERPOL=("LIN", "LIN")
)
...
fsin = DEFI_FONCTION(
    NOM_PARA="INST", NOM_RESU="TEMP", VALE_PARA=lvalx, VALE_FONC=lvaly, INTERPOL=("LIN", "LIN")
)
...
with test.assertRaisesRegex(code_aster.AsterError, "Unexpected type.*str"):
    ...
    bad_type[5] = "a"
    fsin = DEFI_FONCTION(
        NOM_PARA="INST", NOM_RESU="TEMP", ABSCISSE=valx, ORDONNEE=bad_type, INTERPOL=("LIN", "LOG")
    )
