...
test.assertEqual(len(ccid), numeDDL.getNumberOfDOF(local=True) + 1)

vec = code_aster.FieldOnNodesReal(model)
vec.setValues(1.0)
study.zeroDirichletBCDOFs(vec)
test.assertEqual(sum(vec.getValues()), numeDDL.getNumberOfDOF(local=True) - sum(ccid[:-1]))
...
test.assertEqual(numeDDL.getNumberOfDOF(local=True), 3 * len(pMesh.getNodes(localNumbering=True)))
test.assertEqual(numeDDL.getNumberOfDOF(local=False), 3993)
