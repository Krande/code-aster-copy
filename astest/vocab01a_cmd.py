...
class CataChecker:
    ...
    def check_reentrant(self, step):
        ...
        if status not in ("o", "n", "f"):
            self.cr.fatal(
                "Unexpected value for 'reentrant' (not 'o', 'n' or 'f'): %r", status
            )
    ...
    def check_statut(self, step, into=("o", "f", "c", "d")):
        ...
        if step.definition.get("statut") not in into:
            self.cr.fatal(
                "Unexpected value for 'statut' (not in %s): %r",
                into,
                step.definition.get("statut"),
            )
    ...
    def check_op(self, step, valmin=-99, valmax=9999):
        ...
        if (
            ...
            self.cr.fatal(
                "Attribute 'op' must be between %d and %d: %r", valmin, valmax, op
            )
...
def checkDefinition(test, commands):
    ...
    for name, msg in err:
        print(
            ("La vérification du catalogue a échoué pour '{0}':\n{1}".format(name, msg))
        )
    if err:
        raise TypeError(
            "{0} erreurs sur {1} commandes.".format(len(err), len(commands))
        )
...
def checkDocStrings(test, commands):
    ...
    test.assertEqual(
        len(lang),
        0,
        msg="'ang' is deprecated and not used anymore, " "please remove it",
    )
...
def check_material_def(test):
    ...
    test.assertEqual(
        len(atleastone), 1, msg="expect exactly one AtLeastOne/AU_MOINS_UN rule"
    )
    ...
    test.assertTrue(
        not unknown,
        msg="unknown keyword in the AU_MOINS_UN rule: " + ", ".join(unknown),
    )
    test.assertTrue(
        not missing,
...
def vocab01_ops(self, EXISTANT, INFO, **kwargs):
    ...
    test.assertEqual(
        nbWords, nbExist, msg="nombre de mots-clefs (catalogue vs vocab01a.34)"
    )
