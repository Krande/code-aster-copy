...
MODELE = AFFE_MODELE(AFFE=(_F(GROUP_MA=('s1Tuyau', 'Tuyau'),
                              MODELISATION='3D',
                              PHENOMENE='MECANIQUE'),
                           _F(FORMULATION='U_P',
                               GROUP_MA=('s1Eau', 'Eau'),
...
CHMAT = AFFE_MATERIAU(AFFE=(_F(GROUP_MA=('Tuyau'),
                               MATER=(acier, )),
...
BLOQ = AFFE_CHAR_MECA(FACE_IMPO=(_F(DX=0.0,
                                    DY=0.0,
                                    DZ=0.0,
                                    GROUP_MA=('s1Tuyau', )),
                                 _F(GROUP_MA=('s1Eau',),
...
PESA = AFFE_CHAR_MECA(PESANTEUR=_F(GRAVITE=9.81,
                                   DIRECTION=(0., 0., -1.,),),
                      MODELE=MODELE)
