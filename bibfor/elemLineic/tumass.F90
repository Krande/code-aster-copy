...
! ------------- Compute mass matrix
                ...
                do iDof = 1, nbDof
                    do jDof = 1, iDof
                        mass(iDof, jDof) = mass(iDof, jDof)+poids*massPg(iDof, jDof)
...
! - Convert matrix to storage
    nc = nbDof*(nbDof+1)/2
    call mavec(mass, nbDof, zr(jvMatr), nc)
!
end subroutine
