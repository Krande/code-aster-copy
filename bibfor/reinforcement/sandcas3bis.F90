...
subroutine sandcas3bis(effrts, ht, enrobi, enrobs, facier, fbeton, gammas, gammac, &
...
!      CAS 3BIS - COMPRESSIVE REINF NEEDED in SUP + REGULAR REINF NEEDED in INF
!
...
!Variables de calcul
    real(kind=8) :: pi, fcd, fyd, ySUP, yINF, alpha_SUP, Z
    ...
    real(kind=8) :: a11, a12, a21, a22, a00, b00, c00, Delta, X_alpha_sup, t1, t2
    logical :: cond_t1, cond_t2
    real(kind=8) :: Ds(4, 4), SOL(4), det, ci, cs, si, ss
    ...
    do i = 1, 13
        write (p(i), fmt='(A18,I2)') 'POINT_ITER_SANDCAS3BIS_', i
    end do
...
!-------------------------------------------------------------------------------------------------
    ...
    if ((abs(Nxy) .le. epsilon(Nxy)) .and. (abs(Mxy) .le. epsilon(Nxy))) then
        ...
        !alpha_INF varie de 0 à 1 (avec un pas de aphiter) (m=1,N3)

        N1 = 2
        N2 = ceiling(0.5/epiter)+1
        N3 = ceiling(1/aphiter)+1

        N_IIbis = N1*N1*N2*N2*N3
                ...
                do k = 1, N2
                    ...
                    do l = 1, N2
                        ...
                        do m = 1, N3
                            ...
                            !(i,j,k,l,m) ==> indx
                            indx = i+N1*(j-1)+N1*N1*(k-1)+N1*N1*N2*(l-1)+N1*N1*N2*N2*(m-1)
...
99                          continue
        ...
        !tINF varie de 0 à h/2 (avec un pas de h*epiter) (k=1,N2)
        ...
        call wkvect(p(1), ' V V R ', N1, vr=AngleINF)
        call wkvect(p(2), ' V V R ', N1, vr=AngleSUP)
        call wkvect(p(3), ' V V R ', N2, vr=tSUP)

        call wkvect(p(4), ' V V R ', N_IIbis, vr=tINF)
            ...
            do j = 1, N1
                ...
                do k = 1, N2
                    ...
                    elseif (k .eq. N2) then
                        ...
                        tSUP(k) = tINF(k-1)+epiter*ht
                    end if
                    t_sup = tSUP(k)

                    indx = i+N1*(j-1)+N1*N1*(k-1)

                    !Solve 1st equation
                    a11 = -fcINF*cos(theta_inf)*sin(theta_inf)
                    a21 = 0.5*a11
                    a12 = -fcd*t_sup*cos(theta_sup)*sin(theta_sup)
                    a22 = 0.5*(ht-t_sup)*a12

                    if (a12 .le. epsilon(a12)) then
                        tINF(indx) = -1.d0
                    else
                        a00 = a21
                        b00 = -(a21*ht+a11*a22/a12)
                        c00 = -(-(a22/a12)*Nxy+Mxy)
                        ...
                        elseif (Delta .ge. 0) then
                            ...
                            cond_t1 = .false.
                            cond_t2 = .false.
                            if ((t1 .ge. 0) .and. (t1 .le. (0.5*ht))) then
                                cond_t1 = .true.
                            elseif ((t2 .ge. 0) .and. (t2 .le. (0.5*ht))) then
                                cond_t2 = .true.
                            end if
                            if ((cond_t1 .eqv. (.true.)) .and. (cond_t2 .eqv. (.true.))) then
                                tINF(indx) = min(t1, t2)
                    ...
                    if (tINF(indx) .ne. (-1.d0)) then
                        X_alpha_sup = (Nxy-a11*tINF(indx))/a12
                        if (cond109 .eq. 1) then
                            a00 = X_alpha_sup+0.85*3.8
                            b00 = 2*X_alpha_sup-0.85*2.8
