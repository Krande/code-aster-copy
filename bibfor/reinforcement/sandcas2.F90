...
subroutine sandcas2(effrts, ht, enrobi, enrobs, facier, fbeton, gammas, gammac, &
...
!      CAS 2 - FERRAILLAGE [+] REQUIS EN SUP
!
...
!-------------------------------------------------------------------------------------------------
    ...
    do i = 1, 15
        write (p(i), fmt='(A9,I2)') 'SANDCAS2_', i
    end do
...
!!EN EFFET, DANS LE solver_sandcas1, AngleINF a N_INF
!!COMME DIMENSION , ALORS QU'ICI, c'est N_SUP!!!
!!==> RESOLVE with variable MONO
    call wkvect(p(13), ' V V R ', N_SUP, vr=AngleINF)
    ...
    do i = 1, N_SUP
        ...
        !Calc pour tINFA
        tINFA = 0
        call solver_sandcas2(1, yINF, ySUP, ht, effrts, fcd, fcd1, cond109, &
                             AngleSUP(i), tINFA, &
                             tSUP(i), AngleINF(i), ResiduA, &
                             nSX_INF(i), nSY_INF(i), &
                             nSX_SUP(i), nSY_SUP(i), &
                             nS_TOT(i), &
        ...
        do while (condmed .eqv. (.false.))
            tINFmed = 0.5*(tINFA+tINFB)
            call solver_sandcas2(1, yINF, ySUP, ht, effrts, fcd, fcd1, cond109, &
                                 AngleSUP(i), tINFmed, &
                                 tSUP(i), AngleINF(i), Residumed, &
                                 nSX_INF(i), nSY_INF(i), &
                                 nSX_SUP(i), nSY_SUP(i), &
                                 nS_TOT(i), &
                                 ncMAX_INF(i), ncMIN_INF(i), &
            ...
            if ((RESIDUA .ne. (-1.d0)) .and. (RESIDUB .ne. (-1.d0))) then
                if (RESIDUA*RESIDUB .gt. 0) then
                    if (abs(RESIDUA) .le. abs(RESIDUB)) then
                        ...
                        tINFA = tINFB
                        RESIDUA = RESIDUB
                        tINFB = min(tINFB+tINFB-tINFmed, 0.5/ht)
                        !Calc NEW tINFB
                        call solver_sandcas2(1, yINF, ySUP, ht, effrts, fcd, fcd1, cond109, &
                                             AngleSUP(i), tINFB, &
                                             tSUP(i), AngleINF(i), ResiduB, &
