...
! --------------------------------------------------------------------
!
subroutine def3d(ppas, tdef, nrjd, tref0, def0,&
                 sr1, srsdef, teta1, dt, vdef00,&
                 def1, vdef1, CNa, nrjp, ttrd,&
                 tfid, ttdd, tdid, exmd, exnd,&
                 cnab, cnak, ssad, At, St,&
                 M1, E1, M2, E2, AtF,&
                 StF, M1F, E1F, M2F, E2F,&
                 ttkf, nrjf)
...
!     temperature seuil de destabilisation fonction de la concentration en Na
    CNamin1=cnak
!      tempd=ZEROKLV+ttdd/max(CNa,CNamin1)**exnd
    tempd=ZEROKLV+ttdd*((cnak/CNa)**exnd)
!      print*,'CNa,CNamin1,tempd'
!      print*,CNa,CNamin1,tempd-ZEROKLV
!
...
!      influence de la temperature
        coeff1=(exp(-(nrjd/KGAZP)*(1.d0/temp1-1.d0/tempd)))-1.d0
!       print*,'def3d,dissolution coeff1',coeff1
!      influence de la temperature sur la fixation des alu
        ctf=max((exp(-(nrjf/KGAZP)*(1.d0/temp1-1.d0/tempf)))-1.d0,0.001)
        if (coeff1 .gt. 0.) then
...
!       influence de le temperature
        coeff1=((1.d0-exp(-(nrjd/KGAZP)*(1.d0/temp1-1.d0/tempd)))/&
        (1.d0-exp(-(nrjd/KGAZP)*(1.d0/tempr-1.d0/tempd))))*&
        exp(-(nrjp/KGAZP)*(1.d0/temp1-1.d0/tempr))
