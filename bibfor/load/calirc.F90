...
!
    ...
    integer :: nbtyp, nddl2, nbma2, jlistk, jj
    integer :: jnorm, ij, ima1, jlisv1
    integer :: kno2, kkno2, jcoor, n1
!
    aster_logical   :: lrota, dnor, l_error, l_tran, l_cent, l_angl_naut, l_dmax, l_resout
!
    real(kind=8)    :: beta, coef1, mrota(3, 3), normal(3)
    real(kind=8)    :: tran(3)
...
!
    ...
    character(len=8)    :: lfonc(3), lparx(3), maili
    character(len=16)   :: concept, nomcmd, formar
...
!   IDMAX : NOMBRE MAX DE TERMES D'UNE RELATION LINEAIRE = 2*27 + 3 = 57
    idmax=57
    ...
    AS_ALLOCATE(vi=dimens,  size=idmax)
!
...
!
        geom1 = ' '
        geom2 = ' '
!       Il faut remettre à zéro ces 2 objets entre 2 occurences
        dimens(1:idmax)   = 0
        direct(1:3*idmax) = 0.d0
!
        dnor=.false.
        typrac=' '
...
!
!           CREATION :
!               DU TABLEAU DES NUMEROS DES NOEUDS '&&NBNLMA.LN'
!               DES NOMBRES D'OCCURENCES DE CES NOEUDS '&&NBNLMA.NBN'
...
!
!           TABLEAU D'INDIRECTION ENTRE LES INDICES DU TABLEAU DES NORMALES ET LES NOEUDS
            AS_ALLOCATE(vi=vindire, size=nnomx)
            call jelira('&&NBNLMA.LN', 'LONUTI', lno)
!
            do ii = 1, lno
                vindire(1+ln(ii)-1)=ii
            enddo
!
...
!
!
!       1.3 ON ELIMINE DE LINONU2 LES NOEUDS DEJA ELIMINES LORS DES OCCURENCES
...
!          - geom2 est détruit à la fin !!! Ne doit pas pointer vers le maillage initial
!       Read transformation
...
!       Seulement si COQUE
        if (typrac.eq.'COQUE') then
            ...
            if ( l_resout ) then
                maili='&&CALIRC'
                call jedetr(maili)
                call copisd('MAILLAGE', 'V', mesh, maili)
                call jeveuo(maili//'.COORDO    .VALE', 'E', jgeomi)
            endif

            call getvid(motfac, 'TRANSF_GEOM_MAIT', iocc=iocc, nbval=3, vect=lfonc, nbret=nfonc)
            if ( nfonc .ne. 0) then
                ASSERT( nfonc.eq.3 )
!               Récupération de la liste des noeuds des mailles maitres
                motcle(1)='MAILLE_MAIT';   tymocl(1)='MAILLE'
...
!               Modification des coordonnées des noeuds
                ...
                call dismoi('NB_NO_MAILLA', mesh, 'MAILLAGE', repi=nnomx)
                call wkvect(geom1, 'V V R', 3*nnomx, jgdefo)
...
!               Paramètres des fonctions
                ...
                do ii=1,nbno1
                    ...
                    do ifonc=1,3
                        ...
                        ASSERT(ier.eq.0)
                    enddo
                    zr(jgdefo-1+3*(nnode-1)+1)=vx(1)
                    zr(jgdefo-1+3*(nnode-1)+2)=vx(2)
                    zr(jgdefo-1+3*(nnode-1)+3)=vx(3)
!
                    if ( l_resout ) then
                        zr(jgeomi-1+3*(nnode-1)+1)=vx(1)
                        zr(jgeomi-1+3*(nnode-1)+2)=vx(2)
                        zr(jgeomi-1+3*(nnode-1)+3)=vx(3)
                    endif
                enddo
            endif
!
            call getvid(motfac, 'TRANSF_GEOM_ESCL', iocc=iocc, nbval=3, vect=lfonc, nbret=nfonc)
            if ( nfonc .ne. 0) then
                ASSERT( nfonc.eq.3 )
!               Modification des coordonnées des noeuds
...
!                       le nom de la commande          : nomcmd
                ...
                ii = lxlgut(resout)
                write (kiocc,'(A1,I3.3)') '_',iocc
                fichier='REPE_OUT/'//resout(1:ii)//kiocc//'_transf_geom.med'
!               unite libre
                unite = ulnume()
                if (unite.le.0) call utmess('F', 'UTILITAI5_10')
!               On ouvre et écriture
                call ulaffe(unite, fichier, ' ', 'N', 'O')
                call irmail('MED', unite, 0, maili, ASTER_FALSE , m8blan, 1, formar)
!               On ferme
                call ulopen(-unite, m8blan, m8blan, m8blan, m8blan)
                call jedetr(maili)
            endif
        endif
!
...
!                   IMA1: MAILLE A CONNECTER A INO2
                    ima1=com1(ino2)
!                   SI IMA1=0, C'EST QUE INO2 NE FAIT PAS PARTIE DES NOEUDS ESCLAVES
...
!                       SI LA RELATION EST UNE TAUTOLOGIE, ON NE L'ECRIT PAS :
                        if (nuno1 .eq. nuno2) then
                            if (abs(coef(ino1+1)-1.0d0) .lt. 1.0d-02) then
...
!                   ----------------------------------------------------
                    if (dnor) then
                        do ino1 = 1, nno1+1
                            ...
                            nomddl(ino1)='DEPL'
                            do ii = 1, ndim
                                direct(1+(ino1-1)*ndim1+ii-1)=zr(jnorm+(vindire(ino2)-1)*ndim+ii-1)
                            enddo
                        enddo
...
!                   NNO1: NB DE NOEUD_MAIT LIES A INO2
                    ...
                    nomnoe(1:idmax)   = m8blan
                    nomnoe(1:idmax)   = m8blan
                    nomddl(1:idmax)   = m8blan
                    coef(1:idmax)     = 0.0d0
                    dimens(1:idmax)   = 0.0d0
                    direct(1:3*idmax) = 0.0d0
!
                    normal(1:3) = 0.0d0
!
                    nuno2=ino2
...
!                   ----------------------------------------------------
                    if (dnor) then
                        do ii = 1, ndim
                            do jj = 1, ndim
                                normal(ii) = normal(ii) + &
                                             mrota(jj,ii)*zr(jnorm+(vindire(ino2)-1)*ndim+jj-1)
                            enddo
                        enddo
!
                        coef(1)   =1.0d0
                        ...
                        dimens(1)=ndim
                        do jj = 1, ndim
                            direct(jj)=zr(jnorm+(vindire(1+ino2-1)-1)*ndim+jj-1)
                        enddo
                        do ino1 = 2, nno1+1
                            dimens(ino1)=ndim
                            nomddl(ino1)='DEPL'
                            do ii = 1, ndim
                                direct(1+(ino1-1)*ndim1+ii-1)=-normal(ii)
                            enddo
                        enddo
                        call afrela(coef, [cbid], nomddl, nomnoe, dimens, &
                                    direct, nno1+1, beta, betac, kbeta, &
...
!
!   AFFECTATION DE LA LISTE DES RELATIONS A LA CHARGE
