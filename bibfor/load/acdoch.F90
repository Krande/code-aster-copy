...
!
        call lisccr('ACOU', list_load, nb_load, base)
!
...
!
            call load_list_getp('ACOU', l_load_user, v_llresu_info, v_llresu_name, &
...
!
            nb_info_type = 0
            info_type = 'RIEN'
            if (load_type(1:5) .eq. 'CIAC_') then
...
!
            do i_type_neum = 1, nb_type_neum
                ...
                if (list_load_keyw(i_type_neum)) then
                    ...
                    cart_name = load_name(1:8)//'.CHAC'//load_obje(1)
...
!
            if (nb_info_type .gt. 0) then
                call liscad('ACOU', list_load, i_load, load_name, load_func, &
