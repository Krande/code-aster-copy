...
!
    ...
    real(kind=8) :: node_real(3), ksi(3), ksi_ma(2)
    real(kind=8) :: norm(3)
    real(kind=8) :: dff(2, MT_NNOMAX2D)
    integer(kind=8) :: iret_
    real(kind=8) :: tria_coor_es(2, 3)
    integer(kind=8) :: i_tria, i_gauss, nb_tria, nb_gauss
    real(kind=8) :: gauss_weight_es(12), gauss_coor_es(2, 12)
    real(kind=8) :: gauss_coou(3), dire_norm(3), gauss_coot(3)
    real(kind=8) :: tau1(3), tau2(3), ksi1, ksi2
    integer(kind=8) :: tria_node(6, 3)
    character(len=8) :: gauss_family
    real(kind=8) :: shape_dfunc(2, MT_NNOMAX2D)
    aster_logical :: debug
