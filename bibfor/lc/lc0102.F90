...
!
! --------------------------------------------------------------------------------------------------
!   RELATION='ELAS': COMPORTEMENT ELASTIQUE INCREMENTAL
! --------------------------------------------------------------------------------------------------
...
! --------------------------------------------------------------------------------------------------
    integer(kind=8), parameter   :: nbel = 2
! --------------------------------------------------------------------------------------------------
    integer(kind=8)     :: iokel(nbel)
    real(kind=8)        :: valel(nbel)
    character(len=16)   :: nomel(nbel)
    integer(kind=8) :: ndimsi, k, l
    real(kind=8):: sig(BEHInteg%behavPara%ndimsi)
    real(kind=8):: dsde(BEHInteg%behavPara%ndimsi, BEHInteg%behavPara%ndimsi)
    real(kind=8):: vi(nvi), eps_t(BEHInteg%behavPara%ndimsi)
    real(kind=8):: kr(BEHInteg%behavPara%ndimsi)
    real(kind=8):: id(BEHInteg%behavPara%ndimsi, BEHInteg%behavPara%ndimsi)
    real(kind=8):: young, nu, lambda, deuxmu, young_m, nu_m
! --------------------------------------------------------------------------------------------------
    data nomel/'E', 'NU'/
! --------------------------------------------------------------------------------------------------
...
! --------------------------------------------------------------------------------------------------
    ...
    ! Strain offset
    eps_t = ((1+nu_m)*sigm(1:ndimsi)-nu_m*sum(sigm(1:3))*kr)/young_m+deps(1:ndimsi)

    ! Stress computation
    if (BEHInteg%behavPara%lSigm) then

        ! Correction de contraintes planes
        if (BEHInteg%behavPara%lPlaneStress) eps_t(3) = -nu*(eps_t(1)+eps_t(2))

        ! Relation hyperélastique
        sig = lambda*sum(eps_t(1:3))*kr+deuxmu*eps_t
    end if

    ! Tangent matrix
    if (BEHInteg%behavPara%lMatr) then

        dsde = lambda*proten(kr, kr)+deuxmu*id

        ! Correction de contraintes planes
        if (BEHInteg%behavPara%lPlaneStress) then
            ASSERT(ndimsi .eq. 4)
            do k = 1, 4
                if (k .eq. 3) cycle
                do l = 1, 4
                    if (l .eq. 3) cycle
                    dsde(k, l) = dsde(k, l)-dsde(k, 3)*dsde(3, l)/dsde(3, 3)
                end do
            end do
            dsde(:, 3) = 0
            dsde(3, :) = 0
