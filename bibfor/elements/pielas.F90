...
! - CALCUL SUIVANT COMPORTEMENT
    if (relaComp .eq. 'VMIS_ISOT_TRAC' .or. relaComp .eq. 'VMIS_ISOT_LINE') then
        ...
        call daxpy(b_n, 1.d0, epsm, b_incx, epsp, b_incy)
        if (etamin .eq. -r8gaem() .or. etamax .eq. r8gaem()) then
            call utmess('F', 'MECANONLINE_60', sk=relaComp)
        end if
        call pipeds(BEHInteg%materPara, ndim, typmod, &
                    tau, &
                    vim(1, kpg), &
    ...
    else if (relaComp .eq. 'ENDO_LOCA_EXP' .or. relaComp .eq. 'VMIS_ISOT_NL') then
        eps0 = epsm(1:ndimsi)+epsp(1:ndimsi)
        eps1 = epsd(1:ndimsi)
        if (relaComp .eq. 'ENDO_LOCA_EXP') then
            ELE_ldc = ELE_Init(ndimsi, option, 'NONE', &
        ...
        else if (relaComp .eq. 'VMIS_ISOT_NL') then
            VINL_ldc = VINL_Init(ndimsi, option, 'NONE', &
                                 BEHInteg%materPara%schemePara%kpg, &
                                 BEHInteg%materPara%schemePara%ksp, &
                                 BEHInteg%materPara%jvMaterCode, &
                                 100, 1.d-6)
...
!
!
    else if (relaComp .eq. 'ENDO_ORTH_BETON') then
        b_n = to_blas_int(ndimsi)
        b_incx = to_blas_int(1)
        b_incy = to_blas_int(1)
        call daxpy(b_n, 1.d0, epsm, b_incx, epsp, b_incy)
        ...
        call pipedo(BEHInteg%materPara, ndim, typmod, &
                    tau, &
                    vim(1, kpg), epsm, epsp, epsd, etamin, etamax, &
                    copilo(1, kpg), copilo(2, kpg), copilo(3, kpg), copilo(4, kpg), &
                    copilo(5, kpg))

    else if (relaComp .eq. 'BETON_DOUBLE_DP') then
        b_n = to_blas_int(ndimsi)
        b_incx = to_blas_int(1)
        b_incy = to_blas_int(1)
        call daxpy(b_n, 1.d0, epsm, b_incx, epsp, b_incy)
        call pipedp(BEHInteg, ndim, typmod, &
                    epsm, sigma, vim(1, kpg), epsp, &
                    epsd, copilo(1, kpg), copilo(2, kpg))
!
    else
        call utmess('F', 'PILOTAGE_88', sk=relaComp)
    end if
end subroutine
