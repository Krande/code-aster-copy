...
!
        ...
        aster_logical :: debug
!========================
        integer(kind=8), dimension(:), allocatable :: fields_size
        integer(kind=8) :: nb_cmp_global
        integer(kind=8) :: idx, nb_fields, start, end, ntot, i, length1, length2
...
! list of cmp
        call getvtx('SOLVEUR', 'NOM_CMP', iocc=1_8, nbval=0_8, nbret=nb_cmp_global)
        nb_cmp_global = -nb_cmp_global
        allocate (cmp_global(nb_cmp_global), stat=ierr)
        call getvtx('SOLVEUR', 'NOM_CMP', iocc=1_8, nbval=int(nb_cmp_global, 8), &
                    vect=cmp_global, nbret=jerr)
! number of cmp in each fieldsplit
        call getvis('SOLVEUR', 'PARTITION_CMP', iocc=1_8, nbval=0_8, nbret=nb_fields)
        nb_fields = -nb_fields
        allocate (fields_size(nb_fields), stat=ierr)
        call getvis('SOLVEUR', 'PARTITION_CMP', iocc=1_8, nbval=nb_fields, &
...
!
        ...
        integer(kind=8) :: dimgeo, dimgeo_b, jnequ, neqg, jnequl, nterm
        ...
        integer(kind=8) :: numno, icmp, ieq, ix
        integer(kind=8) :: nloc, ndprop
        integer(kind=8) :: il, iga_f, igp_f
        real(kind=8) :: val
...
! Local variables
        ...
        integer(kind=8) :: ieql, ieqg, nuno, neqg
        integer(kind=8) :: nucmp, neql, passe, jcmp
        integer(kind=8) :: ieqgp
        PetscInt, dimension(:), allocatable :: idl
        integer(kind=8), dimension(:), pointer :: deeq => null()
        integer(kind=8), dimension(:), pointer :: nulg => null()
        integer(kind=8), dimension(:), pointer :: nlgp => null()
        integer(kind=8), dimension(:), pointer :: nequl => null()
