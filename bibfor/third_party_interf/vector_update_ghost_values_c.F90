...
! --------------------------------------------------------------------
subroutine vector_update_ghost_values_c(vector, nume_equa, mode)
#include "asterf_types.h"
    implicit none
#include "asterc/asmpi_comm.h"
#include "asterc/asmpi_recv_c.h"
#include "asterc/asmpi_send_c.h"
#include "asterc/asmpi_sendrecv_c.h"
...
!
    complex(kind=8), intent(inout) :: vector(*)
...
!
            call wkvect('&&CPYSOL.TMP1E', 'V V C', max(1_8, lgenvo), jvaleue)
            call wkvect('&&CPYSOL.TMP1R', 'V V C', max(1_8, lgrecep), jvaleur)
            if (lbidir) then
                call wkvect('&&CPYSOL.TMP2E', 'V V C', max(1_8, lgrecep), jvaleue2)
                call wkvect('&&CPYSOL.TMP2R', 'V V C', max(1_8, lgenvo), jvaleur2)
            end if
            ...
            if (lbidir) then
                if (lgrecep > 0) then
                    do jaux = 0, lgrecep-1
                        ...
                        zc(jvaleue2+jaux) = vector(numloc)
                    end do
                end if
            end if
!
            ...
            call asmpi_sendrecv_c(zc(jvaleue), n4e, numpr4, tag4, &
                                  zc(jvaleur), n4r, numpr4, tag4, mpicou)
            if (lbidir) then
                call asmpi_sendrecv_c(zc(jvaleue2), n4r, numpr4, tag4, &
                                      zc(jvaleur2), n4e, numpr4, tag4, mpicou)
            end if
            ...
            if (.not. lbidir) then
                ...
                if (lgrecep > 0) then
                    do jaux = 0, lgrecep-1
                        numloc = zi(jjointr+jaux)
                        ASSERT(zi(jprddl+numloc-1) .eq. numpro)
                        vector(numloc) = vector(numloc)+zc(jvaleur+jaux)
                    end do
                end if
                if (lgenvo > 0) then
                    do jaux = 0, lgenvo-1
                        numloc = zi(jjointe+jaux)
                        vector(numloc) = vector(numloc)+zc(jvaleur2+jaux)
                    end do
                end if
            end if
        ...
        if (nb_comm > 0) then
            ...
            do ijoin = 1, nb_comm
                ...
                if (iret1 .ne. 0) then
                    ...
                    call wkvect('&&CRNUGL.NUM_DDL_GLOB_E1', 'V V C', nbnoee, jnujoie1)
                    if (lbidir) then
                        call wkvect('&&CRNUGL.NUM_DDL_GLOB_R2', 'V V C', nbnoee, jnujoir2)
                    end if
                    do jaux = 1, nbnoee
                        numnoe = -zi(jjoine+jaux-1)
                        nddll = zzprno(ili, numnoe, 1)
                        zc(jnujoie1+jaux-1) = vector(nddll)
                    end do
                ...
                if (iret2 .ne. 0) then
                    ...
                    call wkvect('&&CRNUGL.NUM_DDL_GLOB_R1', 'V V C', nbnoer, jnujoir1)
                    if (lbidir) then
                        call wkvect('&&CRNUGL.NUM_DDL_GLOB_E2', 'V V C', nbnoer, jnujoie2)
                        do jaux = 1, nbnoer
                            numnoe = -zi(jjoinr+jaux-1)
                            nddll = zzprno(ili, numnoe, 1)
                            zc(jnujoie2+jaux-1) = vector(nddll)
                        end do
                    end if
                ...
                if (rang .lt. numpro) then
                    if (iret1 .ne. 0) then
                        call asmpi_send_c(zc(jnujoie1), n4e, numpr4, tag4, mpicou)
                    end if
                    if (iret2 .ne. 0) then
                        call asmpi_recv_c(zc(jnujoir1), n4r, numpr4, tag4, mpicou)
                    end if
                ...
                if (lbidir) then
                    if (rang .lt. numpro) then
                        if (iret1 .ne. 0) then
                            call asmpi_recv_c(zc(jnujoir2), n4e, numpr4, tag4, mpicou)
                        end if
                        if (iret2 .ne. 0) then
                            call asmpi_send_c(zc(jnujoie2), n4r, numpr4, tag4, mpicou)
                        end if
                    else if (rang .gt. numpro) then
                        if (iret2 .ne. 0) then
                            call asmpi_send_c(zc(jnujoie2), n4r, numpr4, tag4, mpicou)
                ...
                if (iret2 .ne. 0) then
                    if (.not. lbidir) then
                        do jaux = 1, nbnoer
                            numnoe = -zi(jjoinr+jaux-1)
                            nddll = zzprno(ili, numnoe, 1)
                            vector(nddll) = zc(jnujoir1+jaux-1)
                        end do
                    else
                        do jaux = 1, nbnoer
                            numnoe = -zi(jjoinr+jaux-1)
                            nddll = zzprno(ili, numnoe, 1)
                            vector(nddll) = vector(nddll)+zc(jnujoir1+jaux-1)
                        end do
                    end if
                end if
                ...
                if (iret1 .ne. 0) then
                    if (lbidir) then
                        do jaux = 1, nbnoee
                            numnoe = -zi(jjoine+jaux-1)
                            nddll = zzprno(ili, numnoe, 1)
                            vector(nddll) = vector(nddll)+zc(jnujoir2+jaux-1)
                        end do
                    end if
                end if
