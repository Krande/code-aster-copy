...
!-----------------------------------------------------------------------
    ...
    if ((abs(effmy) .lt. seuil_moment) .and. (abs(effmz) .lt. seuil_moment)) then
        call cafelsqp(cequi, effmy, 0.5d0*effn, ht, bw, &
        ...
        call cafelsqp(cequi, effmz, 0.5d0*effn, bw, ht, &
    ...
    fyd = 0.5d0*(kvarfy+kvarfz)*facier
            ...
            if (BRES .gt. 1) then
                if (Ass .lt. epsilon(Ass)) then
                    ...
                    if (ferrsyme .eq. 1) then
                        rhoyi = 0.5d0*(dnsyi+dnsys)/Ass
                        rhoys = 0.5d0*(dnsyi+dnsys)/Ass
                        rhozi = 0.5d0*(dnszi+dnszs)/Ass
                        rhozs = 0.5d0*(dnszi+dnszs)/Ass
                    else
                        rhoyi = dnsyi/Ass
                        rhoys = dnsys/Ass
                        rhozi = dnszi/Ass
