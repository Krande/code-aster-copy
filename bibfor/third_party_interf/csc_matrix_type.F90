...
!
    subroutine create_csc_matrix(name, m, n, nnz, a)
        ! Dummy arguments
        character(len=*), intent(in)    :: name
        integer(kind=8), intent(in)             :: m, n, nnz
...
!
        type(csc_matrix), intent(in) :: a
!
        integer(kind=8) :: pos, ii, jj, nnz_col, row, col
!
...
! Local variables
        integer(kind=8) :: nnz11, nnz12
        integer(kind=8) :: ii, jj, nnz_col
        integer(kind=8) :: passe, pos, pos11, pos12
!
...
! Dummy arguments
        ...
        integer(kind=8), dimension(:), intent(in) :: irow, icol
        type(csc_matrix), intent(out) :: b_csc
        character(len=*), intent(in) :: b_name
! Local variables
        integer(kind=8) :: b_nrow, b_ncol, nnz
        integer(kind=8) :: a_icol, b_icol
        integer(kind=8) ::  pass
!
#ifdef ASTER_HAVE_PETSC
        PetscInt :: ii, jj
...
!  Dummy arguments
        type(csc_matrix), intent(inout) :: a
        integer(kind=8), intent(in) :: ja, jv, ldv
...
!   Dummy arguments
        type(csc_matrix), intent(inout) :: a_csc
!   Local variables
        integer(kind=8) :: nrow_max, icol, istart, iend, ierr
        integer(kind=8) :: nrow_cur
        real(kind=8), dimension(:), pointer :: values => null()
#ifdef ASTER_HAVE_PETSC
        PetscInt, dimension(:), pointer :: rowind => null()
...
#else
        integer(kind=4), dimension(:), pointer :: rowind => null()
        integer(kind=4), dimension(:), pointer :: pv => null()
