...
subroutine as_znaitr(ido, bmat, n, k, np, &
                  resid, rnorm, v, ldv, h, &
...
!
    if (msglvl .gt. 1) then
        call as_ivout(logfil, 1, [j], ndigit, '_NAITR: GENERATING ARNOLDI VECTOR NUMBER')
        call as_dvout(logfil, 1, [rnorm], ndigit, '_NAITR: B-NORM OF THE CURRENT RESIDUAL IS')
    end if
!
!        %---------------------------------------------------%
...
!
    if (msglvl .gt. 0) then
        call as_ivout(logfil, 1, [j], ndigit, '_NAITR: ****** RESTART AT STEP ******')
    end if
!
!           %---------------------------------------------%
...
!           %--------------------------------------%
!
    call as_zgetv0(ido, bmat, .false._1, n, j, &
...
!
    if (msglvl .gt. 2) then
        rtemp(1) = wnorm
        rtemp(2) = rnorm
        call as_dvout(logfil, 2, rtemp, ndigit, '_NAITR: RE-ORTHOGONALIZATION; WNORM AND RNORM ARE')
        call as_zvout(logfil, j, h(1, j), ndigit, '_NAITR: J-TH COLUMN OF H')
    end if
!
!        %----------------------------------------------------%
...
!
    if (msglvl .gt. 0 .and. iter .gt. 0) then
        call as_ivout(logfil, 1, [j], ndigit, '_NAITR: ITERATIVE REFINEMENT FOR ARNOLDI RESIDUAL')
        if (msglvl .gt. 2) then
            rtemp(1) = rnorm
            rtemp(2) = rnorm1
            call as_dvout(logfil, 2, rtemp, ndigit, &
...
!
        if (msglvl .gt. 2) then
            call as_zmout(logfil, k+np, k+np, h, ldh, &
