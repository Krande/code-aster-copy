...
!        ENDIF
!  END DO
subroutine as_dnaitr(ido, bmat, n, k, np, &
                  resid, rnorm, v, ldv, h, &
...
!           %--------------------------------------%
!
    call as_dgetv0(ido, bmat, itry, .false._1, n, &
                j, v, ldv, resid, rnorm, &
...
!
    if (msglvl .gt. 2) then
        xtemp(1) = wnorm
        xtemp(2) = rnorm
        call as_dvout(logfil, 2, xtemp, ndigit, '_NAITR: RE-ORTHONALIZATION, WNORM AND RNORM ARE')
        call as_dvout(logfil, j, h(1, j), ndigit, '_NAITR: J-TH COLUMN OF H')
    end if
!
!        %----------------------------------------------------%
...
!
    if (msglvl .gt. 0 .and. iter .gt. 0) then
        ...
        if (msglvl .gt. 2) then
            xtemp(1) = rnorm
            xtemp(2) = rnorm1
            call as_dvout(logfil, 2, xtemp, ndigit, &
...
!
        if (msglvl .gt. 2) then
            call as_dmout(logfil, k+np, k+np, h, ldh, &
