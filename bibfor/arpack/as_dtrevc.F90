...
! ==============================================================
subroutine as_dtrevc(side, howmny, select, n, t, &
                     ldt, vl, ldvl, vr, ldvr, &
                     mm, m, work, info)
!
...
!
                        call as_dlaln2(.false._1, 1, 1, smin, one, &
...
!
                        call as_dlaln2(.false._1, 2, 1, smin, one, &
...
!
                    ...
                    b_n = to_blas_int(ki)
                    b_incx = to_blas_int(1)
                    call dscal(b_n, remax, vr(1, is), b_incx)
!
                    do k = ki+1, n
                        vr(k, is) = zero
                    end do
...
!
                    ...
                    b_n = to_blas_int(n)
                    b_incx = to_blas_int(1)
                    call dscal(b_n, remax, vr(1, ki), b_incx)
                end if
!
            else
!
...
!                    1-BY-1 DIAGONAL BLOCK
!
                        call as_dlaln2(.false._1, 1, 2, smin, one, &
                                       t(j, j), ldt, one, one, work(j+n), &
                                       n, wr, wi, x, 2, &
                                       scale, xnorm, ierr)
!
...
!
                        if (scale .ne. one) then
                            ...
                            b_n = to_blas_int(ki)
                            b_incx = to_blas_int(1)
                            call dscal(b_n, scale, work(1+n2), b_incx)
                        end if
                        work(j+n) = x(1, 1)
                        work(j+n2) = x(1, 2)
!
...
!                    2-BY-2 DIAGONAL BLOCK
!
                        call as_dlaln2(.false._1, 2, 2, smin, one, &
                                       t(j1, j1), ldt, one, one, work(j-1+n), &
                                       n, wr, wi, x, 2, &
                                       scale, xnorm, ierr)
!
...
!
                    ...
                    b_n = to_blas_int(ki)
                    b_incx = to_blas_int(1)
                    call dscal(b_n, remax, vr(1, is-1), b_incx)
                    b_n = to_blas_int(ki)
                    b_incx = to_blas_int(1)
                    call dscal(b_n, remax, vr(1, is), b_incx)
!
...
!
                    ...
                    if (ki .gt. 2) then
                        ...
                        b_n = to_blas_int(n)
                        b_incx = to_blas_int(1)
                        call dscal(b_n, work(ki-1+n), vr(1, jnxt), b_incx)
                        b_n = to_blas_int(n)
                        b_incx = to_blas_int(1)
                        call dscal(b_n, work(ki+n2), vr(1, ki), b_incx)
                    end if
!
                    emax = zero
                    do k = 1, n
                    ...
                    b_n = to_blas_int(n)
                    b_incx = to_blas_int(1)
                    call dscal(b_n, remax, vr(1, jnxt), b_incx)
                    b_n = to_blas_int(n)
                    b_incx = to_blas_int(1)
                    call dscal(b_n, remax, vr(1, ki), b_incx)
                end if
...
!
                        if (work(j) .gt. vcrit) then
                            ...
                            b_incx = to_blas_int(1)
                            call dscal(b_n, rec, work(ki+n), b_incx)
                            vmax = one
                            vcrit = bignum
                        end if
!
...
!
                        call as_dlaln2(.true._1, 2, 1, smin, one, &
                                       t(j, j), ldt, one, one, work(j+n), &
                                       n, wr, zero, x, 2, &
                                       scale, xnorm, ierr)
!
...
!
                    ...
                    b_n = to_blas_int(n-ki+1)
                    b_incx = to_blas_int(1)
                    call dscal(b_n, remax, vl(ki, is), b_incx)
!
                    do k = 1, ki-1
                        vl(k, is) = zero
                    end do
...
!
                    ...
                    b_n = to_blas_int(n)
                    b_incx = to_blas_int(1)
                    call dscal(b_n, remax, vl(1, ki), b_incx)
!
                end if
!
            else
...
!
                        if (work(j) .gt. vcrit) then
                            ...
                            b_n = to_blas_int(n-ki+1)
                            b_incx = to_blas_int(1)
                            call dscal(b_n, rec, work(ki+n2), b_incx)
                            vmax = one
                            vcrit = bignum
                        end if
!
...
!
                        if (scale .ne. one) then
                            ...
                            b_n = to_blas_int(n-ki+1)
                            b_incx = to_blas_int(1)
                            call dscal(b_n, scale, work(ki+n2), b_incx)
                        end if
                        work(j+n) = x(1, 1)
                        work(j+n2) = x(1, 2)
...
!                      ((T(J+1,J) T(J+1,J+1))             )
!
                        call as_dlaln2(.true._1, 2, 2, smin, one, &
                                       t(j, j), ldt, one, one, work(j+n), &
                                       n, wr, -wi, x, 2, &
                                       scale, xnorm, ierr)
!
...
!
                    ...
                    b_n = to_blas_int(n-ki+1)
                    b_incx = to_blas_int(1)
                    call dscal(b_n, remax, vl(ki, is+1), b_incx)
!
                    do k = 1, ki-1
                        vl(k, is) = zero
                        vl(k, is+1) = zero
                    ...
                    if (ki .lt. n-1) then
                        ...
                        b_n = to_blas_int(n)
                        b_incx = to_blas_int(1)
                        call dscal(b_n, work(ki+n), vl(1, ki), b_incx)
                        b_n = to_blas_int(n)
                        b_incx = to_blas_int(1)
                        call dscal(b_n, work(ki+1+n2), vl(1, ki+1), b_incx)
                    end if
!
                    emax = zero
                    do k = 1, n
                    ...
                    b_n = to_blas_int(n)
                    b_incx = to_blas_int(1)
                    call dscal(b_n, remax, vl(1, ki+1), b_incx)
!
                end if
!
            end if
