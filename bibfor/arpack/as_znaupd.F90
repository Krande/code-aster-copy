...
!     APPLIED MATHEMATICS
!     RICE UNIVERSITY
subroutine as_znaupd(ido, bmat, n, which, nev, &
                  tol, resid, ncv, v, ldv, &
...
!     %-------------------------------------------------------%
!
    call as_znaup2(ido, bmat, n, which, nev0, &
...
!
    if (msglvl .gt. 0) then
        ...
        call as_zvout(logfil, np, workl(ritz), ndigit, '_NAUPD: THE FINAL RITZ VALUES')
        call as_zvout(logfil, np, workl(bounds), ndigit, '_NAUPD: ASSOCIATED RITZ ESTIMATES')
    end if
!        %--------------------------------%
