...
! --------------------------------------------------------------------
!
subroutine as_dnapps(n, kev, np, shiftr, shifti, &
                  v, ldv, h, ldh, resid, &
                  q, ldq, workl, workd)
!
...
#include "asterfort/as_dlarfg.h"
#include "asterfort/as_dlartg.h"
...
!
        if (msglvl .gt. 2) then
            call as_ivout(logfil, 1, [jj], ndigit, '_NAPPS: SHIFT NUMBER.')
            call as_dvout(logfil, 1, [sigmar], ndigit, '_NAPPS: THE REAL PART OF THE SHIFT ')
            call as_dvout(logfil, 1, [sigmai], ndigit, '_NAPPS: THE IMAGINARY PART OF THE SHIFT ')
        end if
!
!        %-------------------------------------------------%
...
!
            ...
            if (abs(h(i+1, i)) .le. max(ulp*tst1, smlnum)) then
                if (msglvl .gt. 0) then
                    call as_ivout(logfil, 1, [i], ndigit, &
                               '_NAPPS: MATRIX SPLITTING AT ROW/COLUMN NO.')
                    call as_ivout(logfil, 1, [jj], ndigit, &
                               '_NAPPS: MATRIX SPLITTING WITH SHIFT NUMBER.')
                    call as_dvout(logfil, 1, h(i+1, i), ndigit, '_NAPPS: OFF DIAGONAL ELEMENT.')
                end if
                iend = i
                h(i+1, i) = zero
                goto 40
...
!
        if (msglvl .gt. 2) then
            call as_ivout(logfil, 1, [istart], ndigit, '_NAPPS: START OF CURRENT BLOCK ')
            call as_ivout(logfil, 1, [iend], ndigit, '_NAPPS: END OF CURRENT BLOCK ')
        end if
!
!        %------------------------------------------------%
...
!              %-----------------------------------------------------%
!
                call as_dlarfg(nr, u(1), u(2), 1, tau)
!
                if (i .gt. istart) then
                    h(i, i-1) = u(1)
                    h(i+1, i-1) = zero
...
!
    if (msglvl .gt. 1) then
        call as_dvout(logfil, 1, q(kplusp, kev), ndigit, '_NAPPS: SIGMAK = (E_(KEV+P)T*Q)*E_(KEV)')
        call as_dvout(logfil, 1, h(kev+1, kev), ndigit, '_NAPPS: BETAK = E_(KEV+1)T*H*E_(KEV)')
        call as_ivout(logfil, 1, [kev], ndigit, '_NAPPS: ORDER OF THE FINAL HESSENBERG MATRIX ')
        if (msglvl .gt. 2) then
            call as_dmout(logfil, kev, kev, h, ldh, &
