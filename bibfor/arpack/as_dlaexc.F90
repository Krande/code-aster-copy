...
! ==============================================================
subroutine as_dlaexc(wantq, n, t, ldt, q, &
                     ldq, j1, n1, n2, work, &
                     info)
!
...
#include "asterfort/as_dlanv2.h"
#include "asterfort/as_dlarfg.h"
#include "asterfort/as_dlartg.h"
#include "asterfort/as_dlasy2.h"
...
!
        call as_dlartg(t(j1, j2), t22-t11, cs, sn, temp)
!
...
!
        call as_dlasy2(.false._1, .false._1, -1, n1, n2, &
...
!
            ...
            u(2) = -x(2, 1)
            u(3) = scale
            call as_dlarfg(3, u(1), u(2), 1, tau)
            u(1) = one
            t33 = t(j3, j3)
!
!            PERFORM SWAP PROVISIONALLY ON DIAGONAL BLOCK IN D.
...
!
            ...
            u1(2) = -x(2, 1)
            u1(3) = scale
            call as_dlarfg(3, u1(1), u1(2), 1, tau1)
            u1(1) = one
!
            ...
            u2(2) = -temp*u1(3)
            u2(3) = scale
            call as_dlarfg(3, u2(1), u2(2), 1, tau2)
            u2(1) = one
!
!            PERFORM SWAP PROVISIONALLY ON DIAGONAL BLOCK IN D.
!
...
!
            call as_dlanv2(t(j1, j1), t(j1, j2), t(j2, j1), t(j2, j2), wr1, &
                           wi1, wr2, wi2, cs, sn)
            b_n = to_blas_int(n-j1-1)
            b_incx = to_blas_int(ldt)
            b_incy = to_blas_int(ldt)
...
!
            j3 = j1+n2
            j4 = j3+1
            call as_dlanv2(t(j3, j3), t(j3, j4), t(j4, j3), t(j4, j4), wr1, &
                           wi1, wr2, wi2, cs, sn)
            if (j3+2 .le. n) then
