...
! --------------------------------------------------------------------
!
subroutine as_zgetv0(ido, bmat, initv, n, j, &
...
!
    if (msglvl .gt. 2) then
        call as_dvout(logfil, 1, [rnorm0], ndigit, '_GETV0: RE-ORTHONALIZATION ; RNORM0 IS')
        call as_dvout(logfil, 1, [rnorm], ndigit, '_GETV0: RE-ORTHONALIZATION ; RNORM IS')
    end if
!
...
!
    ...
    if (msglvl .gt. 2) then
        call as_zvout(logfil, n, resid, ndigit, '_GETV0: INITIAL / RESTARTED STARTING VECTOR')
    end if
    ido = 99
!
9000 continue
