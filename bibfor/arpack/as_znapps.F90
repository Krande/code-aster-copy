...
! --------------------------------------------------------------------
!
subroutine as_znapps(n, kev, np, shift, v, &
                  ldv, h, ldh, resid, q, &
                  ldq, workl, workd)
    implicit none
!
...
!
        if (msglvl .gt. 2) then
            ...
            call as_zvout(logfil, 1, sigma, ndigit, '_NAPPS: VALUE OF THE SHIFT ')
        end if
!
        istart = 1
20      continue
...
!
            if (abs(dble(h(i+1, i))) .le. max(ulp*tst1, smlnum)) then
                if (msglvl .gt. 0) then
                    ...
                    call as_zvout(logfil, 1, h(i+1, i), ndigit, '_NAPPS: OFF DIAGONAL ELEMENT.')
                end if
                iend = i
                h(i+1, i) = zero
                goto 40
...
!
    if (msglvl .gt. 1) then
        call as_zvout(logfil, 1, q(kplusp, kev), ndigit, '_NAPPS: SIGMAK = (E_(KEV+P)T*Q)*E_(KEV)')
        call as_zvout(logfil, 1, h(kev+1, kev), ndigit, '_NAPPS: BETAK = E_(KEV+1)T*H*E_(KEV)')
        call as_ivout(logfil, 1, [kev], ndigit, '_NAPPS: ORDER OF THE FINAL HESSENBERG MATRIX ')
        if (msglvl .gt. 2) then
            call as_zmout(logfil, kev, kev, h, ldh, &
