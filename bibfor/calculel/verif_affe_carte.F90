...
!-----------------------------------------------------------------------
    ...
    integer :: nbGrel, iGrel, iCmp, nbCmp, nbOption, nbte, k1, iexi, ient, iad1
    integer :: jnocmp, numgd, joptte, jligrmo, lielSize, iOption, optionNume, joptmod, jvale
    integer :: jmodeloc, nbin, kin, moloc, nbCell, cellNume, iret, typeNume, nbmapb, nbgrma
    integer :: nucalc, iLiel, kma, nec, nbma_verif, nbgdmx, code, decal, ico, kcmp2
...
!   3.2 : on verifie les mailles a verifier (cmp par cmp) :
    do iCmp = 1, nbCmp
        ...
        if (nomgd .eq. 'CAORIE' .and. cmpName .eq. 'ALPHA') cycle
        !
        if (nomgd .eq. 'CINFDI' .and. cmpName(1:3) .eq. 'REP') cycle
        if (nomgd .eq. 'CINFDI' .and. cmpName(1:3) .eq. 'SYM') cycle
        !
        if (nomgd .eq. 'CAGEPO' .and. cmpName .eq. 'TSEC') cycle
        if (nomgd .eq. 'CAGEPO' .and. cmpName .eq. 'HY1') cycle
        if (nomgd .eq. 'CAGEPO' .and. cmpName .eq. 'HZ1') cycle
        if (nomgd .eq. 'CAGEPO' .and. cmpName .eq. 'HY2') cycle
        if (nomgd .eq. 'CAGEPO' .and. cmpName .eq. 'HZ2') cycle
        if (nomgd .eq. 'CAGEPO' .and. cmpName .eq. 'EPY1') cycle
        if (nomgd .eq. 'CAGEPO' .and. cmpName .eq. 'EPY2') cycle
        if (nomgd .eq. 'CAGEPO' .and. cmpName .eq. 'EPZ1') cycle
        if (nomgd .eq. 'CAGEPO' .and. cmpName .eq. 'EPZ2') cycle
        if (nomgd .eq. 'CAGEPO' .and. cmpName .eq. 'EP1') cycle
        if (nomgd .eq. 'CAGEPO' .and. cmpName .eq. 'EP2') cycle
        if (nomgd .eq. 'CAGEPO' .and. cmpName .eq. 'R1') cycle
        if (nomgd .eq. 'CAGEPO' .and. cmpName .eq. 'R2') cycle
        !
        nbmapb = 0
        do kma = 1, nbma_verif
        ...
        if (nbmapb .gt. 0) then
            ...
            do iLiel = 1, min(5, nbmapb)
                ...
                call jenuno(jexnum(mesh//'.NOMMAI', cellNume), nommai)
                valk(1) = nommai
