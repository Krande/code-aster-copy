...
!
        terpl1 = terpl1+( &
                 ( &
s*r8bid3(1)+unsurs*r8bid4(1))**2+(s*r8bid3(2)+unsurs*r8bid4(2))**2+&
                 &(s*r8bid3(3)+unsurs*r8bid4(3) &
                 )**2 &
                 )*poids
!
        termo1 = termo1+( &
...
! --- CARACTERISTIQUES DES FACES DE BORD DE LA FAMILLE 1 ---------------
    call elrefe_info(elrefe=elreff, fami='NOEU', ndim=ndimf, &
                     nno=nnof, nnos=nnosf, npg=npgf, &
                     jpoids=ipoidf, jvf=ivff, jdfde=idfdxf, jgano=jganof)
...
!
    if (elrefb(1:1) .ne. ' ') then
        call elrefe_info(elrefe=elrefb, fami='NOEU', ndim=ndimf, &
                         nno=nno2, nnos=nnos2, npg=npg2, jpoids=ipoid2, &
