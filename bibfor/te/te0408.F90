...
!
    ...
    !   coque_axis  :   MECXSE3
    !
    elem_dkt = (nomte .eq. 'MEDKQU4') .or. (nomte .eq. 'MEDKTR3') .or. &
               (nomte .eq. 'MEDSQU4') .or. (nomte .eq. 'MEDSTR3') .or. &
               (nomte .eq. 'MEQ4QU4') .or. (nomte .eq. 'MET3TR3') .or. &
               (nomte .eq. 'MEC3QU9H') .or. (nomte .eq. 'MEC3TR7H') .or. &
               (nomte .eq. 'MECXSE3')
    !
    ASSERT(elem_dkt)
    !
    ...
    call get_value_mode_local('PCACOQU', valp, valr, iret, retpara_=retpara, nbpara_=2)
    ...
    if (iret .eq. 0 .or. iret .eq. 3) then
        ...
        call fonbpa(nomfon, zk24(jprof), k8bid, MaxPara, nbpara, nompara)
        ! C'est soit :
        !   INST    EPAIS
        !   INST    EXCEN
        !   INST    X   Y   Z
    ...
    do icou = 1, nbcou
        do igauh = 1, npgh
            ...
            if (tempno) then
                ...
                call fointe(' ', zk8(itempf), nbpara, nompu, valpu, tpc, ier)
                if (ier .ne. 0) then
