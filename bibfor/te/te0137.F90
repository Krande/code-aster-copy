...
!
    ...
    real(kind=8) :: valpar(nbres), poids, r, z, nx, ny, tpg, tpg_b, theta, delta_t
    real(kind=8) :: coenp1, sigma, epsil, tz0
    real(kind=8) :: coorse(18), vectt(9)
    integer :: nno, nnos, ndim, kp, npg, ipoids, ivf, idfde, jgano, igeom
    integer :: itemps, iveres, i, j, l, li, iech, iray, itemp, icode, ier, btemp
            ...
            if (option(11:14) .eq. 'COEF') then
                ...
                do i = 1, nno
                    ...
                    vectt(c(ise, i)) = vectt(c(ise, i)) &
                                       +poids*zr(li)*sigma*epsil*(tpg+tz0)**4
                    if (l_stat) then
                        vectt(c(ise, i)) = vectt(c(ise, i)) &
                                           -poids*zr(li)*sigma*epsil*(tpg_b+tz0)**4
                    end if
                end do
            end if
!
