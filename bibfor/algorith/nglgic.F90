...
!
    ...
    real(kind=8), intent(out)       :: fint(nddl), matr(nddl, nddl), siefp(3*ndim+4, npg)
...
! ----------------------------------------------------------------------
    ...
    real(kind=8)  :: t(2*ndim), dgg(2+ndim, 2+ndim)
    real(kind=8)  :: dee(2*ndim, 2*ndim), deg(2*ndim, 2+ndim), dge(2+ndim, 2*ndim)
