...
!
    if (resi) then
        do k = 1, ndimsi
            eps(k) = epsm(k)+deps(k)-kron(k)*(epsthe(2)-kdess*(sref-sechp)-bendo*hydrp)
        end do
    else
        do k = 1, ndimsi
                ...
                do k = 1, 3
                    do l = 1, 3
                        if (t(i, j) .ge. t(k, l)) then
                            ...
                            rtemp2 = rtemp2+vecp(i, 1)*vecp(j, 2)*vecp(k, 1)*vecp(l, 2)*dspdep(4,&
                                     & 4)
                            rtemp2 = rtemp2+vecp(i, 2)*vecp(j, 1)*vecp(k, 2)*vecp(l, 1)*dspdep(4,&
                                     & 4)
                            rtemp2 = rtemp2+vecp(i, 1)*vecp(j, 3)*vecp(k, 1)*vecp(l, 3)*dspdep(5,&
                                     & 5)
                            rtemp2 = rtemp2+vecp(i, 3)*vecp(j, 1)*vecp(k, 3)*vecp(l, 1)*dspdep(5,&
                                     & 5)
                            rtemp2 = rtemp2+vecp(i, 2)*vecp(j, 3)*vecp(k, 2)*vecp(l, 3)*dspdep(6,&
                                     & 6)
                            rtemp2 = rtemp2+vecp(i, 3)*vecp(j, 2)*vecp(k, 3)*vecp(l, 2)*dspdep(6,&
                                     & 6)
