...
!
subroutine pi6046(BEHInteg, typmod, ndim, epsm, epsd_cste, epsd_pilo, &
...
! --------------------------------------------------------------------------------------------------
!  Path-following pred_elas for endo_scalaire
! --------------------------------------------------------------------------------------------------
...
! --------------------------------------------------------------------------------------------------
    ...
    if (.not. lBounds) call utmess('F', 'MECANONLINE_60', sk='ENDO_SCALAIRE')

    ! Compute path-following coefficients
    call piesgv(size(epsm), dtau, BEHInteg%materPara%jvMaterCode, &
                lcquma, vim, epsm, epsd_cste, epsd_pilo, typmod, &
