...
!
subroutine pi6057(BEHInteg, typmod, ndim, epsm, epsd_cste, epsd_pilo, &
...
! --------------------------------------------------------------------------------------------------
!  Path-following pred_elas for endo_fiss_exp
! --------------------------------------------------------------------------------------------------
...
! --------------------------------------------------------------------------------------------------
    ...
    if (.not. lBounds) call utmess('F', 'MECANONLINE_60', sk='ENDO_FISS_EXP')

    ! Compute path-following coefficients
    call piesgv(size(epsm), dtau, BEHInteg%materPara%jvMaterCode, &
                lcmfma, vim, epsm, epsd_cste, epsd_pilo, typmod, &
