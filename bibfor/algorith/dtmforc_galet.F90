...
! --------------------------------------------------------------------------------------------------
!
! dtmforc_galet : Calculates a "discrete model with non-linear elastic behavior's"
!
...
!       Conversion of generalized displacements back to the physical (global) basis
        if (multi_support) then
            call tophys_ms(dplmod, psidel, coedep, depl, depglo)
        else
            call tophys(dplmod, depl, depglo)
        end if
!       Dep GLOB : coordonnée actualisée du noeud  => ajout des "coor_no"
!       Dep LOC  : Longeur actualisée du discret
        nullify (coor_no)
...
!       Conversion of these vectors to the local basis
        call gloloc(depglo, origob, sina, cosa, sinb, cosb, sing, cosg, &
                    deploc(1+(ino-1)*3))
    end do
!
...
!
!   Compute the actual length of the SEG2
    xlong = deploc(4)-deploc(1)-dist1-dist2
!   Axial force (along the local x-axis)
    xforc = 0.0
    flocal(1:3) = 0.d0
    if (xlong <= 0.0) then
!       Behavior type : [nbvale, jprol, jvale]
        call nlget(sd_nl, _NL_FUNC_NAME, iocc=nl_ind, kscal=nomfon, buffer=buffnl)
        call fointe('F', nomfon, 1, ['DX'], [-xlong], xforc, ier)
    end if
    flocal(1) = -xforc
...
!   Internal variables, storage
    if (step .gt. 0.d0) then
        ...
        ASSERT((finish-start) .eq. NBVARINT_DCG)
        ! Force, Déplacement
        vint(start) = xforc
        vint(start+1) = xlong
    end if
