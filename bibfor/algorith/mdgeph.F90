...
! PARAMETRES
    ...
    real(kind=8), intent(in) :: bmodal(neq, nbmode), xgene(nbmode)
...
!
    ...
    real(kind=8) :: tempsl, tempst, zero, un
...
!---------------------------------------------------
!
!    write(ifm,*)'<mdheph> kacce/m/iaux/neq/nbmode/instl/insttl=', &
...
! Affichage/profiling
    if (kprol(1:3) .eq. 'OUI') then
        ...
        if (kprol(1:4) .eq. 'OUIA') then
            write (ifm, *) '<mdgeph> kacce/m/iaux/neq/nbmode/instl=', &
                kaccl, m, iaux, neq, nbmode, instl
            write (ifm, *) '<mdgeph> temps_local/temps_total=', tempsl, tempst
        end if
    end if
    call jedema()
end subroutine
