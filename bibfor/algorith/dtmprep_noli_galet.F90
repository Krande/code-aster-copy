...
! --------------------------------------------------------------------------------------------------
!
! dtmprep_noli_galet : prepare the calculations for a localized nonlinearity of type :
!
!          Discrete model with non-linear elastic behavior.
!
!   This routine adds a single occurence to sd_nl and increments NB_NOLI in sd_dtm
!
...
!   Local variables
    ...
    integer(kind=8)           :: i, n1, ibid, nbdecr, nbnoli, nbfx
...
!
    ...
    real(kind=8)      :: one, axe(3), dist_no1, dist_no2
!
    ...
    character(len=24) :: nl_title, valk(3)
!
...
!       Case with double (or triple) projections (sub-structuring case)
        call utmess('F', 'ALGORITH5_36')
    else
        ASSERT(.false.)
    end if
!
!   Localisation (support nodes) of the non linearity
!   Noeud n°1
    typem = 'NO_NOEUD'
    nbmcl = 2
    limocl(1) = 'GROUP_NO_1'
...
!
!   Filling up the sd_nl with further information regarding the nonlinearity(ies)
    AS_ALLOCATE(vi=ddlcho, size=6)
!   Loop over the detected nonlineary in the current COMPORTEMENT occurence
    call nlsav(sd_nl, _NL_TYPE, 1, iocc=i, iscal=NL_DIS_CHOC_ELAS)
...
!   Other information are read from the user input
    call codent(i, 'D0', intk)
    nl_title = nltype(NL_DIS_CHOC_ELAS)//intk
    call nlsav(sd_nl, _NL_TITLE, 1, iocc=i, kscal=nl_title)
!
!   Get behaviour function
    call getvid(motfac, 'FX', iocc=icomp, scal=nomfon, nbret=nbfx)
    ASSERT(nbfx .eq. 1)
    call nlsav(sd_nl, _NL_FUNC_NAME, 1, iocc=i, kscal=nomfon)
!
!   Calculation of geometrical properties :
!       gap, orientation, local coordinates, distances
...
!   Multi supported systems, filling up of psixdelta for the concerned nodes
...
!   Updating indices for sd_nl and sd_dtm
    call nlsav(sd_nl, _MAX_LEVEL, 1, iscal=nbnoli)
    call dtmsav(sd_dtm, _NB_NONLI, 1, iscal=nbnoli)
!
    call nlget(sd_nl, _NB_DIS_CHOC_ELAS, iscal=nbdecr)
    nbdecr = nbdecr+1
    call nlsav(sd_nl, _NB_DIS_CHOC_ELAS, 1, iscal=nbdecr)
