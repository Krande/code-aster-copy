...
! - Get node for beam
    ...
    if (nbgno .ne. 0) then
        ...
        if (nbNodeBeam .ne. 1) then
            ...
            call jenuno(jexnum(noeuma, beamNodeNume), beamNodeName)
        end if
    end if
...
! - Check rotation dof for pipe/shell elements
    do iNode = 1, nbNode
        call jenonu(jexnom(noeuma, nodeName(iNode)), nodeNume)
