...
!
            if (appui .eq. 1) then
                ...
                if (is_uniform) then
                    ...
                    ! d'un 0.5 correctif
                    surf_cell = sqrt(ddot(b_3, c, b_1, c, b_1))*0.5d0
                    schema = 'NOEU'
                else
                    if (nm .eq. 3) then
                        schema = 'FPG3'
                ...
                do ipg = 1, npg
                    ...
                    if (is_uniform) then
                        ...
                        tan_2 = 0.d0
                        do nn = 1, nm
                            tan_1(1) = tan_1(1)+dff(1, nn)*x(nn)
                            tan_1(2) = tan_1(2)+dff(1, nn)*y(nn)
                            tan_1(3) = tan_1(3)+dff(1, nn)*z(nn)
                            tan_2(1) = tan_2(1)+dff(2, nn)*x(nn)
