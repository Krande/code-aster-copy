...
!
! IN  : MO     : NOM DU LIGREL (FACULTATIF SINON : ' ')
!           SI LE NOM DU LIGREL EST DONNE, ON VERIFIERA QUE LES MAILLES
!           (OU LES NOEUDS) RECUPERES FONT PARTIE DU LIGREL.
...
!
!
!       -- ON VERIFIE QUE LES MAILLES FONT PARTIE DU LIGREL :
!       ----------------------------------------------------
        if (ligrel .ne. ' ') then
            ier = 0
            do ima = 1, nbma
...
!
!       -- ON VERIFIE QUE LES NOEUDS FONT PARTIE DU LIGREL :
!       ----------------------------------------------------
        if (ligrel .ne. ' ') then
