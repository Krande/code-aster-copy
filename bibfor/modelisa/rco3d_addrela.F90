                        ...
                        do jdof=1, dofco
                            ...
                            do l=1, nbterm
                                if( (lisno(l) .eq. nomnoe) .and. (lisddl(l) .eq. dofs(jdof))) then
                                    coer(l) = coer(l) + resu_ptr(i)%rptr(row_index + 6*(ico-1)+jdof) 
                                    found = .true.
                                    exit
                                end if
                            end do
                    ...
                    do i3d=nnco+1, nno-1
                        ...
                        do jdof=1, dof3d
                            ...
                            do l=1, nbterm
                                if( (lisno(l) .eq. nomnoe) .and. (lisddl(l) .eq. dofs(jdof))) then
                                    coer(l) = coer(l) &
                                            + resu_ptr(i)%rptr(row_index + 6*nnco+ 3*(i3d-1)+jdof)
                                    found = .true.
                                    exit
                                end if
                            end do
                            if (.not. found) then
                                ...
                                lisno(nbterm) = nomnoe
                                lisddl(nbterm) = dofs(jdof)
                                coer(nbterm) = resu_ptr(i)%rptr(row_index + 6*nnco+ 3*(i3d-1)+jdof)
                            end if
                        end do
                    end do
                end do
            end do
            !write(*,*) "############ NEW RELATION #################", dofs(idof)
