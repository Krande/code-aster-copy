...
!
! Projection of nodes from entity1 to entity2 when entity1 is 1.5D
!
! --------------------------------------------------------------------------------------------------
!
...
!
    ...
    integer :: iacoo1, iacoo2, ino
    integer :: iabtco, jxxk1, iaconu, iacocf, iacotr
    integer :: ilcnx1
    integer :: iaconb, cellTypeNume, idecal, cellLink1, nbtrou
    aster_logical :: loin, loin2
    real(kind=8) :: dmin, cobary(2)
...
! - Cut cells in SEG2: number of SEG2
...
!
    call wkvect('&&PJXXCO.SEG2', 'V V I', 1+3*nbSeg, iase2)
    zi(iase2-1+1) = nbSeg
...
!      .bt3dnb(3) : ...
!      .bt3dnb(nx*ny*nz) : nombre de seg2 contenus dans la boite(nx,ny,nz)
