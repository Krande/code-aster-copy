...
DataStructure::DataStructure(const DataStructure &other)
    : DataStructure::DataStructure(
          DataStructureNaming::getNewName(other.getName().size()),
...
std::vector<DataStructure::DataStructurePtr>
DataStructure::getDependencies() const {
  return _depsVector;
}
...
void DataStructure::debugPrint(int logicalUnit) const {
  ...
  std::string nameWithoutBlanks = trim(_name);
  try {
    CALLO_UTIMSD(&unit, &niveau, &False, &True, nameWithoutBlanks, &ipos, base,
                 no);
  } catch (...) {
