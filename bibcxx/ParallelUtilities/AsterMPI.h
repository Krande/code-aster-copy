...
public:
  ...
  static void all_gather(const std::map<T1, T2> &in_values,
                         std::map<T1, T2> &out_values,
                         aster_comm_t *_commCurrent = aster_get_current_comm());
  ...
  template <typename T>
  static void bcast(T &value, int root,
                    aster_comm_t *_commCurrent = aster_get_current_comm());
  ...
  // Split communicator
  aster_comm_t *
  splitCommunicator(int color,
                    aster_comm_t *_commCurrent = aster_get_current_comm());
};

//---------------------------------------------------------------------------
template <> inline MPI_Datatype AsterMPI::mpi_type<char>() { return MPI_CHAR; }
template <> inline MPI_Datatype AsterMPI::mpi_type<float>() {
  return MPI_FLOAT;
}
template <> inline MPI_Datatype AsterMPI::mpi_type<double>() {
  return MPI_DOUBLE;
}
template <> inline MPI_Datatype AsterMPI::mpi_type<short int>() {
  return MPI_SHORT;
}
template <> inline MPI_Datatype AsterMPI::mpi_type<int>() { return MPI_INT; }
template <> inline MPI_Datatype AsterMPI::mpi_type<long int>() {
  return MPI_LONG;
}
template <> inline MPI_Datatype AsterMPI::mpi_type<unsigned int>() {
  return MPI_UNSIGNED;
}
template <> inline MPI_Datatype AsterMPI::mpi_type<unsigned long int>() {
  return MPI_UNSIGNED_LONG;
}
template <> inline MPI_Datatype AsterMPI::mpi_type<long long>() {
  return MPI_LONG_LONG;
}
template <> inline MPI_Datatype AsterMPI::mpi_type<bool>() {
  return MPI_CXX_BOOL;
}
...
//---------------------------------------------------------------------------
inline void AsterMPI::all_gather(const std::string &in_values,
                                 VectorString &out_values,
  ...
  aster_mpi_allgatherv(const_cast<char *>(in_values.data()), in_values.size(),
                       MPI_CHAR, _out.data(), pcounts.data(), offsets.data(),
                       MPI_CHAR, _commCurrent);
...
//---------------------------------------------------------------------------
inline void AsterMPI::all_gather(const VectorString &in_values,
                                 VectorString &out_values,
                                 aster_comm_t *_commCurrent) {
  // Get number of procs
  const std::size_t comm_size = getMPISize();
  ...
  const std::size_t local_size =
      std::accumulate(pcounts_lc.begin(), pcounts_lc.end(), 0);
  const std::size_t global_size =
      std::accumulate(pcounts_gl.begin(), pcounts_gl.end(), 0);
  ...
  for (std::size_t p = 0; p < pcounts_gl.size(); ++p) {
    out_values[p] = std::string(_out.begin() + offsets_gl[p],
                                _out.begin() + offsets_gl[p + 1]);
  }
}
//---------------------------------------------------------------------------
template <int length>
inline void
AsterMPI::all_gather(const std::vector<JeveuxString<length>> &in_values,
                     std::vector<JeveuxString<length>> &out_values,
                     aster_comm_t *_commCurrent) {
  // Get number of procs
  const std::size_t comm_size = getMPISize();
  ...
  for (int rank = 0; rank < comm_size; ++rank) {
    ...
    aster_mpi_bcast(retour, length * sizes2[rank], MPI_CHAR, rank,
                    _commCurrent);
    for (int position = 0; position < sizes2[rank]; ++position)
...
void AsterMPI::all_gather(const std::vector<T> &in_values,
  ...
  aster_mpi_allgatherv(const_cast<T *>(in_values.data()), in_values.size(),
                       mpi_type<T>(), recvbuf.data(), pcounts.data(),
                       offsets.data(), mpi_type<T>(), _commCurrent);
...
void AsterMPI::all_gather(const std::vector<T> &in_values,
  ...
  out_values.resize(n);
  aster_mpi_allgatherv(const_cast<T *>(in_values.data()), in_values.size(),
                       mpi_type<T>(), out_values.data(), pcounts.data(),
                       offsets.data(), mpi_type<T>(), _commCurrent);
}
...
void AsterMPI::all_gather(const std::set<T> &in_values, std::set<T> &out_values,
  ...
  std::copy(vect_out.begin(), vect_out.end(),
            std::inserter(out_values, out_values.end()));
}

//---------------------------------------------------------------------------
template <typename T1, typename T2>
void AsterMPI::all_gather(const std::map<T1, T2> &in_values,
                          std::map<T1, T2> &out_values,
                          aster_comm_t *_commCurrent) {
...
void AsterMPI::all_gather(const std::vector<T> &in_values,
  ...
  aster_mpi_allgatherv(const_cast<T *>(in_values.data()), in_values.size(),
                       mpi_type<T>(), out_values->getDataPtr(), pcounts.data(),
                       offsets.data(), mpi_type<T>(), _commCurrent);
}
//---------------------------------------------------------------------------
template <typename T>
void AsterMPI::all_gather(const T in_value, std::vector<T> &out_values,
  ...
  aster_mpi_allgather(const_cast<T *>(&in_value), 1, mpi_type<T>(),
                      out_values.data(), 1, mpi_type<T>(), _commCurrent);
}
//---------------------------------------------------------------------------
template <typename T>
void AsterMPI::all_reduce(const T in_value, T &out_value, MPI_Op op,
                          aster_comm_t *_commCurrent) {
