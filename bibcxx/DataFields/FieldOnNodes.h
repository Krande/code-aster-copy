...
template <> struct AllowedFieldType<JeveuxChar8> {
...
private:
  ...
  typedef std::shared_ptr<SimpleFieldOnNodesValueType>
      SimpleFieldOnNodesValueTypePtr;
...
public:
   ...
   */
  FieldOnNodes(const FiniteElementDescriptorPtr &fed,
  ...
  bool isSimilarTo(const FieldOnNodes<ValueType> &tmp2) {
    ...
    similar =
        (similar && (this->_reference->size() == tmp2._reference->size()));
  ...
  getNodesAndComponentsFromDOF(const bool local = true) const {
    ...
    auto nodeAndComponentsNumberFromDOF =
        _dofDescription->getNodeAndComponentsNumberFromDOF();
   ...
   */
  ASTERDOUBLE norm(const std::string normType,
                   VectorString list_cmp = VectorString()) const {
   ...
   */
  FieldOnNodesDescriptionPtr getDescription(void) const {
...
template <class ValueType>
bool FieldOnNodes<ValueType>::printMedFile(const std::string fileName,
                                           bool local) const {
