...
Table::Table() : DataStructure( ResultNaming::getNewResultName(), 19, "TABLE" ),
          _memoryLocation( JeveuxVectorChar8( getName() + ".TBBA" ) ),
          _description( JeveuxVectorLong( getName() + ".TBNP" ) ),
          _parameterDescription( JeveuxVectorChar24( getName() + ".TBLP" ) ){};
...
const VectorString Table::getParameters() const {
    return _parameters;
};
...
const std::tuple< VectorLong, VectorLong, VectorReal, VectorComplex, VectorString > Table::getColumn( const std::string& param ) const {
    VectorLong exists;
    VectorLong vlong;
    VectorReal vreal;
...
TableOfFunctions::TableOfFunctions() : Table( ResultNaming::getNewResultName(), "TABLE_FONCTION" ){};

void TableOfFunctions::addFunction( GenericFunctionPtr func ) { _vecOfFunctions.push_back( func ); };
