 ...
 * @author Nicolas Sellenet
 */
template <class ValueType> class SimpleFieldOnNodes : public DataField {
private:
...
public:
  ...
  SimpleFieldOnNodes(const std::string name)
      : DataField(name, "CHAM_NO_S"),
        ...
        _allocated(JeveuxVectorLogical(getName() + ".CNSL")), _nbNodes(0),
        _nbComp(0){};
  ...
  SimpleFieldOnNodes(const BaseMeshPtr mesh) : SimpleFieldOnNodes() {
    _mesh = mesh;
  };
  ...
  SimpleFieldOnNodes(const BaseMeshPtr mesh, const std::string quantity,
    ...
    CALL_CNSCRE_WRAP(_mesh->getName().c_str(), quantity.c_str(), &nbComp,
                     tabNames, base.c_str(), getName().c_str(),
                     (ASTERLOGICAL *)&zero);
  ...
  const ValueType &operator()(const ASTERINTEGER &ino,
                              const ASTERINTEGER &icmp) const {
#ifdef ASTER_DEBUG_CXX

    if (!this->hasValue(ino, icmp)) {
      AS_ABORT("DEBUG: Position (" + std::to_string(ino) + ", " +
  ...
  restrict(const VectorString &cmps = {},
